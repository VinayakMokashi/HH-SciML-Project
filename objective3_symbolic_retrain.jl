...
const ROOT        = @__DIR__
# REDIRECTED VARIANT — reads the aggressive-retrain probes and writes ALL outputs
# (symbolic tables + fig9-12) under results/retrain_gca2_20k/, so the canonical
# results/symbolic and figures/fig9-12 (the "before") are never overwritten.
const RUN_DIR     = joinpath(ROOT, "results", "retrain_gca2_20k")
const CALCIUM_DIR = joinpath(RUN_DIR, "calcium")
const SYM_DIR     = joinpath(RUN_DIR, "symbolic")
const FIG_DIR     = joinpath(RUN_DIR, "figures")
mkpath(SYM_DIR); mkpath(FIG_DIR)
