...
Rewritten 2026-08-26 (fourth time), after the option-C conversion, round 8's audit of the cut,
and the commit/snapshot. **Everything through N8 is done; only N9 and N10 remain, and neither
is analysis.** The next session's main job is a full adversarial audit across BOTH manuscripts
and the agreement between them. Copy everything between the fences into a fresh session.
...
THIS SESSION'S MAIN TASK: a FULL ADVERSARIAL AUDIT of both manuscripts and of
whether they agree with each other. I want ZERO discrepancies between
paper/sim2science/main.tex (the 5-page Sim2Science paper) and paper/main.tex
(the archival paper). See "THE AUDIT" below - axis C is the main job.
...
- Only two items remain, both mine, neither analysis:
    N9  send the 5-page draft to Prathamesh (he asked; he will finalise it)
    N10 get his reviewer ID, nominate him, submit both PDFs
...
1. HANDOFF.md:
     Sec 4l  - THE LIVE SECTION. The N0 decision and why (A) was abandoned, the
               figure arithmetic, the first-compile failure (xcolor), and
               ROUND 8 - the audit already done on the cut. Its six findings are
               FIXED; do not re-find them.
     Sec 13  - BINDING vs ADVICE, P1-P9 (both papers), N0-N10 (only N9/N10 are
               open), A10-A12 (archival only), D1/D3/D4. D2 is SKIPPED.
     ...
     Sec 8   - the ELEVEN commands, the current green state, and the note on the
               TWELFTH checker that does not exist yet.
...
B. THE ARCHIVAL PAPER (paper/main.tex, ~2200 lines).
   ...
   paper to (C). Nothing about its framing was touched on 2026-08-26 - the only
   edits were the four listed above. If a finding proposes reframing it, that
   finding is wrong.
   ...
   Both belong with P1 (the AI-tell pass), still open for the archival paper,
   which revisits that title anyway because the mentor called it AI-generated.

C. DO THE TWO PAPERS AGREE? THIS IS THE MAIN JOB and it has never been done
   systematically. The 5-page paper is a DERIVED artifact: it may state nothing
   the archival paper does not, and nothing that contradicts it.
   check_consistency covers 16 repeated-claim groups - that is a FLOOR, not a
   ceiling. Check by hand:
...
--- HOW TO RUN IT (this project learned these the hard way) ---
...
- A confirmed finding does NOT mean its proposed fix is correct. Round 6 had a
  ...
  Verify every remedy separately.
- When you change a claim: update paper/consistency.yaml FIRST, then run
  `python scripts/check_consistency.py` and let it name EVERY site you still
  have to touch, in BOTH manuscripts. Fixing the site I point at and not its
  siblings is this project's single most common failure mode.
- Report findings with a severity and the exact file:line. Do not fix silently,
  and do not bundle unrelated fixes into one pass.

--- BUDGET AND WHEN TO STOP ---
Four days to the deadline and both papers are already shippable. If axis C turns
up nothing MAJOR on the first pass, SAY SO AND STOP rather than manufacturing
minors to look thorough. Tell me the yield honestly - a low number is a result.
...
- NOTHING verifies that a command you use is DEFINED. Eleven green checkers
  passed a document that would not compile (missing xcolor). See Sec 8 - a
  twelfth checker is specced there and not yet written.
