...
- **2026-08 — the full review acted on. See §4.** Reply **sent to Prathamesh on 2026-08-18**
  ...
  **ONE SENTENCE IN IT IS NOW BEHIND THE PAPER.** The email calls
  `[1.63, 2.13]` "a profile-likelihood 95% interval" and then "the interval the data support",
  with no note that it belongs to the **representative seed alone** - and that seed's estimate is
  the LOWEST of the five, so three of the other parametric seeds sit above its upper edge. The
  paper was corrected on this (abstract and Sec 5.5 now say "on the representative seed"); the
  email was not. The claim built on it - "all five of its estimates fall outside the interval" -
  is nevertheless TRUE: the traj-train per-seed values are `1.241, 2.816, 1.607, 1.318, 1.242`
  against `[1.63, 2.13]`, all outside. But seed 3333 misses the lower edge by only `0.023`, which
  is tighter than "all five fall outside" makes it sound. **Do not repeat that phrasing in the
  follow-up**; say the interval is one seed's and quote the pooled `2.092 +/- 0.130` for the
  ensemble.
...
### Blocking submission
2. **Read the rewritten paper yourself.** The thesis changed once, the attribution was
   softened, and then the objective confound was measured and the attribution restated
   again. The checkers cannot tell you whether you agree with what it now says. This is
   the one review no tool here can do.
3. **Check the Sim2Science CFP** — page limit, format, archival status, **and whether it
   is double-blind**. Not cosmetic: double-blind decides whether the repository URL, the
   author block and the acknowledgement may appear at all, and it changes the fresh-repo
   plan (item 13).
4. **Cut to length once the limit is known.** If short-format, the two things that must
   survive are the parametric result and the NODE architecture attribution; the
   window/noise ablations and the appendix are the natural first cuts. The 2026-08-20
   figure round made the paper LONGER (four subfigure pairs became eight standalone
   figures), so this may bite harder than expected.
5. **Fill the three remaining `TODO-` placeholders.** Licences are done. What is left,
   and what each waits on:
   - `TODO-REPOSITORY-URL` (`main.tex:248` and in the availability section) — waits on
     the double-blind answer AND on the fresh repo existing.
   - `TODO-ARCHIVE-DOI` — waits on the repo, then a tagged release deposited to Zenodo.
   - `TODO-COMMIT-SHA` — **last**, after the source is frozen; it must be the commit the
     numbers actually came from.
   Also open: the `[TODO]` at `main.tex:1895` — confirm the Acknowledgements wording with
   the Vizuara team, and the `[TODO - AUTHOR]` citation block at `README.md:283`.
...
2026-08-19), but ONE sentence is now behind the paper: it calls [1.63, 2.13] "the interval the
data support" without noting it is the representative seed's alone, and that seed is the lowest
of five. Do not repeat that phrasing. A follow-up with the corrected PDF is owed if he has the
old build - and he is now several rounds behind it. Awaiting his answer on the thesis lead and
the venue.
