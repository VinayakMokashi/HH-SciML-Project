...
>
> **THE NEXT ACTION IS VINAYAK'S, NOT A SESSION'S: send Prathamesh the note in
> `paper/draft-email-N22-prathamesh.md`, between 23 and 28 Sep.** It is written and
> fact-checked against the committed CSVs; sending costs minutes. He has seen none of E1–E3,
> and two claims in the SUBMITTED paper are contradicted by them (T1, T6). Everything after
> that waits on him or on 29 Sep.
>
...
>
> **BEFORE 29 SEP**
> - **T1 [V] Send Prathamesh `paper/draft-email-N22-prathamesh.md`. WINDOW: 23–28 Sep.**
>   The draft is written, in house style, and every number in it is checked against
>   `results_noiseweighted/paper_macros_e1e3.csv`. It leads with the fact that matters: the
>   submitted paper's decomposition is contradicted at n=28 (T6), in the direction that
>   STRENGTHENS its thesis — not a retraction, but he should not meet it from a reviewer.
>   **NOT before 23 Sep** — his own two reviews are due that day. **NOT after 29 Sep, and this
>   is the real constraint:** if we are accepted, T6 must fix two now-false claims in a
>   ZERO-margin body, inside a camera-ready window whose length we do not even know (T21). A
>   co-author meeting three new experiments for the first time inside that window is the risk.
>   Vinayak was comfortable deferring this (2026-09-21); deferring a week is fine, deferring
>   past the notification is not.
...
> - **T21 [V] FIND THE CAMERA-READY DEADLINE. Numbered late; do it before 29 Sep.** It is not
>   recorded anywhere in this file, and **T6–T10 all live inside that window**. Read
>   `sim2science.com/cfp.html` (the CFP is the authority for this workshop, not the NeurIPS
>   template — that principle is Prathamesh's, §4o). If the window is short, T6's length trade
>   has to be planned BEFORE the decision lands, not after.
>
...
**Rewritten 2026-09-21 (ninth time).** The previous prompt was written on 2026-08-29, said
"branch main", and sent a session to AUDIT the paper before submission. **It was stale for
three weeks and would have sent a session to the wrong branch and the wrong task.** If you are
rewriting this again, delete the old one rather than appending to it.

Copy everything between the fences into a fresh session.

```
We're continuing my Advanced Hodgkin-Huxley x SciML project. Repo:
d:\SciML\bootcamp\Research Project\HH-SciML-Project
Branch: experiments/noise-weighted-loss (HEAD 89177cc, pushed, clean). NOT main.

READ HANDOFF.md FIRST, in this order:
  START HERE (top of file) - the phase and the next action.
  the MASTER TODO LIST (just below it) - T1-T21, owners [V]=me, [P]=Prathamesh,
        [S]=you. Work top-down.
  Sec 4o -> the ★★ RESULTS box at the top of it. This is E1/E2/E3, the n=28
        decomposition, and the 2026-09-12 correction round. Everything in Sec 4o
        BELOW that box is older; the n=5 pilot box is marked SUPERSEDED.
  Sec 9  - gotchas, including the 2026-09-12 block. Non-optional.
  Sec 8  - the twelve checks. ONLY if you touch paper/main.tex.

--- STATE ---
- Sim2Science #89 submitted 30 Aug. NOTIFICATION 29 SEP 2026.
- paper/sim2science/* is FROZEN and under review. Do not edit or audit it. Tag
  sim2science-submitted (3b9e317); it differs from the tag by ZERO printing lines.
- The live manuscript is paper/main.tex (the archival). It carries Sec 4.8,
  "What the spread is made of", and a corrected decomposition throughout.
- E1-E4 closed; W1 and W1b done. NOTHING IS RUNNING. Do not re-run E1/E2/E3:
  resumable, but each re-runs an ~8-minute self-test gate, and results are committed.
- T1 (email Prathamesh) may still be open - ASK ME. The draft is written and
  fact-checked at paper/draft-email-N22-prathamesh.md. Window 23-28 Sep.
- The numbers, so you cannot get them wrong (n=28, true gCa=2.0):
    closure SSE 1.543+-1.237 (rel sd 0.802) | closure chi2 1.925+-0.605 (0.314)
    direct chi2 1.966+-0.132 (0.067)        | direct SSE 1.970+-0.523 (0.265)
    whole gap 11.95x | objective 3.95x on the direct fit, 2.55x on the closure
    representation 3.02x under SSE, 4.69x under chi2 | log-share 45-62% (INTERVAL)
    optimiser 1.00x - ONLY at two parameters; Nelder-Mead cannot run at 337.
  The old "objective is the larger share, representation at most twofold" is DEAD.
- The SUBMITTED paper still states that dead claim at sim2science/main.tex:535-536
  and :734-740. Contradicted at n=28, in the direction that STRENGTHENS the thesis.

--- WHAT I WANT THIS SESSION ---
<fill in from the TODO list. Likely one of:>
  - before 29 Sep: T1 (mine) and T21 (find the camera-ready deadline in the CFP).
  - on/after 29 Sep: read the decision, then T6-T10 (accepted) or T11 (rejected).
  - journal track: T12 framing / T13 n=5-vs-n=28 / T14 language pass / T16 next experiment.
  - compute-only, any time: T16's E7 (~6.7 h unattended; would only strengthen E3).

--- HOW TO RUN EXPERIMENTS HERE ---
- Julia 1.6.7, pinned env. Runs are block-buffered: use file mtimes or the
  incremental CSV as the progress signal, NEVER stdout.
- ALWAYS smoke-test first (NW_SMOKE / E3_SMOKE / PM_NSEEDS=5). It has caught a
  real bug every single time.
- Every new script writes to an ISOLATED tree (results_noiseweighted/,
  results_initdata/). NOTHING under results/ may be written or overwritten.
- ANY harness that re-implements engine code MUST self-test against a published
  number and exit non-zero if it fails. Copy the pattern already in the repo.
- Do not append `; echo "EXIT=$?"` to a backgrounded run - it masks the exit code.

--- IF YOU TOUCH paper/main.tex ---
- Run all twelve checks (Sec 8) AND `python scripts/test_consistency_guards.py`.
- Macros: ADD, never modify. Before regenerating paper/generated/metrics.tex,
  copy it; after, diff the sorted \newcommand sets. "0 existing changed" is the
  invariant that keeps the paper under review from moving.
- If any experiment tree changed: re-run scripts/emit_experiment_macros.py first.
- NOBODY CAN COMPILE (Overleaf 403s), so no checker sees a bad \ref - it prints
  "??". Verify every new \ref target and \val macro resolves, by hand.
- The 28-seed count is written ONLY as \valNwSeeds, never as a word or digit.

--- GROUND RULES ---
- I am the committer. Commits DO carry the Claude co-author line - settled
  2026-09-11, all branch commits have it. Do not silently strip it.
- You may commit and push when I ask, not by default.
- HANDOFF.md is gitignored and snapshot-only. Archive with
  powershell -ExecutionPolicy Bypass -File scripts\snapshot_handoff.ps1 -Push
  and do that from MAIN, then switch back. Do not use 2>&1 on git in PowerShell.
- Do NOT merge this branch into main before 29 Sep: main is the public landing
  page and its README would then contradict the paper under review.
- NEVER write LaTeX or regex through a shell heredoc or python -c. Script to disk first.
- Do not propose re-auditing the submitted paper. That is closed.
- My capacity is limited. Prefer work that costs COMPUTE, not my attention.
```
