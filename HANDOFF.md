...
> **Raise the seed count to ~20–28 on both arms of the noise-weighting experiment.** It is pure
> unattended compute (~3 h/arm), it needs no new code, and it is the one run that converts the
> most promising new result from suggestive into claimable. See the noise-weighting box in §4o.
>
...
> ## ★ THE WORK QUEUE, IN PRIORITY ORDER. START AT THE TOP.
>
> **E1. RAISE THE SEED COUNT ON THE NOISE-WEIGHTING EXPERIMENT. Do this first.**
> The result (â 1.645±0.672 → 2.018±0.482) is promising and **not significant at n=5**
> (F = 1.94 on F(4,4); ~6.4 needed). n≈20–28 on BOTH arms makes it claimable or kills it.
> *No new code.* Extend `NW_SEEDS` in `noise_weighted_ude.jl` and re-run; add the matching seeds
> to the SSE arm so the comparison stays paired. **~8 min/run measured on this box** (not the
> 11 min the paper records), so 2 arms × 20 seeds ≈ 5–6 h unattended. Then re-run
> `scripts/distil_noise_weighted.py`. **This is the single highest-value run available.**
>
> **E2. ISOLATE THE OPTIMISER.** The closure uses Adam+BFGS; the parametric arm uses multi-start
> Nelder–Mead, so the 3.84× residual is still representation **and** optimiser. Re-run the
> 2-parameter fit under Adam+BFGS by extending `parametric_matched_objective.jl`. **Minutes of
> compute — no network, forward solves only.** Converts the paper's last "at most" into a number.
> Honest caveat to state: Nelder–Mead will not run on 337 parameters, so this isolates the
> optimiser only at the two-parameter end.
>
> **E3. DECOUPLE INIT FROM NOISE SEEDS.** Cheaper than the Limitations text implies: the two RNG
> streams are ALREADY separate and merely share an integer — `make_ca_network` uses
> `StableRNG(seed)` (`src/experiment.jl:146-150`) and `gen_noisy_data` a fresh `StableRNG(seed)`
> (`src/hh_core.jl:164`), with `run_ude` passing the same value to both. A 5×5 crossed grid gives
> a real variance decomposition and retires the "joint init-and-data spread" caveat.
>
> **E4. PRINT THE TRUE-PARAMETER LOSS IN THE ARCHIVAL.** Zero compute. The workshop cut prints
> `\valParamFitSseFullTwo` = 39.33 ± 1.26 (p. 5); a grep of `paper/main.tex` finds nothing. It
> converts "we cannot separate this from overfitting" into "we show it IS overfitting", since
> both training losses (35.71, 29.66) sit below it. A referee who computes it themselves and
> finds it missing reads the omission badly.
>
> **E5. THE CLOSURE LADDER.** The paper's own named next experiment. Compute is cheap (~9 h);
> the cost is 3–5 days of implementation, since rungs 1–2 are non-MLP closures needing a new
> basis object. **DESIGN CONSTRAINT: every rung must train under the SAME objective and
> optimiser and distil by the SAME projection**, or the ladder silently reproduces the headline
> comparison's own confound at every step. Claim the TREND, never adjacent rungs — the gCa sweep
> precedent forces that.
>
> **E6. A SECOND SYSTEM.** Biggest item by far, and buys generality of DIRECTION, not of
> threshold. Consider varying the closure's INPUT SET on the existing system first — the paper
> names it as an unvaried axis and it is ~a tenth the cost.
...
> 4. **COMMIT ATTRIBUTION CHANGED MID-PROJECT.** A system-level instruction on 2026-09-08 now
>    requires `Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>` on commits,
>    which **contradicts this file's long-standing "Vinayak is the committer, no Claude
>    co-author line" rule.** The two commits on the experiment branch carry it. **Vinayak has
>    been told and has not yet ruled.** Ask before assuming either way; do not silently strip or
>    silently add it.
