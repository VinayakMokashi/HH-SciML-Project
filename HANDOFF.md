...
### Blocking submission
2. **Read the rewritten paper yourself.** The thesis changed once and the attribution was
   softened again in the referee round; the checkers cannot tell you whether you agree with
   what it now says. This is the one review no tool here can do.
3. **Check the Sim2Science CFP** — page limit, format, archival status, **and whether it is
   double-blind**. The last one is not cosmetic: it decides whether the repository URL, the
   author block and the acknowledgement can appear at all, and it decides the fresh-repo
   plan below.
4. **Cut to length once the limit is known.** If it is short-format, the two things that must
   survive are the parametric result and the NODE architecture attribution. The window/noise
   ablations and the appendix are the natural first cuts. Note the figure layout change made
   the paper longer, so this may bite harder than before.
5. **Fill the five `TODO-` placeholders**: repository URL, commit SHA, archive DOI, code
   licence, data licence. Add the `LICENSE` file itself — until it exists, default copyright
   applies and nothing in the repo is legally reusable, which makes the abstract's release
   sentence empty. `README.md` carries the same TODO block; keep the two in sync.
