...
```
Current state: **all six green — check_numbers OK · check_claims 0 errors, 0 unsupported (31 claims:
4 retired, 27 supported) · lint_prose clean · 286/286 macros match disk · 38 refs, 36 verified,
0 unresolved · tex_sanity clean.**
...
**Paper state:** all four checkers green, 0 unsupported claims, 286 macros, 38 refs verified,
...
### Do this first
0. **Re-upload FOUR files to Overleaf and recompile:** `paper/main.tex`,
   `paper/references.bib`, `paper/generated/metrics.tex`, `paper/generated/metrics.json`.
   `metrics.tex` must land inside `generated/`. The last Overleaf build was stale and showed 47
   blanks plus two `?` citations; see the two Overleaf gotchas in Sec 9. Then send the short
   follow-up to Prathamesh if he has the old build.
...
1. HANDOFF.md, in this order:
     ...
     Sec 13 - next steps. Item 0 is an Overleaf re-upload that may still be outstanding.
...
- Verify with the five commands in HANDOFF Sec 8. All five are currently green:
...
- A blank value in a compiled PDF means the uploaded generated/metrics.tex is STALE, not that
  the source is broken. A "?" citation means references.bib is stale. Re-upload main.tex,
  references.bib and generated/metrics.{tex,json} together.
...
- I compile on Overleaf (you get 403) and paste errors back. I upload figures manually.
