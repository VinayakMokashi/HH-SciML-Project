...
--- STATE ---
- DEADLINE: EXTENDED to 2 Sep 2026 23:59 AoE = 3 Sep 17:29 IST. BUT the OpenReview
  form still showed 30 Aug 17:29 on 28 Aug. The FORM is what stops accepting uploads,
  so work to 30 Aug until you have seen the later date on the form itself. Sec 4n.
...
--- ALREADY SETTLED ON 28 AUG. DO NOT REDO THESE. ---
N11. DONE. The live form has ONE `PDF *` field and NO supplementary field; site
     and venueid verified as ours. The restructure below is REQUIRED, not
     conditional. Sec 4n has the full field inventory.
N12. DONE. Authors = Vinayak Mokashi, Prathamesh Dinesh Joshi, Raj Abhijit
     Dandekar. Licence = CC BY 4.0. Reciprocal reviewer = Prathamesh (remove
     Vinayak from that box on the form, add ~Prathamesh_Dinesh_Joshi1).
     RAJ HAS NO KNOWN OPENREVIEW ID and the form refuses authors without one;
     we ask Prathamesh for it in N20. That blocks pressing Submit, nothing else.
N18. CLOSED. The CFP names our exact two template lines and says nothing about
     footer text. We are compliant. Nothing to change. (Do NOT pass `final` to
     force the workshop footer - neurips_2026.sty:49-51 also sets
     \@anonymousfalse and would print the real byline.)

--- THIS SESSION'S WORK, IN ORDER (Sec 13 has the full rows) ---
...
N14. Build the appendix: hyperparameter table, reproducibility, compute,
     parameter table, remaining ablation details. Ports from the archival's
     Appendix A + B and Table 1. THE CFP PUTS NO PAGE LIMIT ON THE APPENDIX, so
     4-5 pages is Prathamesh's taste, not a rule. The real constraint is the
     other half of that CFP sentence: the five-page BODY must be SELF-CONTAINED
     and reviewers are not obliged to read the appendix. Move detail out of the
     body, never substance.
...
N16. Checklist: move it to body -> references -> CHECKLIST -> appendix (the CFP
     confirms that placement), and rewrite Q8, which currently answers No. THE
     COMPUTE ANSWER IS ALREADY WORKED OUT - see Sec 4n: no logs and no timing
     column exist anywhere, but the parameter-snapshot mtimes measure ~11 min per
     UDE training run, and the hardware is CPU-only, no GPU, 16 GB. State
     hardware as fact, per-run cost as MEASURED with the method named, and the
     run count from the design. Do NOT print a remembered wall-clock total.
     Re-check ALL SIXTEEN answers, not just Q8: the restructure moves section
     numbers and the checklist carries four Sec.~N cross-references. No checker
     reads this file.
...
--- AFTER THE DEADLINE ---
  ...
  ~~D1  regenerate fig2/fig3/fig6.~~ **DONE 2026-08-28 with Vinayak's greenlight.**
      All three print "state propagation". See the Sec 9 entry for the method and
      the before/after manifest. Nothing else moved.
  39 MINOR/NIT findings in paper/audit-2026-08-21-round2.json. None is a blocker.
...
- ~~The code says "state propagation"; the committed PNGs still say "state
  reconstruction".~~ **D1 IS CLOSED, 2026-08-28.** All three overviews were
  regenerated and now print "state propagation"; the apology clause in the
  archival's voltage-only caption (`paper/main.tex`, fig:voltage-only) was
  deleted with them, because it described a figure that no longer exists.
  **The reproducibility result is worth keeping:** fig3 and fig6 come from
  `regenerate_ude_figs`, which LOADS `results/params/ude_*.jld` and does a
  forward solve - no training, no RNG. fig2 needed a real NODE retrain
  (`regen_fig2.jl`, Adam 3000/BFGS 300, ~57 min wall on this machine,
  `bfgs_ok = true`), **and every per-variable panel came back BYTE-IDENTICAL.**
  Only the three `_overview` composites changed, which is exactly the title.
  A 228-file md5 manifest taken before and after confirms: 0 deleted, 9 changed,
  0 results CSV that feeds `metrics_map.yaml` touched, and `extract_metrics
  --check` still reports 322/322 macros matching disk. The four
  `results/calcium/*.csv` that were rewritten compare IDENTICAL cell-by-cell
  (6400 and 512 rows); they differ only in line endings.
