...
- **`arxiv.sty` is not in this repo and never will be** — until somebody downloads it. It is not
  a CTAN package, so Overleaf cannot supply it; it exists only inside the live Overleaf project,
  which is the sole reason the paper has ever compiled there. It is deliberately excluded from
  both artifacts so an overwrite cannot clobber it. **Never delete it**, and if a NEW project is
  ever started, copy it in first or the build dies on line 3. **The durable fix is to get a copy
  into this repo** (Overleaf: `Menu → Download → Source` gives a zip containing it), after which
  the bundle becomes self-contained and a new project would just work — see §13 item 8.
...
0. **Rebuild the upload bundle and recompile on Overleaf:**
   ...
   `overleaf_upload\`, select all four items and **drag them onto the existing project's file
   tree**; overwrite when asked; Recompile. **Do not upload the .zip into the project** — it will
   not extract. Full procedure and every trap are in §7.
...
8. **Get `arxiv.sty` into the repo.** Overleaf: `Menu → Download → Source` yields a zip of the
   project; lift `arxiv.sty` out of it, drop it next to `paper/main.tex`, commit it, and add one
   line to `scripts/make_overleaf_zip.ps1`'s plan. That removes the single external dependency
   this build has, makes the bundle self-contained, and means a fresh Overleaf project (or a
   local tectonic build, §8) would compile without a manual step. Cheap; nobody has done it only
   because the live project already has the file.
...
1. HANDOFF.md, in this order:
     ...
     Sec 7  - the Overleaf upload procedure and its table of traps. Use
              scripts/make_overleaf_zip.ps1 and drag the FOLDER it writes; do not upload the
              zip into the project (never extracts) and do not drag the paper folder itself
              (breaks all three relative paths).
...
- I compile on Overleaf (you get 403) and paste errors back. Build my upload with
  scripts/make_overleaf_zip.ps1, then tell me to DRAG the overleaf_upload/ folder contents onto
  the file tree. Never tell me to upload the .zip into the existing project: Overleaf does not
  extract a zip on upload, it just stores it, and nothing compiles.
