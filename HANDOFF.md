...
~25 more. It found 6.** On `main.tex` alone the rate is **2 findings over 7 areas = 0.29**,
an order of magnitude below round 5. The ninth checker did what it was built to do: the
consistency-debt class it kills is what rounds 4 and 5 were mostly finding, and with those
15 groups enforced mechanically the adversarial rounds have very little left to catch.
**A seventh identical audit of `main.tex` is not a good use of time.**
...
**Updated 2026-08-22: the round-6 audit (§4i) is DONE on both manuscripts and all six
confirmed findings are fixed.** The text is as verified as it is going to get without a
compiled PDF. **Every remaining item below is a human errand — compile, read, trim, submit.
None of them is analysis, and none of them is blocked on anything but you.**

| # | Do this | Waits on | Notes |
|---|---|---|---|
| **S1** | **Compile BOTH documents and read them.** `python scripts/make_sim2science_bundle.py`, then a **NEW** Overleaf project, drag **every** item in `sim2science_upload/`. Compile `main.tex` and `supplementary.tex`. | nothing | **The only open blocker.** The 5-page claim is a word-count estimate; nobody has seen a page count. A new project has no `arxiv.sty` or `neurips_2026.sty` unless you drag them — both are in the bundle. |
| **S2** | **Trim or pad to exactly ≤5 pages excluding references.** | S1 | If long: the retrain subsection and the setup paragraphs go first. If short: add the forecast table from `main.tex` Table 2. |
| **S3** | **Fill `checklist.tex`.** Does not count toward the page limit. | S1 | Ships in the bundle already. |
| **S4** | **Nominate the reciprocal reviewer** — one author, "sufficient relevant expertise and publications". | nothing | Required *at submission time*. A submission-blocker nobody can do for you. |
| **S5** | **Submit to OpenReview.** `NeurIPS.cc/2026/Workshop/Sim2Sci`. All authors listed at submission; **authors cannot be added after review begins**. | S1–S4 | |
| T1 | **Read the compiled FULL paper.** Drag **all five** items from `overleaf_upload/` onto the *existing* Overleaf project. | nothing | Still open, still worth doing — it is the archival version and the supplementary is generated from it. Dragging fewer caused the 2026-08-20 build failure; the canary catches it. |
| T4 | **Create the fresh repo**: copy files into a new `git init`, never clone. | after 29 Aug | **Double-blind confirmed, so this cannot be linked from the submission at all.** Do it for the camera-ready / archival venue. |
| T5–T7 | `TODO-REPOSITORY-URL`, `TODO-ARCHIVE-DOI`, `TODO-COMMIT-SHA` (last). | T4 | **Not needed for the workshop submission** — the cut deliberately withholds all three and says so in Data and Code Availability. Needed for the archival version. |
