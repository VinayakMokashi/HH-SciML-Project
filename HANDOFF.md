...
>
> **Deadline: EXTENDED to 2 Sep 23:59 AoE = 3 Sep 17:29 IST — but the OpenReview form still showed 30 Aug 17:29 on 28 Aug. Work to 30 Aug until the FORM shows otherwise (§4n).**
>
...
checklist, or an unsent email. 27 candidates raised, 20 refuted (74 %), consistent with
round 10's rate.
...
1. **N20. Send the revised draft to Prathamesh.** Drafted at
   `paper/draft-email-N9-prathamesh.md`, **re-reconciled 2026-08-29 by round 11**: its closing
   paragraph and its pre-send check 4 had both gone stale (see the round-11 box). It asks for
   Raj's OpenReview ID and warns him the reciprocal-reviewer role costs two reviews.
   **Recompile and attach the current `sim2science_upload/main.tex` — the bundle was rebuilt
   after round 11's fixes, so any PDF built before 2026-08-29 evening is superseded.**
2. **Raj Abhijit Dandekar's OpenReview profile ID — THE ONLY HARD BLOCKER.** The form refuses
   authors without a profile.
3. **N10. Submit.** Remove Vinayak from the reciprocal-reviewer box, add
   `~Prathamesh_Dinesh_Joshi1`, licence **CC BY 4.0**, Submission Track **Workshop Paper**,
   paste the RENDERED abstract (not the `\val` source), upload the ONE pdf.
4. **F3.** After upload, `git tag -a sim2science-submitted` on the commit whose bundle
   produced the UPLOADED pdf — not a later one — and push the tag.
5. **F4.** Add the FROZEN banner to `paper/sim2science/main.tex`.
6. Post-deadline: A11 (his second round on the archival), P1–P9, A10, A12, D3, T4–T8.
