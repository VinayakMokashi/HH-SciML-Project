...
| N4 | Figures for the main body: the NODE/UDE forecast plot **and one symbolic-recovery graph**. **Only ONE ablation case**, not all three. | N2 | §4k item 4 reads his "trade-off bound" remark as Fig. 7; panel (c) is the one doing argumentative work. Formatting is **P2, P3, P7** — do not restate it here. |
...
| N5 | **Do P1–P9.** | N2 | P1 (AI-tells), P5 (cite the equations, justify additions, notation) and P6 (short Discussion/Limitations) matter most at 5 pages. P7 means the table captions stay where they are. |
...
Rewritten 2026-08-26 (twice): first after the mentor's video (§4k), then again after Vinayak
corrected the framing — the video is **advice**, only the CFP is binding — and after the
A-series was found to be mis-filed (nine of its items apply to both papers and are now the
P-series). Copy everything between the fences into a fresh session and fill in one line.
...
- My mentor sent a VIDEO review on 2026-08-26. HANDOFF Sec 4k has all 64 points.
  It REOPENED the paper's framing. Read it first.
- HIS VIDEO IS ADVICE, NOT SPEC. He is a co-author; I am sole first author and
  corresponding author, and he said four times the call is mine. Only the CFP is
  binding. They already conflict once (table captions) - Sec 13 has the split.
- The old 5-page draft was DELETED on purpose. We rewrite from scratch.
- The full archival paper is DONE, compiled, read, ten checkers green.
- There is a DECISION to make before any drafting: Sec 13, item N0.

--- READ FIRST, in this order ---
1. HANDOFF.md:
     Sec 4k  - THE MENTOR'S VIDEO, all 64 points, with the ADVICE-not-spec banner.
     Sec 13  - the BINDING vs ADVICE split, then P1-P9 (both papers),
               N0-N10 (the 5-page paper), A10-A12 (archival only),
               D1/D3/D4 open decisions. D2 is SKIPPED - do not reopen it.
     Sec 2   - THE THESIS, with the contested-framing banner. The science is NOT
               retracted; only the framing is open.
...
--- HOW TO TREAT THE MENTOR'S FEEDBACK ---
Weigh it, do not execute it. He is experienced and most of it is good, but:
  - one item contradicts the required NeurIPS template (he wants table captions
    below; the template's own sample puts them above, so we keep them above);
  - he argues against the reverse-claim framing on POSITIONAL grounds, then notes
    himself that "NeurIPS workshop accept a preliminary negative result" - so the
    venue is not the objection;
  - his technical argument (a different symbolic algorithm might recover it) is
    only half-answerable from what we have. Sec 4k has the honest reply.
If you think an item is wrong, say so with the reason. Do not quietly drop it and
do not quietly obey it.
...
Do not decide this for me. Lay out the consequences for the 5-page structure and
ask. Note the scope question underneath it: our sanity fit on the TRUE current
returns a = 2.0000 exactly, so the regression is exonerated and the scatter is
upstream in the closure - but we never ran a different symbolic method ON the
learned closure, so "this closure, distilled this way, does not yield the
conductance" is the defensible claim and it is narrower than the paper's current
reach.

--- THE 5-PAGE PAPER: BINDING CONSTRAINTS ---
- Official NeurIPS 2026 template ONLY, already vendored at
  ...
  \usepackage[dblblindworkshop]{neurips_2026} + \workshoptitle{Sim2Science}.
  [sglblindworkshop] silently turns anonymity OFF - never use it.
- 5 pages excluding references. Appendix allowed, 4-5 pages max.
- DOUBLE-BLIND: no names, no affiliations, and NO GITHUB LINK AT ALL.
- checklist.tex filled with justifications; code answer = reproducible from the
  details given, full codebase released upon acceptance.
- Reciprocal reviewer nominated at submission (Prathamesh volunteered).

--- THE 5-PAGE PAPER: HIS SUGGESTED SHAPE (advice) ---
- Abstract 14-15 lines, few numbers: problem (from HH) -> traditional approaches
  -> what we did -> results qualitative then quantitative -> close.
- Introduction 4 paragraphs, last one contributions. Contributions = one headline
  + two lines. Related work <=2 paragraphs. No results in intro or related work.
- Methods: data generation -> preprocessing -> neural ODE -> UDE -> symbolic.
  Cite UDE/SINDy in 1-2 lines; do not explain from first principles. KEEP the
  training details - he called them important.
- Discussion+Conclusion one section; Limitations+Future Work one; <=2 paras each.
- Main body: the NODE/UDE forecast plot AND one symbolic-recovery graph, and only
  ONE ablation case. Sec 4k item 4 reads his "trade-off bound" remark as Fig. 7.
- Then P1-P9 in Sec 13, which apply to this paper as much as to the archival one.

--- THE AI-TELL PROBLEM (his sharpest point) ---
He named the title, the subheading "known physics versus hidden current", and the
sentences "the identifiability vocabulary we use comes from a parallel line of
work" and "we should say plainly what our protocol does not do". His worry is that
a reviewer concludes the paper was agent-written. Write plainly, own every
sentence, vary the rhythm, do not over-exaggerate. He was equally clear that the
tooling itself is fine - he praised the citation audit and the mixed working
style. The objection is to unowned prose, not to using the tools.
...
- consistency.yaml still lists paper/sim2science/main.tex in all 15 groups, and
  ...
  They re-arm the moment that file exists. Do not delete them.
...
- A confirmed audit finding does NOT mean its proposed fix is correct, and a
  refutation is not proof either. One fix was wrong in round 6; one refutation was
  wrong in round 7.
