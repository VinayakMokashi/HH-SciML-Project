...
2. **Venue: Sim2Science (chosen 2026-08-18).** ML4PS is no longer the target.
   **Check the CFP for page limit, format and whether it is archival** — the paper is currently
   well past 4 pages, so how much cutting is needed depends entirely on that, and nothing in this
   repo records it yet. If it is short-format, the parametric result (§2) and the NODE attribution
   (§3) are the two things that must survive the cut.
...
- HANDOFF.md is SNAPSHOT-ONLY: gitignored, but archived into history at each push via
  `scripts/snapshot_handoff.ps1`. Run it as part of committing. It is NOT private —
  the repo is public and history is browsable.
