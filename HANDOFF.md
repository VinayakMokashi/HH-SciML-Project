...
```
Current state (2026-08-19): **all seven green — check_numbers OK (147 numbers, 0 flagged) ·
...
2 review, 0 unresolved · tex_sanity clean · bib_sanity clean (38 entries, 38 unique keys, every
cite key resolves).** `doctor.py` reports 3 FAILs; those are the broken local MiKTeX and are
expected — compilation is Overleaf-only.
...
### Do this first
0. **Rebuild, upload, and READ THE COMPILED PDF.**
   ...
   Recompile. **Do not upload the .zip into the project** — it does not extract. Full procedure
   and every trap in §7.

   As of the last session the build got as far as a clean parse with `arxiv.sty` present, but
   **nobody has yet read a PDF of the current source.** Check, specifically:
   - Fig 5 is the three-panel five-seed sweep (not the old single-seed panel);
   - Figs 6b and 8b carry *different* titles, naming baseline vs aggressive retrain;
   - no blank values anywhere (a blank = stale `generated/metrics.tex`);
   - no `?` citations (`check_bib_sanity.py` says there should be none);
   - Table 2 sits inside the margins after the `\small` fix. If it still overruns, shorten the
     column content rather than shrinking further.
   Expected and NOT to be "fixed": two BibTeX `entry type ... isn't style-file defined` warnings
   (§8), several `Underfull \vbox` messages (float-driven page looseness), and one
   `Underfull \hbox` in `output.bbl`.
