...
> ### THE SCIENCE AND THE WRITING ARE DONE (2026-08-26, after round 9 and the PDF pass).
> **Everything remaining is delivery and one person.** Nine audit rounds, all eleven checkers
> green on both manuscripts, all three artifacts compiled and read. **The compile gate is
> CLOSED:** xcolor fixed, the ~40 checklist errors gone, References lands on **page 5** so
> the body fits inside five pages.
>
> **DO NOT RUN A TENTH AUDIT ROUND.** Not because auditing is exhausted, but because the
> *axes* are. Round 9 found four MAJORs after eight rounds by asking a question nobody had
> asked — *does the derived artifact agree with its source?* — and that axis is now swept.
> The full ledger: prose in `main.tex` (rounds 1–6, §4i says a seventh is a poor use of
> time) · figures and tables (rounds 5, 7) · the cut internally (rounds 6, 8, 9) ·
> **cross-manuscript agreement (round 9)** · **the compiled PDFs (round 9 PDF pass)** ·
> **`checklist.tex` (round 9)**. Every axis anyone has named has now been run.

**ONE RECOMPILE IS OUTSTANDING, and only one.** The bundler fix to the `TODO-` placeholders
changed `sim2science_upload/supplementary.tex`. **`sim2science_upload/main.tex` is
byte-identical to `paper/sim2science/main.tex` and does NOT need recompiling.** So:
recompile **supplementary.tex only**, then N9 (send to Prathamesh), then N10 (nominate,
submit), then F3/F4 (tag, freeze).

> ### WHAT GETS SUBMITTED, AND WHERE IT LIVES. Get this wrong and the paper is desk-rejected.
> | upload | compile this | generated from | notes |
> |---|---|---|---|
> | `main.pdf` (5 pages) | `sim2science_upload/main.tex` | `paper/sim2science/main.tex` | anonymous; `\input{checklist}` after the bibliography |
> | `supplementary.pdf` (29 pp) | `sim2science_upload/supplementary.tex` | `paper/main.tex`, **anonymised at build time** | `\author{}`, `pdfauthor={Anonymous}`, author-overlap clause neutralised, `TODO-` placeholders neutralised |
>
> **EDIT the two SOURCES. UPLOAD the bundle.** `sim2science_upload/` is build output and is
> destroyed on every `make_sim2science_bundle.py` run — a hand-edit there is lost silently.
...
| **N10** | **Nominate Prathamesh as reciprocal reviewer** (he volunteered; get his ID) and submit at `NeurIPS.cc/2026/Workshop/Sim2Sci`. | N9 | All authors listed at submission; none can be added after review opens. Upload BOTH PDFs: `main.pdf` (5 pages) and `supplementary.pdf`. **The CFP wording: "at submission time, authors nominate one author of the paper -- someone with sufficient relevant expertise and publications -- to serve as the paper's reciprocal reviewer."** |
...
Rewritten 2026-08-26 (sixth time), after **round 9** (the cross-manuscript audit), the
**compiled-PDF pass**, and the bundler's `TODO-` fix. **The audit work is finished and the
prompt no longer asks for it.** What remains is delivery, one collaborator, and a
post-deadline list. Copy everything between the fences into a fresh session.
...
READ HANDOFF.md FIRST: Sec 4m (round 9 + the compiled-PDF pass -- THE LIVE
SECTION), Sec 13 (the to-do list and what gets submitted where), Sec 8 (the
eleven checkers and the THREE things they do not check), Sec 9 (gotchas,
non-optional), Sec 3 (numbers + the boxed DOMAIN RULE).

--- STATE: THE PAPERS ARE DONE. DO NOT RE-AUDIT. ---
- DEADLINE: Sim2Science @ NeurIPS 2026, 30 Aug 17:29 IST (= 29 Aug 23:59 AoE).
- NINE audit rounds. All ELEVEN checkers green on BOTH manuscripts. All three
  artifacts (5-page, supplementary, archival) COMPILED and READ in print.
- The compile gate is CLOSED. xcolor fixed, ~40 checklist errors gone,
  References head lands on PAGE 5 -> the body fits inside five pages.
- Sec 4m has the ledger of audit AXES and every one has now been run. A tenth
  round is not the good use of time; delivery is. If you think you have found a
  new axis, say so explicitly rather than re-running an old one.
- Framing (N0) is CLOSED: option (C). Do NOT reopen -- Sec 4l says why (A) was
  drafted in full and then abandoned.
- Evidence base is CLOSED unless I greenlight a run.
- I compile on Overleaf; you get 403. You can NEVER measure a page count. If a
  compile is needed, ask me and I will paste the PDF back.

--- WHAT WE SUBMIT, AND WHERE IT LIVES (get this wrong = desk reject) ---
  main.pdf (5 pages)  <- compile sim2science_upload/main.tex
                         SOURCE: paper/sim2science/main.tex
  supplementary.pdf   <- compile sim2science_upload/supplementary.tex
                         SOURCE: paper/main.tex, ANONYMISED at build time
  EDIT the two sources. UPLOAD the bundle. sim2science_upload/ is build output
  and is destroyed by every make_sim2science_bundle.py run -- a hand-edit there
  is lost silently. NEVER upload a PDF built from paper/main.tex itself: it
  carries the real byline, affiliations and emails.

--- WHAT IS LEFT BEFORE THE DEADLINE ---
  0. Recompile supplementary.tex ONLY (the bundler's TODO- fix changed it;
     sim2science_upload/main.tex is byte-identical to its source and does NOT
     need recompiling). Confirm no "TODO-" appears anywhere in the PDF.
  N9.  Send the 5-page draft to Prathamesh. DRAFTED at
       paper/draft-email-N9-prathamesh.md -- read it before sending, and check
       I actually sent it before assuming.
  N10. Get his OpenReview reviewer ID, nominate him as reciprocal reviewer,
       submit BOTH PDFs at NeurIPS.cc/2026/Workshop/Sim2Sci. ALL authors must be
       listed at submission; none can be added later.
  F3.  git tag -a sim2science-submitted on the commit whose bundle produced the
       UPLOADED PDFs (not a later one), then push the tag.
  F4.  Add the FROZEN banner to paper/sim2science/main.tex. See F-series, Sec 13.

--- AFTER THE DEADLINE (do not let these bleed into the submission window) ---
  A11 Prathamesh's second round on the archival paper.
  P1-P9 presentation pass on the ARCHIVAL paper (still open there). P1 matters
      most: AI-tells, the title he called AI-generated, and the \shorttitle at
      main.tex:194 which states the negative result flatly while the title asks a
      question. The abstract's closing sentence is the same accent. BOTH are
      known and deliberately deferred -- do not "discover" them.
  ...
  T4-T8 fresh repo (COPY files into a new git init, never clone -- the old
      ...
      END of its line. These are also what replaces the TODO- placeholders that
      paper/main.tex still (correctly) holds.
  THREE CHECKER GAPS, all found in round 9, all specced in Sec 8:
      (a) scripts/check_undefined_macros.py -- nothing verifies a command USED
          is DEFINED. Eleven checkers passed a document that would not compile.
  ...
  D1  regenerate fig2/fig3/fig6 from the corrected "state propagation" title.
      TRAINING run -- needs my greenlight; would delete the fig6 caption apology.
  39 MINOR/NIT findings in paper/audit-2026-08-21-round2.json. None is a blocker.
...
- The supplementary is GENERATED from paper/main.tex by
  ...
  to write if any identifying string or TODO- placeholder survives. Never
  hand-maintain a second copy; fix the BUNDLER.
...
- When you change a claim: update paper/consistency.yaml FIRST, then run
  ...
  manuscripts. Fixing the site I point at and not its siblings is this project's
  single most common failure mode. If a group's `require` is an OR, it is a
  FLOOR, not a ceiling -- round 9 added a 17th group for exactly that reason.
...
- NEVER write LaTeX or regex through a shell heredoc, and never through
  ...
  with the Write tool, then run it. (This bit again in round 9.)
...
- The code says "state propagation"; the committed PNGs still say "state
  reconstruction". Deliberate and temporary - Sec 4j, decision D1. Only fig6's
  caption discloses it, because only fig6's caption denies the word.
