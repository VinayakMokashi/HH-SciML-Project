...
**The one blocking task is not analysis, it is a compile.** Upload `sim2science_upload/` to
a NEW Overleaf project and read **where the References head lands** — that is the gauge, not
a page total. Page 5 means the body fits; page 6 means it is six pages. The option-A compile
put it ~85 % down page 5; the (C) revision should put it near 71 %, which leaves room to
restore the forecast figure. The LENGTH DIAL in the preamble of
`paper/sim2science/main.tex` gives the exact steps in both directions and says what must
never be traded for a figure. Then N9 (send to Prathamesh) and N10 (nominate him, submit).
...
| N8 | ~~Bundle, compile, measure.~~ **DONE, twice.** First compile failed on the missing `xcolor` (see §8); second compile put the References head ~75-80 % down page 5, so **the body fits inside five pages.** | -- | **The gauge is where the References head lands, not a page total.** Page 5 = fits. Page 6 = the body is six pages. |
| **N9** | **SEND the 5-page draft to Prathamesh.** He asked for this explicitly and will finalise it. | nothing -- **DO THIS NEXT** | Tell him three things rather than letting him find them: (i) the framing went to **(C)**, which is what his own full-paper draft already does; (ii) the forecast plot and the ablation are in the **supplementary**, not the body, for space, and reviewers are not obliged to read it; (iii) **table captions stay ABOVE** because the template requires it -- this is the one place his advice and a binding requirement collide. |
| **N10** | **Nominate Prathamesh as reciprocal reviewer** (he volunteered; get his ID) and submit at `NeurIPS.cc/2026/Workshop/Sim2Sci`. | N9 | All authors listed at submission; none can be added after review opens. Upload BOTH PDFs: `main.pdf` (5 pages) and `supplementary.pdf`. |
...
| # | Do this | When |
|---|---|---|
| **F1** | **Check whether Sim2Science makes OpenReview submissions publicly visible.** The CFP records it as non-archival with no proceedings but says nothing about submission visibility, and **that one fact sets the stakes**: private-to-reviewers makes later divergence nearly harmless; publicly visible makes the 5-page paper a citable public record. Find out before submitting. | **before N10** |
...
Rewritten 2026-08-26 (fifth time), after round 8, the commit/snapshot, and the F-series
freeze procedure was added to §13. The next session does the full audit **and** works the
remaining to-do list. Copy everything between the fences into a fresh session.
...
TWO JOBS THIS SESSION:
  (1) MAIN: a full adversarial audit of both manuscripts and of whether they
      agree with each other. I want ZERO discrepancies between
      paper/sim2science/main.tex (5-page Sim2Science) and paper/main.tex
      (archival). Axis C below is the real work.
  (2) Then work the REMAINING TO-DO LIST at the bottom of this prompt.

--- DEADLINE AND STATE ---
- DEADLINE: Sim2Science @ NeurIPS 2026, 30 Aug 17:29 IST (= 29 Aug 23:59 AoE).
- Both papers are DONE. All ELEVEN checkers green on both. sim2science_upload/
  is built and anonymity-verified. Everything is committed and pushed.
- Framing (N0) is CLOSED: option (C), an audit of the two-stage
  predict-then-interpret recipe. Do NOT reopen it - Sec 4l says why (A) was
  drafted in full and then abandoned.
- Evidence base is CLOSED unless I greenlight a run.
- I compile on Overleaf; you get 403, so you can NEVER measure a page count.
  The gauge is where the References head lands: page 5 = the body fits, page 6
  = it is six pages. Last compile: ~80% down page 5, so it fits.
- NOT YET VERIFIED: the xcolor fix. The last compile FAILED with ~40 errors
  because neurips_2026.sty defines the checklist answer macros via \textcolor
  and never loads a colour package. Fixed, but nobody has recompiled. Tell me
  to do that early - it gates everything else.

--- READ FIRST, in this order ---
1. HANDOFF.md:
     Sec 4l  - THE LIVE SECTION. The N0 decision and why (A) was abandoned, the
               figure arithmetic, the xcolor failure, and ROUND 8 - the audit
               already done on the cut. Its six findings are FIXED; do not
               re-find them.
     Sec 13  - BINDING vs ADVICE, P1-P9, N0-N10, the new F-series (freezing the
               submitted artifact), A10-A12, D1/D3/D4. D2 is SKIPPED.
     Sec 4g  - why check_consistency exists and the workflow it enforces.
     Sec 4i  - round 6, and the boxed caveat about what the falling rate does
               NOT mean.
     Sec 4j  - round 7. A refutation pass that returns ZERO is a smell.
     Sec 3   - current numbers AND the boxed DOMAIN RULE.
     Sec 8   - the ELEVEN commands, the green state, and the MISSING twelfth.
     Sec 9   - gotchas. Non-optional.
2. paper/consistency.yaml - 16 groups, the repeated-claim ledger.
3. paper/claims.yaml - the claim ledger. The `scope` fields are where
   over-claiming dies; read them before judging any claim.

--- JOB 1: THE AUDIT, THREE AXES ---

A. THE 5-PAGE PAPER (paper/sim2science/main.tex, ~330 body lines).
   Audited ONCE, 2026-08-26, round 8 (Sec 4l): six findings, all fixed. That
   pass read prose and ran mechanical cross-checks. It did NOT re-view the
   figures and did NOT check Table 1 cell-by-cell against the CSVs. DO BOTH.

B. THE ARCHIVAL PAPER (paper/main.tex, ~2200 lines).
   Seven rounds already. Sec 4i concludes a further PROSE audit of the untouched
   regions is a poor use of time, and I agree - do not carpet-bomb it.
   DO audit: (i) everything changed on 2026-08-26 - the claim narrowing in the
   abstract, the contributions and the Discussion, and the NEW
   distillation-scope paragraph in Limitations; and (ii) anything axis C flags.

   IT IS ALREADY OPTION (C). DO NOT "CONVERT" IT. The archival paper was built
   as a recipe audit from the start: question title, the phrase "run the whole
   predict-then-interpret pipeline against ground truth we control" at
   main.tex:309, the two POSITIVE contributions first, and a Conclusion that
   opens positive and only then turns. Discovering that is what moved the 5-page
   paper to (C). If a finding proposes reframing it, that finding is wrong.
   TWO (A)-FLAVOURED ACCENTS SURVIVE, both known, both DEFERRED to the A-series
   after the deadline. Do NOT report them as discrepancies and do NOT fix them:
     - \shorttitle at main.tex:194 flatly states the negative result, while the
       main title asks a question;
     - the abstract's final sentence closes on the (A) claim.
   Both belong with P1, still open for the archival paper.

C. DO THE TWO PAPERS AGREE? THE MAIN JOB, never done systematically.
   The 5-page paper is a DERIVED artifact: it may state nothing the archival
   paper does not, and nothing that contradicts it. check_consistency covers 16
   repeated-claim groups - a FLOOR, not a ceiling. Check by hand:
     - every number in the cut against the archival paper's value for the SAME
       quantity, including WHICH DOMAIN it came from (convex hull vs supervised
       trajectory - Sec 3's boxed DOMAIN RULE; they differ ~6x at gCa=0.4);
     - every QUALIFICATION the archival paper attaches to a claim: does the cut
       carry it or silently drop it? Round 8's MAJOR finding was exactly that
       ("the two spreads are not the same kind of quantity"), and one instance
       means there are probably more;
     - every claim the cut makes: is it in claims.yaml, anchored with
       "% claim: <id>", and INSIDE that claim's `scope` field?
     - the guards: G1 censored rollout, G2 never mean+/-SD for the two
       time-indexed NODE rows, G13 single-seed ablations, form-is-supplied,
       four-of-five-outside-the-interval, representation-share-as-a-bound.

--- HOW TO RUN THE AUDIT (learned the hard way) ---
- Route every candidate to a SEPARATE check that tries to REFUTE it.
- Write refuter prompts NEUTRAL. Sec 4j: "default to refuted" plus a list of
  things that commonly refute findings produced a 15/15 rubber stamp, and one of
  those kills was a real defect. If a refutation pass returns zero, hand-check
  the two or three most substantive kills before believing it.
- A confirmed finding does NOT mean its proposed fix is correct. Round 6 had a
  wrong fix, round 7 a wrong refutation, round 8 a candidate that looked like a
  discrepancy (39.64 vs 39.33) and was not - they are different quantities.
- When you change a claim: update paper/consistency.yaml FIRST, then run
  `python scripts/check_consistency.py` and let it name EVERY site in BOTH
  manuscripts. Fixing the site I point at and not its siblings is this
  project's single most common failure mode.
- Report findings with a severity and the exact file:line. Do not fix silently.
- If axis C turns up nothing MAJOR on the first pass, SAY SO AND STOP rather
  than manufacturing minors. A low yield is a result; tell me honestly.

--- JOB 2: THE REMAINING TO-DO LIST ---
Ordered so each item's prerequisite comes first. Sec 13 has the detail on each.

BEFORE SUBMISSION (30 Aug 17:29 IST):
  0. RECOMPILE sim2science_upload/ on Overleaf and confirm the xcolor fix
     cleared the ~40 checklist errors. Nothing else matters if this fails.
  F1. Find out whether Sim2Science makes OpenReview submissions PUBLICLY
      VISIBLE. Non-archival is already known; visibility is not, and it sets
      how much a later archival-vs-workshop divergence matters.
  N9. Send the 5-page draft to Prathamesh. Tell him three things rather than
      letting him find them: (i) the framing went to (C), which is what his own
      full-paper draft already does; (ii) the forecast plot and the ablation are
      in the SUPPLEMENTARY, not the body, for space - and the CFP says reviewers
      are not obliged to read it, so they are AVAILABLE, not guaranteed seen;
      (iii) table captions stay ABOVE because the template requires it.
  N10. Get his reviewer ID, nominate him as reciprocal reviewer, submit BOTH
      PDFs (main.pdf and supplementary.pdf) at NeurIPS.cc/2026/Workshop/Sim2Sci.
      All authors must be listed at submission; none can be added later.
  F3/F4. AT submission: tag the exact submitted commit
      (`git tag -a sim2science-submitted`), push the tag, and add the FROZEN
      banner to paper/sim2science/main.tex. See the F-series in Sec 13.

AFTER THE DEADLINE (do not let these bleed into the submission window):
  A11. Prathamesh's second round of feedback on the archival paper.
  P1-P9. The presentation pass on the ARCHIVAL paper - still open there. P1
      (AI-tells, including the title he called AI-generated and the \shorttitle)
      matters most.
  A10. Decide the archival destination: arXiv preprint, a journal, or both.
  A12. Restore whatever the 5-page paper dropped for anonymity or length. D4
      (the author-overlap disclosure) is MANDATORY, not optional.
  D3. Add neurips_2026.sty + checklist.tex to the LICENSE third-party carve-out.
  T4-T8. Fresh repo (COPY files into a new `git init`, never clone - the old
      history carries a private Overleaf URL and verbatim mentor quotes), repo
      URL, Zenodo DOI, commit SHA, README citation block. Law I applies: a URL,
      a DOI and a SHA all carry digits, so each needs a `% numok:` at the TRUE
      END of its line.
  12th checker. Write scripts/check_undefined_macros.py - Sec 8 specs it. Eleven
      green checkers passed a document that would not compile.
  D1. Regenerate fig2/fig3/fig6 from the corrected "state propagation" title.
      This is a TRAINING run and needs my greenlight; it would let the fig6
      caption apology be deleted.
  39 MINOR/NIT findings in paper/audit-2026-08-21-round2.json. None is a blocker.

DEFERRED WITH REASONS (do not start these without asking me):
  closure-ladder experiment; multiple-trajectory positive control;
  multiple-shooting / curriculum horizon. All are follow-up-paper scope.
...
- NEVER write LaTeX or regex through a shell heredoc, and never through
  `python -c` either. Backslashes get eaten - it happened three times in one
  session. Write the script or patch to disk with the Write tool, then run it.
- consistency.yaml `require` regexes must use \s+ between words, never a literal
  space: the checker joins context with ' '.join(), so a qualifier straddling a
  line break arrives as "for this   closure" and a literal space never matches.
- NOTHING verifies that a command you use is DEFINED. See Sec 8.
