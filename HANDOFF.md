...
> ## START HERE — 2026-09-09. THE PAPER IS SUBMITTED; THE PROJECT IS NOW IN EXPERIMENTS.
> **§4o is the live section.** §4l and §4n are history; read them only for the *why* behind a
> settled decision. The §4x sections run NEWEST FIRST: **4o** → 4n → 4m → 4l → 4k → 4j → 4i →
> 4h → 4g…4b. §4–4g are chronological history.
>
> ### THE PHASE CHANGED ON 30 AUG. READ THIS BEFORE ANYTHING ELSE.
> **The Sim2Science paper is SUBMITTED (#89) and FROZEN.** Everything below about audits,
> checkers and page limits is now HISTORY — kept because the decisions are reusable, not
> because there is anything left to do to that paper. **Do not audit it. Do not edit it.**
>
> | | |
> |---|---|
> | **What phase are we in?** | Waiting for the Sim2Science decision on **29 Sep 2026** while running experiments for the ARCHIVAL/journal version. |
> | **Which branch?** | **`experiments/noise-weighted-loss`** carries the new work. `main` carries the submitted paper and the HANDOFF snapshots. |
> | **What is frozen?** | `paper/sim2science/*` — tagged `sim2science-submitted` (commit `3b9e317`). It has a FROZEN banner. Branch from the tag if it ever must change. |
> | **What is live?** | `paper/main.tex` (the archival) and the experiment scripts. |
> | **Deadline pressure?** | **NONE.** Nothing is due. The journals under consideration are rolling. |
>
> ### THE NEXT ACTION, IF YOU ONLY DO ONE THING — UPDATED 2026-09-12
> **Nothing is running and nothing is half-done.** E1–E4 are closed, W1 is written, and the
> 2026-09-12 round measured the last missing cell and **retired the "at most twofold" bound**
> from the archival and README (commit `89177cc`, pushed). All twelve checks are green; **the
> submitted workshop paper is untouched — zero printing lines changed against the tag.**
>
> **THE NEXT ACTION IS VINAYAK'S, NOT A SESSION'S: send Prathamesh the ★★ RESULTS box (§4o).**
> He has seen none of E1–E3, and two claims in the SUBMITTED paper are now contradicted by
> them (T1 below). Everything after that waits on him or on 29 Sep.
>
> ## ☐ THE MASTER TODO LIST — 2026-09-12. OWNER IN BRACKETS. WORK TOP-DOWN.
> **Nothing here is urgent before 29 Sep except T1.** Owners: [V] Vinayak, [P] Prathamesh,
> [S] a Claude session, [V+P] a joint decision.
>
> **BEFORE 29 SEP**
> - **T1 [V] Send Prathamesh the ★★ RESULTS box.** Lead with the fact that matters for the
>   review cycle: the submitted paper's decomposition is contradicted at n=28 (T6), in the
>   direction that STRENGTHENS its thesis — so it is not a retraction, but he should not be
>   surprised by it if a reviewer asks about seed count or the objective/representation split.
> - **T2 [everyone] Do not edit `paper/sim2science/*`.** Frozen, under review, tag
>   `sim2science-submitted` (`3b9e317`). Branch from the tag if it ever must change.
> - **T3 [P] His two reciprocal Sim2Science reviews are due 23 Sep.** Do not ask him about the
>   assigned papers — confidential.
> - **T4 [P→V] Wait for his internal-tool feedback.** He said he would share it; it should
>   shape the choice between E5, E6 and E7 (T16).
> - **T5 [V] Do NOT merge `experiments/noise-weighted-loss` into `main` before 29 Sep.** `main`
>   is what the public repo's landing page shows. Merging publishes a README that contradicts
>   the paper under double-blind review. The branch itself is pushed and safe where it is.
>
> **ON 29 SEP — THE DECISION**
> - **T6 [V+S] If ACCEPTED — the camera-ready must fix two now-false claims in the cut.**
>   `paper/sim2science/main.tex:535-536` *"a matched-objective control charges the larger share
>   of that gap to the loss function"* and `:734-740` *"The residual twofold bounds the
>   representation's own contribution rather than measuring it"*. At n=28 the representation
>   owns 45–62 % of the log gap and exceeds the bound under either objective. **Branch from
>   the tag. THE BODY MARGIN IS ZERO** — enforce `new_chars <= old_chars` mechanically; one added
>   sentence pushes References onto page 6. Then rebuild with `make_sim2science_bundle.py`.
> - **T7 [S] If accepted — restore the author-overlap disclosure (A12). MANDATORY**, not optional.
> - **T8 [S] If accepted — fix the bundler (A13):** it DELETES the disclosure instead of blinding it.
> - **T9 [V] If accepted — build the clean public repo from the tag by COPYING files into a fresh
>   `git init`. NEVER `git clone`** — this repo's history carries every HANDOFF snapshot.
>   Discharges the code-release promise in checklist Q5/Q13.
> - **T10 [V+P] If accepted — decide whether E1–E3 go into the camera-ready.** Read the CFP's
>   camera-ready rules first; the margin is zero, so this is a length trade, not an addition.
> - **T11 [S] If REJECTED — fold the reviews into the archival (`paper/main.tex`).**
>
> **THE JOURNAL VERSION (after 29 Sep; Prathamesh prefers a Bio-ML journal)**
> - **T12 [V+P] W2, the framing pass.** The decomposition in the abstract, introduction and
>   conclusion is ALREADY corrected (`89177cc`). What remains is the THESIS: the evidence now says
>   *the loss you choose creates an init×noise interaction, the likelihood removes it, the
>   objective and the representation contribute comparably, and the optimiser is negligible
>   where it can be isolated*. That is a change to the paper's argument. **Not from a session.**
> - **T13 [V+P] Decide whether the journal version restates the n=5 numbers at n=28
>   throughout.** Several archival sites still describe the five-seed results in words — "about
>   six times wider", "roughly forty per cent", "two-fifths of the mean" (grep for them). They
>   are CORRECT for five seeds and are labelled as such where they meet §4.8, but a journal
>   version should probably lead with n=28 everywhere. A W2 decision.
> - **T14 [S] P1, the language pass on the archival — OWED; we told Prathamesh we would.** Carry
>   the review-3 register fixes from the cut into `paper/main.tex`. Grep, do not trust old line
>   numbers: `This two-stage recipe`, `Nor is it under-training`. Method:
>   `Paper Writing Skills/references/writing-craft.md` §4.7 and
>   `examples/hh-sciml/humanized-passage.md`. Never invent an owned expectation.
> - **T15 [V] Verify the npj Systems Biology and Applications APC.** Never confirmed.
> - **T16 [V+P] Choose the next experiment, after T4 and the reviews.** In rising cost: **E7**
>   (re-run the E3 grid on non-published data seeds; ~6.7 h compute, zero attention; would only
>   strengthen E3) · **vary the closure's input set** (~a tenth of a second system) · **E5** the
>   closure ladder (3–5 days of implementation) · **E6** a second system. A journal referee's
>   biggest objection will be one system, one current, synthetic data. Do not start days of
>   work before the venue's bar is known.
>
> **HOUSEKEEPING (whenever)**
> - **T17 [V] Prune decision on ~75 MB of `grid_*.csv` probes** before the fresh-repo step.
>   Only `probe_*.csv` feeds any analysis; grids regenerate from the committed `.jld` snapshots.
> - **T18 [S] D3: add `neurips_2026.sty` and `checklist.tex` to the `LICENSE` carve-out.**
> - **T19 [V] Attribution.** Settled as "keep the co-author line" (2026-09-11). If ever reversed,
>   it is a rewrite of every commit on the experiment branch, not one.
> - **T20 [S] The remaining §13 housekeeping (T4–T8) and presentation items P2–P9.**
>
> **DO NOT RE-RUN E1/E2/E3.** All three are resumable and would skip every fit, but each
> re-runs its ~8-minute self-test gate by design. Their outputs are committed.
>
> ### HISTORICAL, FOR CONTEXT ONLY — the submitted paper's state at freeze
> Twelve audit rounds. **All SIXTEEN checks were green** (§8) and the bundle was byte-identical
> to its source. **DO NOT START ANOTHER AUDIT ROUND ON THE SUBMITTED PAPER.** The stopping rule
> was agreed and the last two rounds met it: round 10 confirmed 4 of 9 (44 %), round 11 **7 of
> 29 (24 %)**, round 12 **2 of 22 (9 %)**. At a 9 % true-positive rate another round is likelier
> to damage than improve. The axes are exhausted: body prose · figures and tables · the cut
> internally · cross-manuscript · the compiled PDFs · `checklist.tex` (four passes) ·
> mentor-ask coverage · the paragraph-by-paragraph lift audit.
>
> ### THE DATES THAT MATTER NOW
> Submitted **30 Aug 2026** (#89). Reviewing opened **8 Sep**. Reviews due **23 Sep**.
> **NOTIFICATION 29 SEP 2026.** Workshop **11–13 Dec 2026, Paris**.
> Everything about the submission deadline, Raj's OpenReview profile and the reciprocal-reviewer
> nomination is DISCHARGED — see the submitted box in §4o if you need the detail.
>
> ### THE FILE MAP AS OF NOW. Getting this wrong is how a session wastes a round.
> | what | path | note |
> |---|---|---|
> | **THE SUBMITTED PAPER — ✗ FROZEN, DO NOT EDIT** | `paper/sim2science/main.tex` + `checklist.tex` | Under review. Tagged `sim2science-submitted`. Has a FROZEN banner. Branch from the tag if it truly must change. |
> | **THE ARCHIVAL — ✓ THIS IS WHAT YOU EDIT NOW** | `paper/main.tex` | Real byline, 3 authors. The journal version grows from here. |
> | **NEW EXPERIMENTS — ✓ live** | `noise_weighted_ude.jl`, `scripts/distil_noise_weighted.py`, `results_noiseweighted/` | On branch `experiments/noise-weighted-loss`. |
> | BUILD OUTPUT | `sim2science_upload/` | Destroyed and rewritten by every `make_sim2science_bundle.py` run. Only needed if the submitted PDF is ever rebuilt. |
>
> **NEVER upload a PDF built from `paper/main.tex`:** it carries the real byline and breaks
> double-blind. **ONE pdf is submitted, not two** — body ≤5 pp + references + checklist +
> appendix A–E, all in that one file. `sim2science_upload/supplementary.tex` is still generated
> but is **NOT submitted**; do not delete that path (§4o, A13).
>
> ### THE COMPILED PDF WAS READ END TO END ON 2026-08-29. EVERYTHING BELOW IS NOW MEASURED.
> **This is the first time the finished artifact has been checked against what the source
> claimed, and it all held.** 19 pages:
>
> | pages | what | CFP status |
> |---|---|---|
> | **1–5** | body, Secs 1–5, ending at line 178 | **the five-page limit — MET** |
> | 6–7 | References, head opens page 6 at line 179 | excluded |
> | 8–14 | checklist, all 16 answered | excluded |
> | 15–19 | Appendix A–E | excluded |
>
> Order is body → references → checklist → appendix, which is the order the CFP names verbatim.
> **THE MARGIN IS STILL ZERO: one added sentence pushes the body onto page 6.** Nobody in a
> Claude session can compile (Overleaf 403s), so **never estimate length in source lines** —
> they mis-price captions, displayed equations and float graphics. Enforce edits with the
> character gate instead (`new_chars <= old_chars` inside the body) and verify with:
> `diff <(git show <commit>:paper/sim2science/main.tex | sed -n '1,770p') <(sed -n '1,770p' paper/sim2science/main.tex)`
>
> **Also confirmed in print, each of which had only ever been asserted from source:** the
> byline reads "Anonymous Author(s)" · the footer reads "Submitted to the Sim2Science workshop
> at the 40th Conference…" · citations render with **no coloured link boxes** · Fig. 4's
> in-image title reads **"UDE : state propagation"** (the D1 fix, finally visible) · the loss
> equation is inline and **the document contains no numbered equation**, which is why
> checklist Q4 must not cite one · all three "We expected" sentences are on the page.
...
## 4o. STATE AS OF 2026-08-29. READ THIS FIRST; IT SUPERSEDES THE PLANNING IN §4n.

> # ★★ RESULTS, 2026-09-10/11. E1, E2 AND E3 ARE DONE. READ THIS BEFORE ANY §4o PROSE BELOW. ★★
> **Branch `experiments/noise-weighted-loss`, commits `5d60e21` (E1), `fa9bcad` (E2),
> `64def0e` (E3). ALL PUSHED.** Nothing under `results/` or `figures/` was touched by any of
> them, so no published macro can have moved — verified with `git status` after each commit.
> **Every n=5 number in the noise-weighting box further down this section is SUPERSEDED.**
>
> ### THE ONE-SCREEN SUMMARY
>
> | quantity | published (n=5) | NOW (n=28) |
> |---|---|---|
> | SSE-trained closure, â | 1.645 ± 0.672 (rel sd 0.408) | **1.543 ± 1.237 (rel sd 0.802)** |
> | chi2-trained closure, â | 2.018 ± 0.482 (rel sd 0.239) | **1.925 ± 0.605 (rel sd 0.314)** |
> | direct 2-param fit, â | 2.092 ± 0.130 (rel sd 0.062) | **1.966 ± 0.132 (rel sd 0.067)** |
> | direct fit under **SSE**, â | 2.478 ± 0.489 (rel sd 0.197) | **1.970 ± 0.523 (rel sd 0.265)** |
> | whole gap (closure SSE / direct chi2) | 6.57× ("six times") | **11.95×** |
> | objective, on the direct fit | 3.2× ("threefold") | **3.95×** |
> | objective, on the closure | not measured — ASSUMED equal to the above | **2.55×** — the assumption fails |
> | representation under SSE | 2.07× ("at most a further twofold") | **3.02× — the bound is EXCEEDED** |
> | representation under chi2 | 3.84× | **4.69×** |
> | representation's share of the log gap | "about two-fifths", "less than the objective's" | **45–62 %** — comparable, an interval |
> | optimiser effect | unmeasured, "at most" | **1.0000× at two parameters** |
>
> ### E1 — THE NOISE-WEIGHTING EFFECT IS REAL. `5d60e21`
> ```
> F test (var sse / var chi2)   F(27,27) = 4.177   p = 0.0002
> Brown-Forsythe (robust)       W = 4.389          p = 0.041
> paired Wilcoxon on |a-2.0|    T = 304            p = 0.010
> ```
> The pilot's F = 1.94 on F(4,4) needed ~6.4 and got nowhere; at n=28 all three tests clear.
> **Quote carefully: the F and Brown-Forsythe tests are about SPREAD; the paired Wilcoxon is
> about ACCURACY.** They answer different questions and the paper's claim is about spread.
>
> **TWO THINGS n=5 COULD NOT HAVE SHOWN, AND BOTH MATTER TO THE WRITE-UP:**
> 1. **THE PUBLISHED FIVE SEEDS WERE A BENIGN DRAW.** SSE relative spread nearly DOUBLED,
>    0.408 → 0.802, when the other 23 arrived. The parametric arm barely moved (0.062 →
>    0.067), so this is specific to the closure, not an n-effect. **Three of 28 SSE seeds
>    recover a NEGATIVE conductance** (−1.32, −0.92, −1.09) — not a wide estimate, an
>    impossible one. Zero chi2 seeds and zero parametric seeds do. Only 12/28 SSE seeds land
>    within 25 % of truth, against 17/28 chi2 and **28/28** parametric.
> 2. **chi2 FORECASTS BETTER, which at n=5 looked like a slight cost.** 0.363 ± 0.192 mV (SSE)
>    against 0.233 ± 0.140 (chi2), better in 20/28 seeds, paired p = 0.0033.
>
> ### E2 — THE OPTIMISER CONTRIBUTES NOTHING AT TWO PARAMETERS. `fa9bcad`
> Same 2 parameters, same chi2, same 28 seeds, same 3 starts, the CLOSURE'S optimiser
> (Adam 1e-2 × 5000 + BFGS × 300). 168 fits.
> ```
> Adam+BFGS   1.966 ± 0.132   rel sd 0.067098
> Nelder-Mead 1.966 ± 0.132   rel sd 0.067098      B/C = 1.0000
> ```
> Every one of 168 cells lands on the Nelder–Mead optimum (worst rel diff 3.5e-06; worst
> WITHIN-seed range across a seed's six cells 3.5e-08). All 168 BFGS polishes converged.
> **So A/B = A/C = 4.68× and the paper's last "at most" becomes a measured number.**
>
> > **THE CAVEAT MUST TRAVEL WITH THE NUMBER.** Nelder–Mead cannot run on 337 parameters, so
> > this isolates the optimiser **only at the two-parameter end**. It shows that where the
> > optimum is unique and well conditioned the choice of optimiser does not matter. It does
> > **NOT** show Adam contributes nothing to the closure's scatter — and at 337 parameters
> > representation and optimiser may not be separable even in principle, because the flexible
> > representation is precisely what creates the multi-optimum landscape. **Do not let a
> > write-up phrase this as "the optimiser is ruled out".**
>
> ### E3 — THE SPREAD IS THE DATA DRAW, AND chi2 WORKS BY KILLING THE INTERACTION. `64def0e`
> 5×5 crossed init × data, both objectives, 50 cells. The published runs are the DIAGONAL.
> ```
>                    SSE arm                      chi2 arm
>   initialisation   -0.022    0.0%   F=0.474 p=.754    0.017    5.6%   F= 3.68 p=.026
>   data draw         0.286   57.7%   F=7.824 p=.0011   0.250   84.0%   F=41.35 p<.0001
>   interaction       0.209   42.3%                     0.031   10.4%
>   TOTAL             0.429                             0.253
> ```
> **The initialisation contributes ~nothing** (negative estimate, F = 0.47 < 1). The closure is
> not finding different optima on identical data; it is hostage to which noise realisation it
> was handed. **That retires the "joint init-and-data spread" caveat with a direction.**
>
> **AND THE SECOND FINDING IS THE ONE WORTH THE COMPUTE — E1 could not have produced it.**
> SSE → chi2 moves the components: init 0.000 → 0.017, data 0.286 → 0.250 (−13 %),
> **interaction 0.209 → 0.031 (−85 %)**. So noise-weighting does NOT make the closure less
> sensitive to the data. It stops init and data from INTERACTING — under the unweighted loss a
> given initialisation is good or bad depending on which noise draw it meets. **E1 said chi2
> halves the spread; E3 says which half and why.**
>
> **The interaction reading is licensed, not assumed.** With one run per cell, interaction is
> normally confounded with run-to-run irreproducibility — so it can only be called interaction
> if training is deterministic given (init, data). It is, and the run PROVES it: all ten
> diagonal cells reproduce their E1 counterparts to **0.000e+00**, in forecast RMSE and in â.
>
> **CAVEAT, AND IT CUTS SAFELY.** The grid's five data seeds are the published five, which E1
> showed were benign: SSE rel sd 0.374 across all 25 cells against 0.802 over 28 seeds. **The
> data component here is an UNDERSTATEMENT**; a less benign sample strengthens the conclusion.
> Also worth recording: the diagonal (0.408 / 0.239) tracks the full grid (0.374 / 0.245)
> closely — **the published five-seed sampling was not a biased estimate of the TOTAL spread;
> it simply could not decompose it.**
>
> ### WHAT ALL THIS DOES TO THE PAPER'S STORY — REVISED 2026-09-12
> The thesis moves from *"flexible closures lose identifiability"* (a caution) to a mechanism
> plus a partial prescription: **the loss you choose creates an interaction between
> initialisation and noise; the likelihood removes it; the objective and the representation
> contribute COMPARABLY to what remains; and the optimiser is negligible at the end where it
> can be checked.**
>
> **THE OLD SPLIT IS DEAD — do not revive it.** The n=5 paper said the objective was "the larger
> share" and the representation "at most a further twofold". That split was ARITHMETIC: it
> measured the objective on the direct fit and assumed the same factor applied to the closure.
> Commit `89177cc` measured the direct fit under SSE at n=28, completing the 2×2, and the
> assumption fails (objective 3.95× on the direct fit, 2.55× on the closure). The
> representation now owns 45–62 % of the log gap. **Quote that as an INTERVAL** — the product
> identity holds along both orderings, and they give different splits. Never quote one ordering
> alone, and never the whole-gap ratio, which is the two effects multiplied.
>
> ### THE CORRECTION ROUND (2026-09-12, commit `89177cc`) — AND THE ERROR IT CORRECTED
> **W1 (`aea9647`) left the archival contradicting itself, in the abstract.** The abstract and
> Contributions still said "at most a factor of two"; the new §4.8 said 4.69×. **Every checker
> passed**, because `consistency.yaml` matches PHRASES, not MAGNITUDES — it cannot see that
> "at most 2" and "4.69" conflict when they sit in different sentences. The W1 box below framed
> the untouched abstract as a co-author framing question. **That was wrong**: a numerical bound
> the new section exceeds is a contradiction, not framing. Caught on 2026-09-12 while answering
> "what are the next steps", before anyone read the archival.
>
> **What changed:** eight archival sites (abstract, Introduction, Contributions box, the
> matched-objective paragraph of §4.5, two places in §4.8, Limitations, Conclusion) and the
> README. The §4.5 paragraph KEEPS its five-seed numbers — correct for five seeds — and now says
> *"We first read that remainder as an upper bound on the representation's share. It is not
> one."* That is an owned expectation grounded in the record, per the voice-pass rule.
>
> **Guards:** `representation-share-is-an-upper-bound` is RESCOPED to the frozen cut only (its
> bound is false, but the cut still says it and cannot be edited). NEW
> `representation-share-is-measured-not-bounded` forbids the old bound in the archival AND the
> README. **The README had been a blind spot all along**: it said "a further factor of two" and
> the old probe wanted "factor of about two", so it was never probed. `seed-count-is-five-everywhere`
> records the deliberate exception (28 written ONLY as `\valNwSeeds`).
> **`python scripts/test_consistency_guards.py`** (new, committed) injects each defect the new
> guards exist to catch and requires each to fire. All six fire. **RUN IT AFTER ANY EDIT TO
> THE ARCHIVAL'S DECOMPOSITION OR OPTIMISER PROSE.**
>
> **Check state after the round:** all twelve green — extract_metrics **379/379** ·
> check_numbers OK ×2 · check_claims PASS · lint_prose · verify_references 36/38 · tex_sanity ·
> bib_sanity · provenance · consistency **76 sites / 20 groups** · workshop_cut · undefined_macros.
> Macros: **10 added, 0 of the original 322 changed.**
>
> ### WHAT W1 WROTE, AND WHAT IT DELIBERATELY DID NOT (2026-09-11, commit `aea9647`)
> **`paper/main.tex` §4.8 `sec:results-decomposition`** reports all three experiments with
> their caveats. **Three existing passages were corrected because they had become false:**
> the Methods and Limitations statements that init and data "cannot be separated from these
> runs" (E3 separates them — both now also say what was NOT re-run crossed: the ablations, the
> sweep, the retrain control), and the Limitations claim that the representation's share is
> "at most that and probably less" (**"probably less" was wrong in the flattering direction**;
> at two parameters the optimiser contributes nothing, so the residual IS the representation).
>
> **THE THREE COUPLINGS TO THE FROZEN SUBMISSION WERE CHECKED, NOT ASSUMED.** This is the part
> to re-verify if anyone edits the archival again:
> 1. **`paper/generated/metrics.tex` is SHARED by both manuscripts.** Regenerating added 47
>    macros and changed NONE of the 322 — verified by diffing the sorted `\newcommand` sets.
>    **Had one moved, a number in a paper under review would have moved with it.**
> 2. **`paper/consistency.yaml` enforces groups ACROSS both files** and the cut cannot be
>    edited to satisfy a new rule. The groups pin PHRASING, not values, so this was safe — and
>    the existing `representation-share-is-an-upper-bound` group was LEFT ALONE because it
>    still correctly guards the cut, whose evidence really does stop at a bound.
> 3. **`metrics_map.yaml` sources the new trees via `../`** so `results/` is never written to.
>
> **NEW GUARD: `optimiser-share-is-two-parameter-only`.** Forbids "the optimiser is ruled out"
> and requires the two-parameter qualification at every site quoting E2. **It was adversarially
> tested and the FIRST test was wrong** — `require` is satisfied from a ±3-line window, so
> deleting the qualifier beside the claim left the paragraph heading to satisfy it. The real
> failure mode is the claim ESCAPING to another section unqualified, which is how the older
> bound was breached (abstract said "is", Limitations "at most"). Retested that way: it fires.
>
> **NEW MACROS ARE DERIVED AND COMPUTED AS DATA.** `scripts/emit_experiment_macros.py` →
> `results_noiseweighted/paper_macros_e1e3.csv`, 55 quantities, self-testing 14 of them against
> the distillation output. `extract_metrics` cannot compute a ratio of SDs, an F statistic or a
> variance component, and putting those in prose is the unsourced number `check_numbers` exists
> to stop. **Re-run it if any experiment tree changes.** One trap it handles: chi2's `F_data`
> p-value rounds to 0.0000 and **a p-value is never zero** — emitted as the bound 0.0001 and
> quoted in the text as "p <".
>
> **VERIFY WHAT THE CHECKERS CANNOT SEE.** Nobody here can compile, so a `\ref` to a missing
> label prints "??" and no checker in §8 catches it. The added text's 4 `\ref` targets and 39
> `\val` macros were verified to resolve by hand. **The first draft cited
> `sec:results-forecast`, which does not exist** — the label is `sec:results-forecasting`.
>
> ### WHAT W1 DELIBERATELY DID NOT DO — PARTLY SUPERSEDED 2026-09-12
> ~~The ABSTRACT, INTRODUCTION and DISCUSSION still argue the pre-round story.~~ **The
> DECOMPOSITION in those sections was corrected on 2026-09-12 (`89177cc`)** — see the correction
> round above; leaving it was a contradiction, not a deferral. **What genuinely remains for the
> co-authors is the THESIS framing (T12) and whether to restate n=5 numbers at n=28 (T13).**
> **Do not do either from a session.**
>
> ### THE FILES
> | what | where |
> |---|---|
> | E1 driver | `noise_weighted_ude.jl` (NW_SEEDS = 1111k, k=1..28; resumable; `NW_FORCE=1`) |
> | E1 distillation + tests | `scripts/distil_noise_weighted.py` |
> | E1/E2 output | `results_noiseweighted/` |
> | E2 driver | `parametric_adam_bfgs.jl` |
> | E2 denominator | `parametric_matched_objective_28seed.jl` → `parametric_chi2_28seed.csv` |
> | the 4th cell (2026-09-12) | same script, `PM_OBJECTIVE=sse_full` → `parametric_sse_full_28seed.csv`. **Default is still chi2**, so the E2 file is reproducible unchanged. |
> | paper macros for E1–E3 | `scripts/emit_experiment_macros.py` → `results_noiseweighted/paper_macros_e1e3.csv` → `paper/metrics_map.yaml` (entries source `../results_noiseweighted/...`). **Re-run it, then `extract_metrics`, if any experiment tree changes.** |
> | guard tests | `scripts/test_consistency_guards.py` — never run two copies at once; they race on `paper/main.tex` |
> | E3 driver | `init_vs_data_grid.jl` (`E3_GRID` sets the size; resumable) |
> | E3 distillation + decomposition | `scripts/distil_init_vs_data.py` |
> | E3 output | `results_initdata/` |
>
> ### EVERY SCRIPT SELF-TESTS, AND THAT IS WHAT LICENSES THE ATTRIBUTIONS
> - `noise_weighted_ude.jl` → published forecast V RMSE `0.2191970340109573`, **bit-for-bit**.
> - `scripts/distil_noise_weighted.py` → the fresh SSE arm must reproduce all five published
>   traj-train â values. **0.000e+00.** (Value check at the harness; this is one stage later.)
> - `parametric_matched_objective_28seed.jl` → the five published chi2 fits. **0.000e+00.**
> - `parametric_adam_bfgs.jl` → TWO gates: the mirrored chi2 equals the published `eval_all`
>   (7.0e-12, both scalings), **and** the substituted sensealg reproduces the closure's ADJOINT
>   *gradient* (9.4e-08) at a point OFF the optimum.
> - `init_vs_data_grid.jl` → stage 1 the published RMSE before any grid runs; stage 2 every
>   diagonal cell against its E1 row. **0.000e+00 on all twenty comparisons.**
>
> **A NOTE ON WHY THE DIAGONAL SELF-TEST IS THE IMPORTANT ONE IN E3.** A diagonal cell and an
> E1 run are the same computation under two different names. Any mismatch means the crossing
> was implemented as a relabelling rather than a crossing — the likeliest bug in the script,
> and one a single-cell value check sails straight past.

**THE 5-PAGE PAPER IS FINISHED AND FITS. THE ARCHIVAL IS CURRENT. THE BUNDLE BUILDS.**
Everything mentor review 2 asked for is done, round 10's audit is done, and what remains is
ERRANDS AND DELIVERY, not writing.

### The compile, confirmed by Vinayak
Body ends at the foot of page 5; the **References head sits at the top of page 6**, which is
the target state (the CFP excludes references, checklist and appendix from the five pages).
**THE MARGIN IS ZERO.** Pages 3, 4 and 5 are all full. Three cut passes spent ~12 lines
getting there and everything cheap is gone. **One added sentence puts the body back on page
6.** The LENGTH DIAL at the top of `paper/sim2science/main.tex` lists every remaining lever
and what each costs, and records how to size an edit without compiling: count words per
printed line off the last compile (12 after a bold run-in head, 16 on a full line), NEVER
source lines, which mis-price captions, displayed equations and float graphics.

### What the submission is
**ONE pdf.** Body (5 pp) + references + checklist + appendix A–E. Compile
`sim2science_upload/main.tex`; EDIT `paper/sim2science/main.tex` and its `checklist.tex`.
`sim2science_upload/` is build output and every `make_sim2science_bundle.py` run destroys it.
The cut now carries FIVE figures: `fig2_neural_ode_voltage`, `fig3_ude_voltage` (the Stage-1
pair), `fig13_parametric_identifiability`, and in App. E
`fig5_metrics_bar_train_vs_forecast` and `fig6_voltage_only_overview`.

### Everything mentor review 2 asked for: DONE
Stage-1 figure (and old Fig. 1 demoted and shrunk) · headline claim softened to his wording
at five sites · appendix A–E · **`tab:hyper`, the hyperparameter table he asked for by name**
· checklist moved between the bibliography and the appendix · Q8 compute answered No → Yes ·
release-on-acceptance wording matched between paper and checklist · footer correctly left
alone · table captions already above.

### D1 IS CLOSED
`fig2`, `fig3` and `fig6` were regenerated on 2026-08-28 with Vinayak's greenlight and now
print **"state propagation"**. The archival caption clause that apologised for the stale
title is deleted. **fig3/fig6 needed no training** (`regenerate_ude_figs` loads the saved
`.jld` and does a forward solve); fig2 was a real NODE retrain and **every per-variable panel
came back BYTE-IDENTICAL**. A 228-file md5 manifest confirms 0 deleted, 9 changed, and no
results file feeding `metrics_map.yaml` touched. `regen_fig3_fig6.jl` is the new script.

### A12/D4 byline: CLOSED
`paper/main.tex` now carries **three** authors, matching the workshop submission. Four coupled
sites were fixed together: the `\author` block, `pdfauthor`, the author-overlap disclosure
("four of five" → **"two of three"**, re-derived from `references.bib`), and the bundler's
`SELF_ID_CLAUSE` regex, which hard-coded the number words and would have refused to build.

### Round 10 audit (2026-08-29): four defects, all fixed, five refuted
Solo audits of all three documents, then cross-document, then adversarial refutation.
**Verdict: the three documents ARE consistent.** Details in the round-10 box in §13.
**Round 11 then found seven more — see the round-11 box below. Do not read round 10's
verdict as a stopping rule; it was accurate about the axes it ran and blind to the rest.**
The one worth carrying forward: **App. C claimed "a single consumer laptop" and four lines
later "a shared desktop", and checklist Q8 repeated the pair under the words "Hardware is
stated as fact". Both form factors were invented** — the recorded hardware is "CPU only, no
GPU, 16 GB RAM" and nothing more. **"Stated as fact" is a promise; find the source before
writing it. An unsourced detail that sounds mundane is the one nobody re-checks.**

### Round 11 audit (2026-08-29, later): SEVEN defects, all fixed, all FREE
Round 10 signed off with "the three documents ARE consistent". Round 11 pointed different
axes at them and found seven more. **NOT ONE touched the body of the 5-page paper**, so the
compile that was measured on 2026-08-28 still stands: all seven landed in the appendix, the
checklist, or an unsent email. **27 agent candidates raised, 22 refuted (81 %)**; the seven
fixes are the 5 that survived plus 2 found by hand. Round 10 confirmed 4 of 9 (44 % real),
round 11 confirmed 7 of 29 (24 % real) — **the signal is getting noisier, which is what
approaching the noise floor looks like.**

**The two that a reviewer would have seen, both in `checklist.tex`, which NOTHING verifies:**
- **Q4 cited "(Eq.~1)". THE SUBMISSION CONTAINS NO NUMBERED EQUATION.** Cut pass 2 inlined
  the displayed loss equation and converted both `\eqref{eq:loss}` sites to
  `$\mathcal{L}_{\mathrm{full}}$` — it is logged in the LENGTH DIAL at `main.tex:196` as the
  round's biggest single win. The only display environment left in the whole document is the
  **starred** `align*` in App. A. `Eq.~1` was a hand-typed literal, not a `\ref`, so LaTeX
  emitted no warning and no checker looked. **A cross-reference typed as a literal is
  invisible to every tool that guards cross-references.**
- **Q6 claimed Sec. 2 gives the optimiser and iteration budget "for every arm".** Sec. 2 has
  five paragraphs and none of them is the parametric arm, whose multi-start Nelder--Mead sits
  in Sec. 3.2. Rescoped to the neural ODE and the UDE, with a pointer to `sec:parametric`.

> **AND THEN THE SIBLING SWEEP EARNED ITS KEEP.** A follow-up round asked only "what else has
> the shape of the defects we just fixed?" and found **Q4 carrying the identical omission** —
> its reproducibility inventory names Sec. 2, App. A and App. B, and the parametric arm's
> procedure is in none of the three, while Q4 answers **Yes** to "fully disclose all the
> information needed to reproduce the MAIN experimental results" and that fit is a main
> result. **Fixing Q6 and not Q4 is this project's named worst failure mode, and it happened
> inside the very round that fixed Q6.** Always sweep for siblings in the same file, not just
> the sites a consistency group names.

**Three more, all appendix, all from lifting prose out of `paper/main.tex`:**
- **App. E's `fig:voltage-only` caption said the gate agreement "is quantified in
  Table~\ref{tab:main}".** It is not: the cut's Table 1 has three columns (forecast $V$ RMSE,
  rollout, recovered $g_{Ca}$) and **the cut quotes no gate quantity anywhere at all**. The
  caption came from the archival, whose `tab:forecast` *does* carry a gate RMSE column, and
  the `\ref` was retargeted without rechecking that the new target held the same content.
  Now says the agreement should not be read off the panel, which keeps the guard.
- **App. C's replication inventory named "the neural-ODE baseline" — one condition where
  Table 1 has THREE separately trained rows** (as published, parity, autonomous parity), so a
  reproducer multiplying the 11-minute figure by the printed inventory undercounts by ten
  trainings. The paragraph's entire purpose is to be multiplied. Now "the three neural-ODE
  conditions of Table~\ref{tab:main}", which pins the count to a target the reader can count.
- **App. B said "the rug in that panel"** — a term the cut's `fig:ident` caption never
  introduces, because the sentence naming the tick marks was cut in the lift. Named instead.

**The seventh was not in any manuscript:** `paper/draft-email-N9-prathamesh.md` still closed
by telling Prathamesh that two figures print the stale title "state reconstruction", that
fixing them needs a retrain, and that **neither is in the workshop paper**. All three were
false once D1 closed: the figures were regenerated on 2026-08-28, fig3/fig6 needed no
training, and `fig6_voltage_only_overview` now ships in App. E as round 10's own D2 fix. The
pre-send checklist's "nobody has compiled it" was stale for the same reason. **A draft written
before the work and reconciled once is stale the next time the work moves; re-read it against
the artifact immediately before sending, not once when it was written.**

> **THE LESSON THAT GENERALISES, and it is the same shape three times over: A POINTER IS
> ONLY AS GOOD AS ITS TARGET'S CONTENT.** `\ref` checking, `check_undefined_macros` and
> LaTeX itself all confirm that a reference RESOLVES. Nothing confirms that the thing it
> resolves to contains what the sentence says it contains. Every one of round 11's manuscript
> defects is a pointer that resolves cleanly to the wrong content, and every one entered when
> prose was LIFTED from the archival into the cut. **After any lift, re-verify the target,
> not just the label.**

### Round 12 audit (2026-08-29, overnight): TWO defects, 20 of 22 refuted (91 %)
Run at Vinayak's request — he was not ready to submit and wanted more assurance. Two axes,
both chosen because they had never been run: **(1) the LIFT AUDIT** — nine agents, one section
each, aligning every paragraph and caption of the cut against the archival paragraph it was
copied from, asking only what the trim BROKE; **(2) the CHECKLIST BY ASSERTION-CHECKING** —
three agents decomposing all sixteen justifications into their individual factual assertions
and verifying each against the manuscript. Both survivors are free; **the body was not touched
and lines 1-770 remain byte-identical to the 28 Aug compile.**

- **App. B claimed "two metrics carry knobs the reported numbers depend on" and then defined
  the SPIKE DETECTOR — which the cut reports no quantity for.** The archival earns that
  sentence (`main.tex:1023`, "Four of the five seeds reproduce the spike count"); the cut
  dropped that result and kept the definition, so `\Vthresh`, `\SpikeSep` and `\SpikeTol`
  govern nothing printed. **Reworded, NOT deleted** — the mentor asked explicitly to keep the
  metric definitions (HANDOFF:707), so the fix says only the first affects a reported number.
- **Q5's reproducibility inventory omitted the parametric arm.** THE SAME OMISSION AS Q6 AND
  Q4, in a third question.

> ### THE SIBLING PATTERN, AND IT TOOK THREE ROUNDS TO EXHAUST
> Round 11 found it in **Q6**. Round 11's own sibling sweep found it in **Q4**. Round 12 found
> it in **Q5**. Three questions, one defect, three separate rounds — and each round had been
> told about the previous one. **A sibling sweep that finds one sibling is not finished; keep
> sweeping the same file until a pass comes back empty.** All three inventories now name
> `Sec.~\ref{sec:parametric}`; Q12 was checked and correctly does not (it is an assets
> question, not a methods inventory). Verified exhaustively, not by sampling.

**THE REFUTATION RATE IS THE HEADLINE, NOT THE FINDINGS.** Round 10: 4 of 9 real (44 %).
Round 11: 7 of 29 (24 %). Round 12: **2 of 22 (9 %)**. Twenty of twenty-two proposals were
adversarially killed, most on grounds already recorded in this file. Among the refuted were
attacks on the abstract's scope qualifier, the Stage-1 caption, the Table 1 bold convention,
the retrain paragraph's domain labelling, `walch2016`'s and `silvestri2023analysis`'s
characterisations, and four separate checklist answers. **The body of the 5-page paper has now
been attacked across rounds 11 and 12 by more than a dozen independent agents and has not
yielded a single confirmed defect.**

> ### THE STOPPING RULE, AGREED WITH VINAYAK BEFORE ROUND 12 RAN
> **Stop when a round produces zero body-affecting findings AND refutes more than 75 % of its
> candidates.** Round 11 met it (0 body, 81 %). Round 12 met it far more decisively (0 body,
> 91 %). **THE AUDIT IS DONE. Further rounds are now expected to cost more than they return:
> at a 9 % true-positive rate the next round is likelier to damage the paper than improve it,
> and this project has twice injured itself while fixing something — a rewrap orphaned a
> `numok` in round 10, and round 11's Q6 fix left Q4 broken.** If someone reopens this, the
> burden is to name an axis never run, not to run the same ones again.

### MENTOR REVIEW 3 (2026-08-29, video). LANGUAGE ONLY. ALL OF IT IS DONE.
**He approved the science, the depth, the numbers, the abstract, the references, the appendix
and the checklist placement** (*"the content is very nice, the technical depth is good"*,
*"this abstract is good"*, *"overall the paper looks good to me"*). His asks were register and
two mechanical defects. **Nothing here needed a new number and nothing changed meaning.**

**THE BLOCKER IS GONE.** *"You did not require a specific ID... just type Raj Dandekar and you
will be able to get it in the drop-down box, just select that."* Vizuara AI Labs, principal
researcher. **N10 is unblocked — do not go hunting for a `~Raj_...` string.**

**1. Coloured link boxes — a REAL defect, and a lift-audit miss.** `paper/main.tex:201-203` has
carried `\hypersetup{hidelinks, pdfborder={0 0 0}}` all along. **The cut had bare
`\usepackage{hyperref}` and no `\hypersetup` at all**, so the compiled submission drew coloured
borders round every citation, `\ref` and `\url`. Lifted across. Links stay clickable, which is
what he asked for ("hide, not remove"). **Round 12's lift audit could not have found this: every
agent was scoped to a PROSE line range (411+) and nobody was pointed at the preamble. THE
PREAMBLE IS PART OF THE LIFT AND MUST BE IN SCOPE NEXT TIME.**

**2. The footer now names the workshop.** `neurips_2026.sty` builds `\@trackname` with
"Workshop: \@workshoptitle" but **only uses it under `[final]`** (sty:419-421); submission mode
prints the generic string instead. `[final]` would fix the footer AND set `\@anonymousfalse`,
printing the real byline — **never use it**. Instead `\@noticestring` is renewed in OUR preamble
between `\makeatletter`/`\makeatother`. **The `.sty` is untouched**, which matters: the template
warns that tweaking the style files may be grounds for desk rejection.

**3. The checklist: 1016 → 648 words, 36 % shorter, all sixteen answers.** He said it
*"entirely looks Claude-written"* and *"you need not to write such a long checklist"*.
**THE TEMPLATE AGREES WITH HIM** — its instruction block says *"Please provide a short (1--2
sentence) justification"*, so we were outside the template's own guidance, not just his taste.
Q8 went 153 → **38** words, Q7 774 → 417 chars, Q5 now opens with his own phrasing. **Every
`\answerYes/No/NA` is untouched and every section and appendix pointer was kept** — those were
verified across four audit rounds and are the only part a reviewer actually uses. Em-dashes in
the checklist: **1 → 0**.

**4. Register, in the body.** *"recipe"* → **"pipeline"** at both live sites (already house
vocabulary: "the pipeline loses what the data carry"). **All three sentence-initial "Nor"
constructions are gone** — he flagged two by name and said of one *"I am not able to figure out
exactly what does Nor"*. Plus the flourishes: "carries the scientific weight", "does what it
promises", "the shape is right; the scale is not", "not merely X but Y", and several
colon-plus-elaboration openers.

> **THE LENGTH GATE HELD, AND IT WAS ENFORCED MECHANICALLY, NOT BY EYE.** Every proposed body
> edit had to satisfy `new_chars <= old_chars`; the applier aborts otherwise. Measured on the
> printed body (comments stripped, abstract through Sec. 5): **20929 → 20875 chars, 2766 → 2761
> words. It SHRANK.** So the 28 Aug page measurement still stands and the References head still
> opens page 6. **Recompile and confirm anyway — this is the first round that touched the body.**

> ### TWO PROCESS LESSONS FROM THIS ROUND
> **(a) `lint_prose` cannot see what he saw.** It reports "no mechanical uniformity flags" on
> the body AND on the checklist, em-dash rate 0.3-1.3 per 1k against a threshold of 4. His
> complaint was REGISTER, and the tool says so itself: *"the register and the argument are
> judgment calls no linter catches."* **Do not answer a register complaint with a linter score.**
> The fix was to hunt named constructions — inverted negatives, balanced antithesis, elegant
> variation, colon-elaboration, rule-of-three — not to chase the dash count.
> **(b) THE BUNDLER CAUGHT ME.** The two comments added for fixes 1 and 2 said "Prathamesh
> flagged this", and `make_sim2science_bundle.py` REFUSED TO BUILD: *"main.tex:264 still contains
> identifying string 'Prathamesh'"*. Reworded to "a co-author". **The guard works, it fired on
> a comment that would never have printed, and that strictness is correct — keep it.**

### THE VOICE PASS (2026-08-29, late). USE THE PROJECT'S OWN SKILL — IT EXISTS AND IT WORKS.
After the language round above, **Vinayak still said the paper reads as AI-written.** He asked
whether a skill could help. **There is one, in this repo, and it had never been used:**
`Paper Writing Skills/references/writing-craft.md` §4.7, and
`Paper Writing Skills/examples/hh-sciml/humanized-passage.md`, which is a worked BEFORE/AFTER
on a paragraph of THIS PAPER. **Read both before any future writing pass.**

> **THE LINTER IS NOT THE JUDGE, AND THIS ROUND PROVED IT.** Before the pass, `lint_prose`
> reported sentence CV **0.75** (human band), discourse openers **0 %**, low-information
> sentences **0 %**, LLM-tell-word density **0.6/1k**, em-dashes **1.3/1k** against a ceiling of
> 4 — and the verdict "no mechanical uniformity flags". Two human readers still said it read as
> machine-written, **and they were right.** The linter says so itself: *"the register and the
> argument are judgment calls no linter catches."* **Never answer a register complaint with a
> linter score.**

**THE MEASURED GAP, found by running the greps §4.7 prescribes: THE BODY CONTAINED ZERO OWNED
EXPECTATIONS.** "we expected", "we were surprised", "we thought", "we assumed" — all returned 0.
All fourteen first-person verbs were procedural ("we ran", "we fit", "we compare"). §4.7 calls
an owned expectation *"close to unfakeable"* because *"a machine will not risk an expectation it
might have to retract"*. **Three are now in, each grounded in the documented record, none
invented:**
- `main.tex:589` "We expected to use our black-box baseline as published. It needed repair."
- `main.tex:654` "We expected the natural reading to hold: one short noisy trajectory does not
  carry enough information... It does not survive."
- `main.tex:716` "We expected four times the budget to tighten the estimate. It loosened it."
  — this is almost verbatim §4.7's own worked example, and it is what §3 records as
  *"the optimiser succeeded and the science got worse"*.

**AN INVENTED EXPECTATION WOULD BE A LIE ABOUT HOW THE RESEARCH WENT.** The agents were given a
closed list of four expectations documented in this file and forbidden to invent a fifth. Keep
that constraint on any future voice pass.

Also applied: burstiness (a short declarative after two long ones — "It needed repair.",
"It loosened it.", "It does not survive."), and the `not only X but Y` antithesis at
`main.tex:760` removed. **The printed body SHRANK again, 20875 → 20818 chars.**

> ### TWO PROCESS FAILURES IN THIS ROUND. BOTH ARE MINE AND BOTH ARE REUSABLE.
> **(a) I NAMED LINE RANGES WITHOUT NAMING THE FILE.** The `voice:intro` and `voice:methods`
> agents read `paper/main.tex` — THE ARCHIVAL — and edited it. Their quotes gave them away:
> `\eqref{eq:membrane}` and `kainth2025pinodesr` **exist only in the archival** (the cut has no
> numbered equation and does not cite that key). **ALWAYS put the full path in the range itself,
> not only in a preamble paragraph.** The archival edits were reverted with `git checkout --`:
> the archival is not being submitted and its polish is post-deadline P1–P9, so changing it the
> night before submission is pure drift risk.
> **(b) THE AGENTS HAD WRITE ACCESS AND APPLIED THEIR OWN EDITS.** I had designed the round to
> return proposals for me to adjudicate, but the workflow subagents can Edit, and several did.
> My applier then reported "matches neither file" for ten edits that were already applied — and
> that near-miss is the lesson. **`git diff` IS THE ONLY TRUSTWORTHY REPORT OF WHAT CHANGED.**
> Do not reconcile a round from what the agents say they did; reconcile it from the diff.
> To keep proposals as proposals, say so explicitly in the prompt and verify with `git status`
> before and after.

### THE ROLLOUT CEILING: ≥70 vs 69.863. ADJUDICATED 2026-08-29 — **DO NOT "FIX" THIS.**
Found by reading the compiled PDF. It will be re-found by anyone who reads carefully, so the
ruling is recorded here to stop it being reopened.

**The tension is real.** Table 1 and the body print the UDE rollout as **`≥ 70` ms**. App. B
tells the reader the bound is *"the 70 ms cap, or the time actually left in the trace after the
first forecast sample"*, then supplies the numbers to compute it — 512 samples over 100 ms, the
first forecast sample at 30.137 ms — which give **69.863 ms**. That is exactly what the corrected
metric records:
`awk -F, '$5=="30.0" && $7=="forecast" && $8=="rollout_horizon_ms"' results/identifiability/rollout_censoring.csv`
→ `69.86301369863014`. So a careful reviewer can derive 69.863 from the appendix and see the
table say ≥70.

**WHY IT STAYS.**
1. `\valUdeForecastRollout` renders at **0 dp**, and 69.863 rounds to 70. At the printed
   precision the two agree.
2. The direction is **conservative**: we claim slightly LESS horizon than the tube survived,
   never more. There is no overclaim.
3. `emit: ge` + `censored_at: 70.0` is guard **G1** and is mandatory — the value is a censoring
   sentinel, not a measurement, and the `≥` is what says so.
4. **§4c already ruled on the source file:** `metrics_baseline_multiseed.csv` was deliberately
   NOT rewritten when the metric was corrected, because *"appending there would change the
   inputs of already-published macros."* Re-deriving a headline macro to gain 0.137 ms is a
   far worse risk than the exposure.
The archival discloses the correction in its `fig7_ablation_noise` caption (`main.tex:2199`);
the cut has no room and does not need it. **If a future audit round raises this, close it with
this box.**

### A COMMITMENT MADE TO PRATHAMESH — IT IS NOW OWED
The N21 email says, of "recipe" and the sentence-initial "Nor": *"Both still stand in the full
paper and I will do the same pass there after the deadline."* That is a promise to a co-author,
not a nice-to-have. **`paper/main.tex:296` still has "This two-stage recipe" and `main.tex:259`
still has "Nor is it under-training".** Do them in the P1 pass (see the post-deadline table).

### THE FILE MAP — what is real, what is derived, what must never be committed
Asked directly by Vinayak on 2026-08-29, which means it was not findable enough. It is here now.

**THE FOUR FILES THAT ARE THE PAPER.** Everything else is derived from them or supports them.

| file | what it is |
|---|---|
| `paper/sim2science/main.tex` | **THE SUBMISSION.** 1105 lines. Preamble + LENGTH DIAL 1–408 · abstract 411–431 · **BODY (Secs 1–5) 434–770** · bibliography 772 · `\input{checklist}` 778 · appendix 790–1105 (A eqns 792 · B hyper 882 · C compute 992 · D ablations 1021 · E figures 1064). |
| `paper/sim2science/checklist.tex` | 246 lines, the 16 NeurIPS answers. **Ships inside the submitted PDF and NO CHECKER READS IT.** Highest defect rate in the project. |
| `paper/main.tex` | **THE ARCHIVAL.** 2212 lines, real 3-author byline. Not submitted. The source every line of the cut was lifted from. |
| `paper/references.bib` | 38 entries. Shared by both manuscripts. |

**THE LEDGERS — these are what make the numbers trustworthy. Edit these BEFORE the prose.**
`paper/claims.yaml` (32 claims; the `scope:` fields are where over-claiming dies) ·
`paper/consistency.yaml` (18 groups, 78 sites, pins exact phrasings across both manuscripts) ·
`paper/metrics_map.yaml` (traces every `\val` macro to a results CSV) ·
`paper/claim-guards.md` (G1 censoring, G2 no mean±SD on NODE time-indexed rows).

**GENERATED — never hand-edit; regenerate instead.**
`paper/generated/metrics.tex` and `.json` (322 macros, written by `extract_metrics.py`) ·
`paper/figures/` (staged from `figures/` by `scripts/stage_figures.ps1`) ·
`paper/citations-audit.md` (written by `verify_references.py`).

**BUILD OUTPUT — destroyed and rewritten by every `make_sim2science_bundle.py` run.**
`sim2science_upload/` — `main.tex`, `checklist.tex`, `supplementary.tex`, `figures/`,
`generated/`, the two `.sty` files and `references.bib`. **A hand-edit here is lost silently.**
This is what you compile and upload. `overleaf_upload/` and `overleaf_upload.zip` are the
archival's equivalent.

**THE ENGINE.** `src/` (the source of truth for the model and metrics) · `HH_model.jl` ·
`experiments_runner.jl` · the `figure_*.jl` / `regen_*.jl` / `recover_*.jl` scripts ·
`results/` (every CSV behind every number) · `Project.toml` / `Manifest.toml` (pinned Julia 1.6.7).

**THE CHECKERS.** `scripts/` (12 python + 3 ps1) and `Paper Writing Skills/scripts/` (4 python:
`extract_metrics`, `check_numbers`, `check_claims`, `lint_prose`). §8 lists all sixteen checks
and the exact command line for each. **`Paper Writing Skills/` is gitignored — it is a separate
private skill repo, not part of this project.**

> **NEVER COMMIT, and all are already in `.gitignore`:** `main.jl` · `Main_Clean.jl` ·
> `Paper Writing Skills/` · `.claude/` · `__pycache__/` · `overleaf_upload/` ·
> `overleaf_upload.zip` · `sim2science_upload/` · `HANDOFF.md` (snapshot-only, see the top).
> Because every never-commit path is already ignored, **`git add -A` is safe** — and it is what
> you want, since `git add -u` stages TRACKED files only and silently leaves new scripts and
> new figures behind.

### The checkers: THIRTEEN checks across twelve scripts
Two were added on 2026-08-28. `scripts/check_undefined_macros.py` is the twelfth checker and
caught a real undefined macro on its first run. `check_tex_sanity.py` gained a check for a
line-end comment with no space before its `%`, which silently glues two words together in the
PDF. **`check_claims.py` takes BOTH manuscripts at once** — `--tex A B` gives 0 errors;
pointed at the cut alone it emits 13 spurious ones.

### WHAT ACTUALLY REMAINS
**REWRITTEN IN FULL 2026-09-09. The paper is submitted; this is now an EXPERIMENTS list, not an
errands list. Ordered so each item's prerequisite comes first.**

> ## ★ THE WORK QUEUE — E1–E4 ARE CLOSED. REWRITTEN 2026-09-11.
>
> **~~E1. RAISE THE SEED COUNT.~~ DONE 2026-09-10, commit `5d60e21`, pushed.**
> n=28 on BOTH arms, paired. The effect is REAL: F(27,27) = 4.177, p = 0.0002.
> Full numbers in the ★ RESULTS box at the top of §4o.
>
> **~~E2. ISOLATE THE OPTIMISER.~~ DONE 2026-09-10, commit `fa9bcad`, pushed.**
> The optimiser contributes **nothing** at two parameters (1.0000×), so the whole 4.68× is
> representation. `parametric_adam_bfgs.jl`.
>
> **~~E3. DECOUPLE INIT FROM NOISE SEEDS.~~ DONE 2026-09-11, commit `64def0e`, pushed.**
> The spread is the DATA DRAW; initialisation contributes ~0. And noise-weighting works by
> killing the init×data INTERACTION (0.209 → 0.031, −85 %), not by reducing data sensitivity.
> `init_vs_data_grid.jl` + `scripts/distil_init_vs_data.py`.
>
> **~~E4. PRINT THE TRUE-PARAMETER LOSS IN THE ARCHIVAL.~~ NOT A TASK. ALREADY DONE.**
> **The queue item's premise was false and cost a session's worth of deferring.** It claimed
> "a grep of `paper/main.tex` finds nothing". It is at **`paper/main.tex:1681`**, printing
> `\valParamFitSseFullTwoMeanStd` and making the exact argument E4 asked for: *"Both UDE
> losses are below it… it is over-fitting."* Verified 2026-09-11 by reading the line.
> **STRIKE IT; DO NOT "FIX" IT.** And note the trap for anyone who re-checks: there are TWO
> nearby numbers and they are different quantities. **39.33 ± 1.26** is the 5-seed mean of the
> unweighted loss at the *maximum-likelihood* (gCa, ECa) — that is what the macro holds and
> what the archival correctly describes. **39.64** appears in
> `results/identifiability/loss_surface_gca2.0.csv` and is a single-seed GRID point at
> (gCa 1.9, ECa 129), seed 1111. They are not the same thing and neither is wrong.
>
> ### WHAT IS ACTUALLY LEFT, AND NEITHER IS A COMPUTE ERRAND
>
> **~~W1. THE WRITE-UP.~~ DONE 2026-09-11, commit `aea9647`.** `paper/main.tex` gained §4.8 and
> three corrections. All twelve checks green; submitted paper untouched. **What remains of W1
> is the part that needs a co-author** — see the next item.
>
> **W1b. RECONCILE THE DECOMPOSITION — DONE 2026-09-12, `89177cc`.** W1 had left the abstract
> saying "at most twofold" beside §4.8's 4.69×. Measured the missing 2×2 cell and corrected
> eight archival sites plus the README. See the correction round in the ★★ RESULTS box.
>
> **W2. THE FRAMING PASS — NEEDS PRATHAMESH, NOT A SESSION.** Now narrower: the decomposition
> numbers are fixed; what remains is the THESIS (T12) and the n=5-vs-n=28 restatement (T13).
> **Send him the ★★ RESULTS box first (T1).**
>
> **E5. THE CLOSURE LADDER.** Unchanged. Compute is cheap (~9 h); the cost is 3–5 days of
> implementation, since rungs 1–2 are non-MLP closures needing a new basis object.
> **DESIGN CONSTRAINT: every rung must train under the SAME objective and optimiser and distil
> by the SAME projection**, or the ladder silently reproduces the headline comparison's own
> confound at every step. Claim the TREND, never adjacent rungs — the gCa sweep precedent
> forces that. **E2 now supplies the missing control**: at two parameters the optimiser is
> worth 1.00×, so a ladder held at one optimiser is not obviously confounded by it.
>
> **E6. A SECOND SYSTEM.** Unchanged. Biggest item by far, and buys generality of DIRECTION,
> not of threshold. Consider varying the closure's INPUT SET on the existing system first — the
> paper names it as an unvaried axis and it is ~a tenth the cost.
>
> **E7 (NEW, and E3 is what motivates it).** E3's grid used the five PUBLISHED data seeds,
> which E1 proved were a benign draw — SSE rel sd 0.374 over those 25 cells against 0.802 over
> 28 seeds. So E3's data component is an UNDERSTATEMENT. Re-running the crossed grid on a
> less benign data sample would sharpen it. **The conclusion would only get stronger, so this
> is polish, not a correction.** ~6.7 h at 5×5; `E3_GRID` already parameterises it and the
> resume cache makes an extension nearly free.

#### HISTORICAL — the pre-submission errand list, all discharged

> ## ⚙ REPO MECHANICS AS OF 2026-09-09. FOUR THINGS A NEW SESSION WILL OTHERWISE GET WRONG.
> 1. **TWO BRANCHES, AND THE WORK IS NOT ON `main`.**
>    `main` → the submitted paper + every HANDOFF snapshot. `experiments/noise-weighted-loss`
>    → the new scripts and `results_noiseweighted/`. **Both are pushed.**
>    `git checkout experiments/noise-weighted-loss` before doing experiment work.
> 2. **HANDOFF SNAPSHOTS GO ON `main`, DELIBERATELY.** Every previous snapshot is there and
>    keeping the record linear is worth the branch dance. `HANDOFF.md` is gitignored, so it
>    survives `git checkout` untouched: switch to `main`, run
>    `scripts\snapshot_handoff.ps1 -Push`, switch back. Do not snapshot onto a branch.
> 3. **THE `.gitignore` ENTRIES FOR SMOKE OUTPUT LIVE ONLY ON THE BRANCH.** `results_nw_smoke/`
>    and `figures_nw_smoke/` are ignored there but NOT on `main`. If you run
>    `NW_SMOKE=1` from `main`, that meaningless output is not protected — and a smoke run once
>    truncated `results/metrics_all.csv` from 696 rows to 321. **Merge the branch or copy those
>    two lines before running a smoke test from `main`.**
> 4. **COMMIT ATTRIBUTION — SETTLED 2026-09-11, KEEP THE CO-AUTHOR LINE.** The system-level
>    instruction requires `Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>`,
>    which contradicts this file's older "Vinayak is the committer, no Claude co-author line"
>    rule. Vinayak was asked on 2026-09-11 and answered *"do what is recommended"*, i.e. left
>    it to the session. **Ruled: KEEP THE LINE.** All five commits on the experiment branch now
>    carry it, so the branch history is uniform, and it matches the active instruction. The §9
>    gotcha "Commits must have NO Claude co-author" is SUPERSEDED for this branch.
>    **If Vinayak later rules the other way, it is a rewrite of five commits, not one.**
> 5. **THREE ISOLATED RESULT TREES NOW, NOT ONE.** `results_noiseweighted/` (E1, E2),
>    `results_initdata/` (E3), and the published `results/` which **nothing may overwrite**.
>    Smoke output goes to `results_nw_smoke/` and `results_e3_smoke/`, both gitignored ON THIS
>    BRANCH ONLY — the `.gitignore` entries do not exist on `main` (see item 3).
> 6. **THE REPO IS ~75 MB HEAVIER AND IT IS ALL GRID PROBES.** E1 and E3 committed
>    `grid_*.csv` off-trajectory probes (~670 KB each, 106 files). `.gitignore:74` says
>    `results_noiseweighted/` is deliberately tracked because it is evidence, so they were
>    committed. **Only the `probe_*.csv` (on-trajectory) files feed any current analysis**; the
>    grids exist for a hull-domain analysis nobody has run. They are regenerable from the
>    committed `.jld` parameter snapshots without retraining. **Worth a prune decision before
>    the fresh-repo step — but that is Vinayak's call, not a session's.**

> # ██ SUPERSEDED 2026-09-10 — THE n=5 PILOT. KEPT FOR THE REASONING, NOT THE NUMBERS. ██
> **Every figure below is n=5 and has been replaced by the ★★ RESULTS box at the top of
> §4o. The pilot's "NOT SIGNIFICANT" warning is now DISCHARGED: at n=28 it is significant
> (p = 0.0002). Do not quote 1.645 ± 0.672, 2.018 ± 0.482, 3.84× or 6.57× as current.**
>
> # ██ FIRST NEW RESULT SINCE SUBMISSION (2026-09-08). NOISE-WEIGHTING PARTLY WORKS. ██
> **Branch `experiments/noise-weighted-loss`. Scripts: `noise_weighted_ude.jl`,
> `scripts/distil_noise_weighted.py`. Output: `results_noiseweighted/`. Nothing under
> `results/` was touched, so no published macro can have moved.**
>
> **THE QUESTION.** The matched-objective control showed that at a FIXED two-parameter
> representation, swapping the unweighted SSE for the noise-weighted chi-square takes relative
> spread from 0.197 to 0.062. `make_ude_loss` descends the unweighted one. So: *if the UDE
> descended the likelihood instead, would the conductance come back?*
>
> **THE ANSWER: PARTLY.** `a_hat` on the supervised trajectory, true $g_{Ca}=2.0$, n=5:
>
> | arm | a_hat | rel sd | t vs truth |
> |---|---|---|---|
> | SSE-trained (published) | 1.645 ± 0.672 | 0.408 | −1.18 |
> | **chi2-trained (new)** | **2.018 ± 0.482** | **0.239** | **+0.09** |
> | direct parametric fit | 2.092 ± 0.130 | 0.062 | — |
>
> The point estimate lands essentially on the truth and relative scatter falls ~40 %, **at no
> cost to forecasting** (0.269 ± 0.161 mV against the published 0.245 ± 0.028).
>
> > ### ⚠ NOT SIGNIFICANT AT n=5. DO NOT WRITE IT AS A DEMONSTRATED EFFECT.
> > Variance ratio $(0.672/0.482)^2 = 1.94$ on $F(4,4)$; ~6.4 is needed for $p<0.05$. What this
> > establishes is a DIRECTION consistent with the fixed-representation control, not a proven
> > one. **Raising the seed count to ~20–28 on both arms is now the single highest-value next
> > experiment**, and it is pure unattended compute.
>
> **THE SECOND FINDING IS THE MORE USEFUL ONE, AND IT WAS NOT THE POINT OF THE RUN.** With both
> arms now descending the SAME objective, the closure still scatters **3.84×** wider than the
> direct two-parameter fit, down from 6.57×. That is a *measured* representation effect under a
> controlled objective, and it replaces the old arithmetic decomposition (≈3× objective, residual
> "at most 2×" to representation **and** optimiser jointly). **It is LARGER than that bound
> implied, so the old decomposition was conservative.** **[CORRECTED 2026-09-12: WRONG. An upper bound that is exceeded has been VIOLATED, not been conservative. See the ★★ RESULTS box: at n=28 the representation is 3.02× under SSE and 4.69× under chi2.]** The optimiser is still not isolated —
> the closure uses Adam+BFGS, the parametric arm multi-start Nelder–Mead.
>
> **WHAT IT DOES TO THE PAPER.** It moves it off "flexible closures lose identifiability"
> (a caution) toward "they lose it partly because of the objective you chose, and here is the
> change that recovers most of the accuracy" — a partial prescription — **while strengthening,
> not weakening, the representation attribution.** Both halves are publishable.
>
> **THE SELF-TEST PASSED EXACTLY**, which is what licenses the attribution: the `:sse` arm at
> seed 1111 returned forecast V RMSE `0.2191970340109573`, bit-for-bit the published per-seed
> value. The harness IS `run_experiment`, so the difference is the objective and nothing else.

> # ██ POST-SUBMISSION. THE PLAN IS: WAIT FOR THE REVIEWS, EXPERIMENT MEANWHILE. ██
> **Agreed with Prathamesh, 2026-09-04/08.** Vinayak proposed waiting for the Sim2Science
> decision (29 Sep) before choosing the next venue, and running experiments in the meantime.
> He agreed: *"Yes, let us wait for the reviews before deciding on our next steps."*
>
> **THE FOUR THINGS HIS REPLY SETTLED:**
> 1. **ICLR 2027 IS OFF.** Not on merit — on sequencing. Its abstract deadline is **18 Sep**
>    and its paper deadline **25 Sep** (he had quoted only the paper one), and Sim2Science
>    notifies **29 Sep**. We would submit to an A\* venue four days before ever seeing the only
>    outside review this work has had. **Do not re-open this on a claim that "there is still
>    time" — the binding date was the abstract deadline, and it passes first.**
> 2. **HE HAS ACCEPTED THE REVIEWER INVITATION** and completed expertise selection. The
>    "remind him around 8 Sep" item is DISCHARGED. He will say when he has reviewed, and
>    nothing more: *"I cannot disclose any details beyond that due to confidentiality."*
>    **Do not ask him about the assigned papers.**
> 3. **HE IS STRONGLY INCLINED TO A BIO-ML JOURNAL** and asked us to watch that space:
>    *"I am strongly inclined to target a Bio-ML related journal for this work, as I believe
>    it would be a perfect fit."* That moves **npj Systems Biology and Applications** to the
>    front — Nature Portfolio, squarely Bio-ML, and it published the Philipps et al. 2025
>    open-problems-in-UDEs paper this study is a direct empirical answer to.
> 4. **HE APPROVED THE EXPERIMENTS, WITH A CONSTRAINT:** *"please ensure we optimize our
>    compute resources and experimental scope effectively."* That constraint is why the cheap,
>    high-lift experiments go first and the closure ladder and a second system are deferred.
>    He will also share his internal tool's feedback to inform what else to run.
>
> **VINAYAK'S CAPACITY IS LIMITED right now.** He said so explicitly. The experiment plan is
> therefore built to cost COMPUTE rather than his attention: the runs are unattended, and what
> genuinely needs him is reading results and rewriting.

> # ██ SUBMITTED. 2026-08-30. SUBMISSION #89. ██
> **Sim2Science @ NeurIPS 2026, `NeurIPS.cc/2026/Workshop/Sim2Sci`.** One pdf, 19 pages,
> uploaded as `main.pdf`, recompiled from the current bundle immediately before upload.
> **THE PAPER IS OUT OF OUR HANDS UNTIL 29 SEP.**
>
> | | |
> |---|---|
> | **Tag** | `sim2science-submitted` → commit **`3b9e317`**, pushed. `git checkout sim2science-submitted` restores the exact submitted source. |
> | **Frozen** | `paper/sim2science/main.tex` carries a FROZEN banner as of the commit after the tag. It is comments only — every printing line is byte-identical to the tag, verified with `diff` on the non-comment lines. |
> | Reviewing opens | **8 Sep 2026** |
> | Reviews due | **23 Sep 2026** |
> | **Notification** | **29 Sep 2026** |
> | Workshop | **11–13 Dec 2026, Paris** |
>
> **PRATHAMESH NOW OWES TWO REVIEWS.** He is the nominated reciprocal reviewer
> (`~Prathamesh_Dinesh_Joshi1`) and the CFP assigns each nominee **two other submissions**,
> starting 8 Sep. He accepted knowingly — the N21 email warned him in as many words — but he
> may well have forgotten by September. **A reminder around 8 Sep is a kindness, not a chase.**
>
> **DO NOT EDIT `paper/sim2science/main.tex` BEFORE 29 SEP.** Branch from the tag if something
> truly cannot wait. All post-deadline work belongs in `paper/main.tex`.

> ### CAN THE TAG SEED A CLEAN PUBLIC REPO FOR REVIEWERS? YES — WITH ONE HARD RULE.
> Asked by Vinayak on submission day, and the answer was already half-recorded in the
> fresh-repo plan. **The tag is exactly the right starting point**, because it is the only
> reference to the state the reviewers are actually reading.
>
> **THE HARD RULE: COPY THE FILES INTO A NEW `git init`. NEVER `git clone`, NEVER push this
> repo's history anywhere.** Cloning carries the whole history with it, and this history
> contains **every HANDOFF snapshot** — the mentor's quoted emails, the venue strategy, the
> real three-author byline, the internal deliberations, and the Overleaf URL. GitHub shows
> deleted files in their commit diffs, so "it is not in the tree" hides nothing.
> ```
> git worktree add ../hh-clean sim2science-submitted   # exact submitted state, safely
> # then copy the files you WANT into a fresh directory and run git init there
> ```
>
> **BUT DO NOT SHARE IT WITH A REVIEWER BEFORE THE NOTIFICATION.** The submission is
> DOUBLE-BLIND and checklist Q5 states, on the record, that the code is withheld at submission
> and released on acceptance. A GitHub repo under Vinayak's own account deanonymises the paper
> the moment a reviewer opens it. **If a reviewer asks for code during review:** the honest
> answer is what Q5 already says. If you decide to supply it anyway, it must go through an
> anonymising host (e.g. `anonymous.4open.science`), never a named account — and that is a
> decision for Vinayak, not a session.
> **After 29 Sep, if accepted, the clean repo is exactly right** and discharges the release
> promise in Q5, Q13 and the "Data and code" paragraph. See also **A10** and **A13**.

> ## ★ APPROVED TO SUBMIT — 2026-08-30. HIS WORDS: *"You can now proceed to submission."*
> Prathamesh replied to N21 and cleared all three points. *"Footnote updated, language looks
> better."* On the checklist guidelines: *"I mentioned it as optional, so it is fine if you
> keep them."* **THE WRITING IS DONE. THERE IS NOTHING LEFT TO EDIT.** What remains is the
> form, a tag and a banner.
>
> ### HIS ONE REUSABLE POINT, AND IT IS A GOOD ONE
> *"Different workshops have different templates. The basic guideline that the template has is
> always with respect to the main conference. Workshops may have different templates; many
> follow NeurIPS, some allow the checklist and appendix, and some do not even allow an
> appendix. It is always better to look at the workshop website to check the explicit
> guidelines."*
> **WE DID, AND THAT IS WHY THIS PAPER IS SHAPED THE WAY IT IS.** The CFP at
> `sim2science.com/cfp.html` was read end to end on 2026-08-28 and it is the authority for all
> four structural decisions: the 5-page body, the unlimited appendix, the checklist placed
> *between* the references and the appendix, and the single PDF upload. He confirms the last
> one himself: *"This workshop's main highlight is to have the checklist between the references
> and the appendix, which we have followed."*
> **Carry the principle to the next venue: read the WORKSHOP's own page, and treat the main
> conference template as the fallback, not the spec.** Where the two collide, this project has
> ruled for the template only on typography it explicitly mandates (table captions above, keep
> the checklist guidelines) and for the CFP on everything structural.

**1. ~~N21 — SEND THE DRAFT TO PRATHAMESH.~~ SENT 2026-08-29. APPROVED 2026-08-30.** Draft:
`paper/draft-email-N21-prathamesh.md` (this SUPERSEDES `draft-email-N9-prathamesh.md`, which
was the reply to review 2 — do not send that one). Fact-checked by three agents before sending;
13 problems found and fixed, including two of his asks the first version silently dropped.
~~Now waiting on his final read.~~ **RECEIVED AND APPROVED 2026-08-30 — see the box above.**

> ### ⚠ CHECK THE OPENREVIEW COUNTDOWN BEFORE ANYTHING ELSE. THE TWO DATES NEVER RECONCILED.
> The CFP says **2 Sep 23:59 AoE**. The live OpenReview form still read **"Deadline: Aug 30,
> 2026, 5:29:00 PM"** when it was last looked at, on 28 Aug. **Today is 30 Aug.**
> **The form is what stops accepting uploads, not the CFP.** If it still shows 30 Aug, the
> runway is HOURS, not days, and everything below must happen now. Open the submission page
> and read the countdown first; only if it shows the later date is there slack.
> Nothing is blocked — the paper is approved, the bundle is built and every check is green —
> so this is purely about not discovering the earlier date at 17:30.

**2. ~~THE BLOCKER — Raj's OpenReview profile ID.~~ CLEARED 2026-08-29.** No ID string is
needed: type **"Raj Dandekar"** into the form's author box and select him from the dropdown
(Vizuara AI Labs, principal researcher). Confirmed by Prathamesh on video.
**THERE IS NO BLOCKER LEFT.** Everything from here is a form to fill in and two git commands.

> **IF HIS READ COMES BACK WITH CHANGES, READ THIS FIRST.** The body has **zero margin** and
> the compile is now measured (body ends p. 5 line 178, References opens p. 6). Any body edit
> must be **length-neutral or shorter** — enforce it mechanically with `new_chars <= old_chars`,
> not by eye. Appendix and checklist edits are free. The LENGTH DIAL at the top of the cut lists
> every remaining lever and what each costs; everything cheap is already spent.
> **If his changes cannot be paid for, recompile and re-check where the References head lands
> before submitting — do not assume.**

**3. N10 — SUBMIT** at `NeurIPS.cc/2026/Workshop/Sim2Sci`. Get every field right; the form is
where a finished paper still gets desk-rejected.
  - **Upload ONE pdf, not two.** Body ≤5 pp + references + checklist + appendix A–E, one file.
    There is a single `PDF *` field and no supplementary slot — verified on the live form.
  - Reciprocal reviewer: **remove Vinayak, add `~Prathamesh_Dinesh_Joshi1`** (or search the
    full name and pick from the dropdown; his author line imports automatically).
  - Licence: **CC BY 4.0**. Submission Track: **Workshop Paper**.
  - Abstract: paste the **RENDERED** text, not the LaTeX source. It contains no `\val` macros —
    checked — but it does contain `$R^2$`, which must be typed as **R²** in the form.
  - **All authors must be listed at submission. None can be added after review opens.**

**4. F3 — TAG THE SUBMITTED STATE.** `git tag -a sim2science-submitted` on **the commit whose
bundle produced the UPLOADED pdf**, not a later one, then `git push origin sim2science-submitted`.
Get the commit right: if you rebuilt the bundle after committing, the tag still belongs on the
commit whose source produced that build.

**5. F4 — FREEZE.** Add the FROZEN banner to the top of `paper/sim2science/main.tex` so no
later session edits a submitted artifact by accident.

#### AFTER THE DEADLINE — nothing here is urgent, all of it is real

| # | Item | Notes |
|---|---|---|
| **A11** | Prathamesh's second round on the **archival** paper. | He sends it after Sim2Science. Not on the critical path. |
| **A10** | Decide the destination: **arXiv, a journal, or both.** | Interacts with the OpenReview licence choice and with the fresh-repo plan. |
| **A12** | Restore what the cut dropped for anonymity or length. | The author-overlap disclosure is **mandatory** to restore, not optional. |
| **A13** | **Fix the bundler: it DELETES the author-overlap disclosure instead of blinding it.** | Found 2026-08-29. Full box in §13's A-series. Affects A10 only — `supplementary.tex` is not submitted. |
| **A10 — NARROWED 2026-09-08** | Destination: **a Bio-ML journal.** Prathamesh: *"I am strongly inclined to target a Bio-ML related journal for this work, as I believe it would be a perfect fit."* | **Front-runner: npj Systems Biology and Applications** — Nature Portfolio, squarely Bio-ML, rolling submission, and it published Philipps et al. 2025 *"Current state and open problems in universal differential equations for systems biology"*, which this study is a direct empirical answer to. **VERIFY THE APC BEFORE COMMITTING — it was never confirmed.** Alternatives: TMLR (its criteria are technical correctness and audience interest, not novelty — which is this paper by construction), Neural Computation, J. Comput. Neurosci. **Assessed and ruled out: PNAS and Nature Communications** — out at every achievable version, because the cap is set by the KIND of result, not its execution. **PLOS Comp Biol** is reachable only as a Research Article with a real-data arm. **eLife** charges $3,750 when it commits to review and publishes a permanent public assessment either way — a bad bet here. |
| **P1–P9** | The presentation pass, **still OPEN for the archival only.** | P1 (AI-tells), P5 (notation) and P6 (shorten Discussion) matter most. P7 = leave table captions ABOVE. Includes the **"baseline" overloading** defect: `main.tex:736` calls the neural ODE "the baseline" (a MODEL) while four other sites say "the baseline condition" (a SETTING). Pre-existing, archival-only, do NOT fix under deadline pressure. |
| **P1 — OWED, NOT OPTIONAL** | Carry the review-3 language pass into `paper/main.tex`. | **We told Prathamesh we would.** `main.tex:296` "This two-stage recipe" and `main.tex:259` "Nor is it under-training" both survive. Use `Paper Writing Skills/references/writing-craft.md` §4.7 and `examples/hh-sciml/humanized-passage.md`, which is a worked before/after on a paragraph of this very paper. **Two agents edited the archival by mistake in the voice pass and were reverted — see the process-failure box.** |
| **D3** | Add `neurips_2026.sty` + `checklist.tex` to the `LICENSE` third-party carve-out. | Both files' own headers already say the carve-out should name them. `LICENSE:38-52` currently names only `paper/arxiv.sty` and `Vizuara_Paper_HH.pdf`. |
| **T4–T8** | The remaining housekeeping tasks in §13. | |
| — | **The fresh-repo decision, if double-blind survives to camera-ready.** | This repo is PUBLIC and its history carries every HANDOFF snapshot. See the snapshot warning at the top of this file. |
...
### N11 IS ANSWERED FROM THE FORM ITSELF (2026-08-28). HE WAS RIGHT.

Vinayak opened the live submission form and screenshotted it. **Site verified as correct:**
`openreview.net/group?id=NeurIPS.cc/2026/Workshop/Sim2Sci`, "Sim2Science: ML with Imperfect
Scientific Models / Sim2Sci 2026", Paris, `sim2science.com`, venueid
`NeurIPS.cc/2026/Workshop/Sim2Sci/Submission`, **deadline Aug 30 2026 5:29:00 PM** — which
is the 17:29 IST we have been working to. Same venue, same deadline.

**THE FORM HAS EXACTLY ONE FILE FIELD: `PDF *` — "Upload a PDF file that ends with .pdf."
THERE IS NO SUPPLEMENTARY-MATERIAL FIELD.** Prathamesh is confirmed. **The restructure
(N13–N17) is required, not conditional.**

**The complete field inventory, because several of these have never been thought about:**

| field | state | note |
|---|---|---|
| Submission Track * | `Workshop Paper` | the other option is Tiny Paper (2 pp). Correct as set. |
| Title * | empty | *Predict, then interpret: how far a UDE gets on a hidden ionic current* |
| Authors * | **only Vinayak Mokashi** | *"All authors must have an OpenReview profile prior to submitting a paper."* See N12. |
| Keywords * | empty | reuse the archival `\keywords` line. |
| TL;DR | empty, optional | worth writing; it is the first thing a reviewer sees. |
| Abstract * | empty | **PASTE THE RENDERED TEXT, NOT THE SOURCE** — the cut's abstract is full of `\val` macros that resolve to numbers. TeX is supported for maths, but `\valUdeForecastVRmseMeanStd` will not resolve. |
| **PDF *** | empty | **THE ONLY UPLOAD. One file.** |
| Venueid * | prefilled | leave. |
| Nominated Reciprocal Reviewer * | **Vinayak Mokashi is in the box**; `~Prathamesh_Dinesh_Joshi1` typed into search, resolves to *"Prathamesh Dinesh Joshi ✓ — Researcher at Vizuara AI Labs"*, **not yet added** | *"every submission must provide **one** author to serve as a reviewer."* Prathamesh volunteered and is the one with the publication record the CFP asks for, so **remove Vinayak (✕) and add Prathamesh (+)**. |
| Email Sharing * | unset | radio: authorise sharing author emails with Program Chairs. |
| **Data Release *** | unset | *"accepted submissions, along with their author names, will be released to the public after the conference is over."* **This is F1 confirmed from the venue itself, not inferred from the CFP — accepted ⇒ public. The F-series freeze discipline is load-bearing.** |
| **License *** | **`Select License…` — UNCHOSEN, and REQUIRED** | **Nobody has ever decided this.** It is not the MIT/CC-BY-4.0 split the paper already states for code and results; it is the licence on the OpenReview record itself. It interacts with **A10** (arXiv or journal later). **ASK VINAYAK.** |
| Readers *, Signatures * | prefilled | leave. |

> ### THREE THINGS THE FORM SURFACED THAT NOBODY HAD PLANNED FOR
> 1. **The License field is required and undecided.** Blocks submission.
> 2. **Raj Abhijit Dandekar has no known OpenReview profile ID.** Prathamesh's is
>    `~Prathamesh_Dinesh_Joshi1`; Raj's is not recorded anywhere in this project, and the
>    form refuses authors without a profile. **Get it before submission day.**
> 3. **Does the appendix count toward the five pages?** The CFP says *"5 pages (excluding
>    references)"* and says nothing about an appendix. The NeurIPS-family convention is that
>    the checklist and appendix follow the references and do not count, and Prathamesh
>    submitted his own paper there with an appendix — but **if the appendix does count, N14
>    is impossible as specified.** Verify on the CFP before writing four pages of it.

### THE CFP, READ END TO END (2026-08-28). THE DEADLINE MOVED AND THE APPENDIX IS FREE.

Vinayak flagged a possible date change; verified on `sim2science.com/cfp.html` and the
front page. **Five findings, three of which change the plan.**

> ### 1. THE DEADLINE IS EXTENDED TO **2 SEP 2026, 23:59 AoE** = **3 SEP, 17:29 IST**.
>
> **CONFIRMED BY VINAYAK, 2026-08-28: "The deadline is on 2nd September for sure. I
> verified it."** The CFP-vs-form disagreement below is CLOSED and the working deadline is
> the later one. The paragraph that follows is kept because the REASONING is reusable -- when
> a CFP page and a submission form disagree, the form is what stops accepting uploads -- but
> it is no longer a live instruction. Do not re-derive a 30 Aug deadline from it.
> The CFP prints it struck through: *"Submission deadline: ~~Aug 29, 2026~~ Sep 2, 2026
> (23:59 AoE)."* The front page shows the same strike-through, plus reviewing starts 8 Sep,
> reviews due 23 Sep, **notification 29 Sep**, workshop 12 or 13 Dec, Paris.
>
> **DO NOT RELAX ON THIS YET.** Vinayak's screenshot of the live OpenReview form, taken
> 03:26 on 28 Aug, still read **"Deadline: Aug 30, 2026, 5:29:00 PM"**. The CFP page and the
> submission form disagree, and **the form is what actually stops accepting uploads**.
> **Treat 30 Aug 17:29 IST as the working deadline until the OpenReview form itself shows
> the later date.** Re-check the form; if it still says 30 Aug, submit by 30 Aug.

> ### 2. THE APPENDIX IS UNLIMITED, AND THE BODY MUST STAND ALONE.
> *"no page limit on supplementary/appendix material, but reviewers are not obligated to
> read it — your main submission (within the page limit) should be self-contained."*
> Also: the checklist *"does not count toward the page limit"*.
>
> **Three consequences, and the third is a correction to something written earlier today:**
> - **N14's "4–5 pages" is Prathamesh's taste, not a rule.** There is no cap. His advice to
>   stay concise is still good, but it is advice.
> - **"Reviewers are not obligated to read it" now attaches to the APPENDIX**, exactly as it
>   used to attach to the supplementary. So the old *available-not-guaranteed-seen* problem
>   did not disappear with the supplementary — **it moved**. That is precisely why N13's
>   Stage-1 figure has to go in the **body**.
> - **CORRECTION to §4n's "the appendix is the new space lever".** It is a lever for
>   *detail*, not for *substance*: the CFP requires the five-page body to be
>   **self-contained**. Methods detail can move out; anything a reviewer needs in order to
>   follow the argument cannot. Do not hollow out the body to buy figure space.

> ### 3. THE CHECKLIST PLACEMENT IS CONFIRMED, AND SO IS PRATHAMESH.
> *"placed after the references and before the appendix; it does not count toward the page
> limit."* Exactly what he said. **N16 stands as written.**

> ### 4. N18 IS CLOSED — WE ARE ALREADY COMPLIANT.
> The CFP names the required setup verbatim: *"`\usepackage[dblblindworkshop]{neurips_2026}`
> and `\workshoptitle{Sim2Science}`"*, and warns *"Submissions not using the required
> template may be desk-rejected."* **`sim2science/main.tex:90-91` is byte-for-byte those two
> lines.** The CFP says **nothing** about the footer text. So Prathamesh's footer concern is
> answered: the generic "Submitted to 40th Conference…" is the template's own submission
> behaviour, we are compliant, and **there is nothing to change.** The `final` trap in the
> footer box below stands as a warning, not as a task.

> ### 5. NEW, AND IT IS A COURTESY WE OWE PRATHAMESH.
> *"each nominated reviewer is assigned **2 other submissions** to review."* He volunteered
> before that number was on the table. **Tell him in the next email** — it is a real
> commitment and he should accept it knowingly, not discover it after submission.

### N12 IS DECIDED (2026-08-28, Vinayak's call, as Prathamesh insisted)

**Author list for the workshop submission: Vinayak Mokashi, Prathamesh Dinesh Joshi, Raj
Abhijit Dandekar.** Not Rajat Dandekar, not Sreedath Panat. His reason, recorded verbatim
because the decision is his: *"We should add Prathamesh and Raj. No one else imo cause I have
interacted with only them and no one else."*

> **THIS DIVERGES FROM THE ARCHIVAL AND TWO THINGS ARE COUPLED TO IT — RAISED, NOT ACTED ON.**
> `paper/main.tex:172` lists **five** authors (adds Rajat Dandekar and Sreedath Panat). So:
> 1. The same work would carry a **three-author workshop byline and a five-author archival
>    byline**. That is a question for Vinayak, not a defect — but it should be a decision,
>    not a side effect of only the workshop paper being edited.
> 2. **D4's arithmetic breaks.** The archival's author-overlap disclosure says
>    `kainth2025pinodesr` *"shares four of this paper's five authors"*. With a three-author
>    byline the overlap is two of three, and that sentence becomes wrong. It is currently
>    neutralised in the anonymised build and **must be restored correctly at camera-ready**.
> **CONFIRMED IN ONE LINE AND DONE, 2026-08-28: "Yes, cut the archival to the same three
> authors."** `paper/main.tex` now carries **Vinayak Mokashi, Prathamesh Dinesh Joshi, Raj
> Abhijit Dandekar** and the two bylines match. **A12/D4 for the byline is CLOSED.**
>
> **FOUR THINGS WERE COUPLED AND ALL FOUR WERE DONE IN ONE PASS.** Doing any one alone is
> how this project's consistency debt is created:
> 1. the `\author{}` block, 5 -> 3;
> 2. `\hypersetup` **`pdfauthor`**, which also listed all five;
> 3. **D4's disclosure**, "shares four of this paper's five authors" -> "**two of this
>    paper's three**". **The count was re-derived from `references.bib`, not assumed:**
>    `kainth2025pinodesr` is Kainth, Joshi, Raj Dandekar, Rajat Dandekar, Panat, so against
>    the old five the overlap was Joshi+Raj+Rajat+Panat = four, and against the new three it
>    is Joshi+Raj = two. **If the byline ever changes again, re-derive it the same way.**
> 4. **`scripts/make_sim2science_bundle.py`'s `SELF_ID_CLAUSE` regex, which nothing would
>    have flagged until build time.** It hard-coded `four ... five` and asserts the sentence
>    matches EXACTLY ONCE, so editing the sentence alone makes the bundler refuse to build.
>    **The two number words are now `\w+`**, so a future byline change cannot break it, while
>    a reworded or deleted sentence still fails loudly. Verified by rebuilding the bundle.
>
> The anonymiser now strips **6** identifying comments, up from 3, because the notes left at
> the author block name the removed authors. That is the guard working, not a leak.

### THE REMAINING FORM DECISIONS (2026-08-28)

- **Licence: CC BY 4.0.** Confirmed by Vinayak. It is also the licence the paper already
  names for the results tables and figures, so the submission record and the artefact
  licensing agree \u2014 and CC BY is the permissive choice that leaves **A10** (arXiv, or a
  journal later) unobstructed.
- **Raj Abhijit Dandekar's OpenReview profile ID: NOT KNOWN, and we are asking Prathamesh
  for it in the next email.** Raj is going on the author line; the form refuses authors
  without a profile, so **this is a hard blocker on pressing Submit.** It is now an explicit
  ask in `paper/draft-email-N9-prathamesh.md`.
- **Reciprocal reviewer: remove Vinayak from the box, add `~Prathamesh_Dinesh_Joshi1`.**
  The CFP wants one author with "sufficient relevant expertise and publications".

### Q8 COMPUTE: NO LOGS EXIST, BUT THE DISK STILL CARRIES EVIDENCE

**Searched and found nothing instrumented:** no `.log` files anywhere in the tree, and **no
timing, elapsed, duration or wall-clock column in any results CSV**. So the honest position
in the current checklist ("we did not record those figures") is factually right.

**But the trained-parameter snapshots are written as each run finishes, so their mtimes
bracket real runs.** `results/params/*.jld`, the gCa sweep of 6–7 Aug 2026, ran twelve UDE
trainings back to back:

```
gCa=0.4  seeds 2222 -> 3333 -> 4444 -> 5555   10.9, 10.7, 10.5 min
gCa=1.0  seeds 2222 -> 3333 -> 4444 -> 5555   11.1, 11.1, 11.2 min
gCa=4.0  seeds 2222 -> 3333 -> 4444 -> 5555   10.8, 11.2, 16.0 min
```

**≈ 11 minutes per UDE training run (Adam \AdamUde{} + BFGS \BfgsUde) on this machine**,
including the symbolic post-processing between runs. The many 0.0-minute gaps in the same
directory are **bulk file copies** (identical timestamps, 03 Jul 01:07 and 19 Aug 11:00) and
are not runs — do not quote them.

**Hardware, from Vinayak:** CPU only, **no GPU**, **16 GB RAM**. His recollection of the
total is *"everything run under 12–14 hours"*, and that is consistent with ~11 min × the run
count in the design (roughly fifty UDE-equivalent runs plus six aggressive-retrain runs at
four times the budget) — **but it is a recollection, and the checklist must not print it as
a measurement.**

> **THE HONEST Q8, and it is stronger than either "No" or a remembered total:** state the
> hardware as fact (CPU-only, no GPU, 16 GB), state the **per-run cost as measured from the
> snapshot timestamps** with the method named, state that wall-clock was never instrumented,
> and give the run count from the experimental design so a reproducer can multiply. That
> answers what Prathamesh asked for without stating from memory what we said we would not.
...
python scripts/check_undefined_macros.py
python "Paper Writing Skills/scripts/check_numbers.py" "paper/sim2science/main.tex" --macros "paper/generated/metrics.tex"
```
> **THE TWELFTH CHECKER EXISTS (2026-08-28): `scripts/check_undefined_macros.py`.**
> Gap (a) is CLOSED. It collects every `\command` in the live text, subtracts what the file
> defines, what it `\input`s, what the sibling `.sty` files define and a curated
> base-LaTeX/loaded-package whitelist, and reports the remainder. **It found a real defect on
> its first run:** the N14 appendix used `\ProfileStep`, which is defined in `paper/main.tex`
> and had not been carried into the cut's preamble -- an "Undefined control sequence" that
> `check_workshop_cut.py` cannot see, because that one resolves `\val` macros and `\cite`
> keys only. Calibration: it is CLEAN on `paper/main.tex`, which has compiled, so the
> whitelist is not merely permissive. It also runs on the BUILT bundle
> (`python scripts/check_undefined_macros.py sim2science_upload/main.tex`), which is the
> stricter test because that is the tree Overleaf actually sees.
> **If it reports a base-LaTeX command it does not know, ADD IT TO `BASE` with a note. Do
> not delete the check.** tikz commands are excused only in a document that loads tikz.

> ### GAP (b) IS CLOSED PROPERLY: `check_claims.py` TAKES BOTH MANUSCRIPTS AT ONCE.
> ```
> python "Paper Writing Skills/scripts/check_claims.py" --claims "paper/claims.yaml" --tex "paper/main.tex" "paper/sim2science/main.tex" --metrics-tex "paper/generated/metrics.tex" --metrics-json "paper/generated/metrics.json" --figures-root "."
> ```
> **=> 32 claims, 28 anchored, 0 errors, 16 warnings, PASS.** This is the command to use.
>
> **CORRECTION to what was written here earlier on 2026-08-28.** The first attempt passed
> `--tex paper/sim2science/main.tex` ALONE, got 13 `E-CLAIM-NOT-ANCHORED` errors, and this
> file recorded them as an expected subset artifact to be read past. That reasoning was
> sound but the conclusion was wrong: **`--tex` is variadic.** Point it at BOTH manuscripts
> and every claim the ledger holds is anchored in one or the other, so there is no backlog
> to read past and no judgement call to make. **Do not run it against the cut alone** -- a
> real defect would be buried in 13 spurious errors, which is exactly how a checker stops
> being read.
...
>
> **1. ~~Nothing verifies that a command USED is actually DEFINED.~~ CLOSED 2026-08-28 —
> `scripts/check_undefined_macros.py` is the twelfth checker. See the box above. It caught a
> real undefined macro (`\ProfileStep`) in the N14 appendix on its first run.**
>
> **2. ~~`check_claims.py` has NEVER been run against the 5-page cut.~~ RUN 2026-08-28.**
> Result and how to read its FAIL are in the box above. The cut's anchors are all valid; the
> 13 errors are subset artifacts. Still worth running after any edit to the cut's anchors.
>
> **3b. A THIRTEENTH CHECK was added to `check_tex_sanity.py` on 2026-08-28, and
> it closes a sibling of the mid-line-% defect.** A comment at the TRUE END of a
> line still needs a **SPACE before its `%`**. TeX discards everything from `%`
> to the end of line INCLUDING THE NEWLINE, so the newline's interword space
> goes too: `... centred on% numok:` followed by `\emph{that} estimate` typesets
> as **"centred onthat"**. `check_numbers` reads the numok as a valid exemption
> and is happy; the mid-line check only fires when PROSE follows the `%`. The new
> check flags a line-end comment with no preceding space **only when the next
> line actually begins with text** -- a line-end `%` before `}` is the CORRECT
> idiom and is how the `\GenericError` stale-upload canary in both preambles
> suppresses a spurious space, which is why that exemption exists.
> Verified both ways: silent on both manuscripts (84 numok comments, 0 flagged),
> and it FAILS on a copy with one space deleted. It was added because moving a
> sentence between sections during the N13-N17 cut put four numok comments at
> line ends at once.
>
...
>
> **THIS FILE HAS NOW YIELDED A DEFECT IN EVERY SINGLE ROUND THAT READ IT** — Q7 (round 9),
> Q2 and Q12 (N16), Q8 (round 10), Q4 and Q6 (round 11), and Q4 AGAIN on round 11's sibling
> sweep. Six of sixteen answers have been wrong at some point. **It is the highest-defect-rate
> file in the project and it is the only one no tool reads. WALK ALL SIXTEEN BY HAND AFTER
> ANY CHANGE TO THE PAPER, and when you fix one answer, immediately check whether its
> neighbours make the same claim** — round 11's Q6 fix left the identical omission standing
> in Q4 one question earlier. Two cheap invariants a future checker could enforce: every
> `Sec.~N`/`App.~X` a justification names must exist, and no justification may repeat a
> number that also lives in the appendix.
...
### Added 2026-09-11, all four earned during the E1–E3 round

- **JULIA SOFT SCOPE BIT TWICE IN ONE SESSION, AND THE SECOND TIME WAS AFTER FIXING THE FIRST.**
  `n += 1` or `worst = d` inside a **top-level** `for` creates a new local; the read throws
  `UndefVarError`. It killed the self-test in `parametric_matched_objective_28seed.jl`, was
  fixed by wrapping in a function, and was then written again in `init_vs_data_grid.jl` and
  killed that one too. **The rule is not "remember the gotcha" — it is NEVER ACCUMULATE AT TOP
  LEVEL IN THIS CODEBASE.** Wrap every loop that accumulates in a function, always.
  Worse, it fails LATE: both times the loop printed its first line before throwing, so the
  output looked like a working check that had merely stopped early.
- **A TRAILING `echo` MASKS THE REAL EXIT CODE FROM THE HARNESS.** Launching as
  `nohup julia … > log 2>&1; echo "EXIT=$?"` makes the *compound* command's status `echo`'s,
  which is always 0. A run that genuinely exited 1 was reported as "completed (exit code 0)"
  and was only caught because its output looked truncated. **Do not append an echo to a
  backgrounded run.** Same family as the `git commit` gotcha above: ask for the real status,
  do not read a wrapper's.
- **`_calcium_id` (`src/experiment.jl:274`) APPENDS `_seed<n>` UNLESS THE TAG ALREADY ENDS IN
  ONE.** A tag like `e3_sse_i1111_d1111` therefore lands as
  `probe_e3_sse_i1111_d1111_seed1111.csv`. This broke a Python glob loudly and a Julia resume
  check **silently** — it tested a path that could never exist, so it would have re-run every
  cached cell on any restart. **Derive probe filenames from `_calcium_id` itself, never from a
  restatement of its rule.** Same failure shape as the `stage_figures.ps1` map above.
- **THE SENSEALG IS NOT THE OPTIMISER, AND FOR FEW PARAMETERS THE PUBLISHED ONE IS THE WRONG
  TOOL.** `src/experiment.jl:75` sets `InterpolatingAdjoint(ReverseDiffVJP(true))` — correct
  for the closure's 337 weights, catastrophic for 2, where it buys a full backward ODE solve
  for a gradient forward mode gets nearly free. Measured **0.083 s/grad vs 0.006 s/grad**,
  which was the difference between a 38-hour grid and a 4-hour one. `ForwardSensitivity()` is
  a legitimate substitution **only because it is gated**: `parametric_adam_bfgs.jl` recomputes
  the gradient under both and fails if they differ by more than 1e-5 (measured 9.4e-08).
  **Check the gradient at a point OFF the optimum** — at the optimum two near-zero vectors
  agree and the check proves nothing.

### Added 2026-09-12, from the decomposition round

- **`consistency.yaml` MATCHES PHRASES, NOT MAGNITUDES.** "At most a factor of two" in the
  abstract and "4.69×" in §4.8 both passed every checker, because they sit in different
  sentences and no regex compares numbers. **When a new result supersedes a BOUND, inventory
  every site of the old bound BY MEANING** — grep the words it was written in ("twofold",
  "threefold", "six times", "larger share", "two-fifths", "at most") across `paper/main.tex`
  AND `README.md` AND the cut — then add a `forbid` group for the old phrasing.
- **`paper/generated/metrics.tex` IS SHARED WITH THE FROZEN CUT.** Before every regeneration,
  copy it; after, diff the SORTED `\newcommand` sets. **"0 existing changed" is the invariant
  that keeps a paper under review from moving.** Adding macros is safe; changing one is not.
- **`compile_rx` squeezes the whitespace a folded YAML alternation inserts**
  (`scripts/check_consistency.py`), so a `forbid: >-` written across several lines DOES work.
  I nearly "fixed" a working guard on the opposite belief; read `compile_rx` before assuming.
- **THE 28-SEED COUNT MUST BE WRITTEN ONLY AS `\valNwSeeds`** in either manuscript.
  `seed-count-is-five-everywhere` forbids any spelled or digit count but five, and records this
  exception. The README has no macros, says 28 in plain text, and is outside that group — **so
  its numbers (28, 45–62 %) are HAND-TYPED and must be updated by hand if the macros move.**
- **GUARD TESTS MUTATE `paper/main.tex` — never run two at once.** They race on the file.
- **`2>&1` on `git` in PowerShell 5.1 produces `NativeCommandError` noise** even when git
  succeeded (bit again 2026-09-11). Omit the redirect; stderr is captured anyway.
...
>
> **ROUND 10 RAN ON 2026-08-29 AND FOUND FOUR REAL DEFECTS** (§4o), so the old advice not
> to audit again was wrong in the event: the axes were not exhausted, they had simply not
> been pointed at the NEW prose. The productive axes have been: prose in `main.tex`
> (rounds 1–6) · figures and tables (rounds 5, 7) · the cut internally (rounds 6, 8, 9) ·
> cross-manuscript agreement (rounds 9, 10) · the compiled PDFs · `checklist.tex` ·
> mentor-ask coverage (round 10, and it was the round's best finding).
> **The rule that actually holds: audit whatever was WRITTEN since the last audit.**
> §4i's profile for fresh writing is "every finding was an error introduced in the
> writing", and rounds 8, 9 and 10 all bore that out.
>
...
> ### ~~VERIFY THIS FIRST.~~ ANSWERED 2026-08-28 FROM THE LIVE FORM: THERE IS NO SLOT.
> One `PDF *` field, no supplementary-material field. The restructure was required and
> **N13–N17 and N19 are all DONE** (§4o). Nothing here is conditional any more. Kept only
> so nobody re-opens the question: the answer is no slot, one PDF.
...
> **Deadline: EXTENDED to 2 Sep 23:59 AoE = 3 Sep 17:29 IST. VERIFY ON THE FORM — it still read 30 Aug 17:29 on 28 Aug, and the form is what closes (§4n).**
...
> ### RAISED AND DECIDED 2026-08-28: the archival's "baseline condition" stays.
> The length-cut analysis flagged that `paper/main.tex`'s Fig. 2 caption says
> *"at the baseline condition"* where the 5-page cut now says *"at the baseline
> condition ($g_{Ca}=\gcaMain$)"*, and called it a consistency-debt seed.
> **Decision: do NOT align them, and here is why, so it is not re-raised.**
> "Baseline condition" is a defined internal term used consistently at FOUR
> archival sites (main.tex:1053, 1079, 1127, 1196). Spelling the value out at one
> of them creates the inconsistency it was meant to cure. The two documents
> differing here is correct adaptation rather than drift: five pages cannot lean
> on a term defined in a Methods section a reviewer may skim, and the archival
> can. No checker and no claim binds them.
>
> **But the analysis brushed against something real, and it is a P5 item.**
> "Baseline" is OVERLOADED in the archival. `main.tex:736` calls the black-box
> neural ODE *"the baseline"* (a MODEL); the four sites above say *"the baseline
> condition"* (an EXPERIMENTAL SETTING). Fig. 2's caption uses BOTH senses three
> lines apart: *"representative seed at the baseline condition"* and *"That
> configuration is not the baseline we rest any comparison on"*. That is a
> genuine legibility defect, it is pre-existing, it is archival-only, and it
> belongs to the P5 notation pass. **Do not fix it under deadline pressure** --
> renaming one sense means touching every site of both, which is precisely the
> kind of edit that seeds the next audit round.
...
| **N10** | **Nominate Prathamesh as reciprocal reviewer** — ID **`~Prathamesh_Dinesh_Joshi1`** (or search the full name, pick from the dropdown; his author line imports automatically) — and submit at `NeurIPS.cc/2026/Workshop/Sim2Sci`. | N11–N17 | All authors listed at submission; none can be added after review opens. **UPLOAD ONE PDF, NOT TWO.** `main.pdf` only -- body + references + checklist + appendix. The form has a single `PDF *` field and no supplementary slot (verified on the live form, §4n). This row said "upload BOTH PDFs" until 2026-08-29; that instruction was a desk-reject waiting to happen. **The CFP wording: "at submission time, authors nominate one author of the paper -- someone with sufficient relevant expertise and publications -- to serve as the paper's reciprocal reviewer."** |

> ## THE 5-PAGE PAPER FITS. CONFIRMED ON THE COMPILE, 2026-08-28.
> Body ends at line 177 at the foot of page 5; the **References head sits at the top of
> page 6**, which is the target state (the CFP excludes references, checklist and appendix
> from the limit). N13-N17, N19 and D1 are all done, all twelve checkers are green, and the
> bundle is rebuilt and md5-matches its source.
>
> **THE MARGIN IS ZERO, AND THAT IS THE THING TO CARRY FORWARD.** Pages 3, 4 and 5 are all
> full. Three cut passes spent ~12 lines getting here and everything cheap is gone. **One
> added sentence puts the body back on page 6.** The LENGTH DIAL at the top of
> `paper/sim2science/main.tex` lists what is left and what each option costs -- every one of
> them costs something real now. If Prathamesh's next round asks for added text, something
> else has to come out in the same edit, and the dial says which.

> ## ROUND 10: THE THREE-DOCUMENT AUDIT (2026-08-29). FOUR DEFECTS, ALL FIXED, ALL FREE.
> Solo audits of each manuscript, then a cross-document round, then adversarial refutation.
> **Verdict: the three documents ARE consistent** -- every `al` macro in the cut's
> appendix also appears in the archival, the two parameter tables are row-for-row identical,
> the profile interval matches including provenance, and the byline change landed at all
> three coupled sites with the overlap count re-derived from `references.bib`.
> **5 of 9 candidate findings were REFUTED**, several because they proposed narrowing a
> qualifier that `claims.yaml` explicitly forbids narrowing.
>
> **D1. No hyperparameter TABLE.** He asked for "one table with complete training
> hyperparameters"; App. B had all of it as prose. **FIXED** -- `tab:hyper`, built entirely
> from existing preamble macros, so no new number and no new `numok`.
>
> **D2. The appendix shipped ZERO figures.** Of the eight things the deleted "supplementary"
> paragraph had promised, six landed in App. A-D and two -- the forecast overview and the
> voltage-only overview -- landed nowhere. **The voltage-only forecast RMSE is still quoted
> in the body**, so a reported number was shipping with no picture in a submission whose
> premise is that there is nowhere else to look. §4n said RELOCATE, not drop. **FIXED** --
> new App. E carries both, captions lifted from the archival WITH their guards (the fig5
> caption must say the neural ODE bars are the ORIGINAL configuration whose solve does not
> stay finite, so that error bar is not a dispersion estimate).
>
> **D3. Checklist Q8 hand-duplicated App. C's numbers.** NOTHING verifies `checklist.tex`,
> so the next revision of the 11-minute figure would update the appendix and leave the
> checklist asserting the old one INSIDE THE SAME PDF. **FIXED** -- Q8 now points at App. C
> and keeps only the qualitative answer.
>
> **D4. THE SERIOUS ONE, AND IT WAS SELF-INFLICTED.** App. C said "a single consumer laptop"
> and four lines later "a shared desktop", and Q8 repeated the pair while framing it as
> "Hardware is stated as fact". **Both form factors were invented.** §4n records Vinayak's
> words as *"CPU only, no GPU, 16 GB RAM"* and nothing more; "laptop" appears ZERO times in
> this file and "shared desktop" appears nowhere at all. **FIXED** -- the form factor is
> gone from both places and the elapsed-vs-processor-time caveat now names the CONFOUND
> ("a machine that was also running other work") instead of a machine.
> **THE LESSON: "stated as fact" is a promise. Before writing it, find the source. An
> unsourced detail that sounds mundane is exactly the one nobody re-checks.**
>
> A rewrap during the D4 fix moved a literal `16` off its `% numok:` line and
> `check_numbers` caught it immediately -- **the numok must sit on the SAME line as the
> literal it exempts**, which is a second reason never to rewrap a line carrying one.
...
> ## N13-N17 AND N19 ARE DONE (2026-08-28, later the same day). N20 IS DRAFTED.
> **All TWELVE checkers green on both manuscripts. The bundle is rebuilt. Nothing was
> retrained and no greenlight was needed.**
>
> ### N13: THE D1 HAZARD DID NOT BITE, AND THE REASON IS REUSABLE.
> §4n's hazard box listed three routes and all three were bad: (a) regenerate = a training
> run needing a greenlight, (b) an apology clause the 5-page cut has no room for, (c) fig5,
> which is not what he asked for. **There is a FOURTH route and it is strictly better.**
> The `"state reconstruction"` title lives ONLY on the `_overview` composites. It is written
> by `src/experiment.jl:224`, which titles the stacked 3-panel figure. The **per-variable**
> panels are written by the loop at `src/experiment.jl:213-221` with the title
> `"<stem> : truth vs <model>"` — i.e. **"Membrane voltage : truth vs Neural ODE"**, which
> never contained the word. So `figures/fig2_neural_ode_voltage.png` and
> `figures/fig3_ude_voltage.png` are (i) exactly the trajectories he asked for, (ii) free of
> the D1 defect, (iii) already committed, (iv) co-generated with the overviews in the same
> call, so same run, same representative seed 1111, same baseline condition, same y-limits
> (both framed to the truth by `_ylims_data`). **Verified by reading the images, not by
> inferring it.** They are now Fig. 1, two panels at `0.47\textwidth`.
> **THE GENERAL LESSON: before paying for a figure defect, check whether the defect is in the
> composite or in the panel. This project has both, and only the composite was broken.**
>
> **A NODE trajectory cannot be replotted without retraining.** `results/params/` holds
> `ude_*.jld` only — there is NO saved neural-ODE parameter snapshot anywhere, so a combined
> single-axes NODE+UDE+truth panel is not reachable without a training run. Checked before
> designing the figure, not after.
>
> ### WHAT ELSE CHANGED
> - **N14.** Appendix A–D: model constants + kinetics + the physiological rescaling + the
>   parameter table; hyperparameters, both distillations, metric definitions and the silent
>   failure paths; compute and hardware; the window and noise ablations. Body sections stay
>   **1–5**, so the checklist's four `Sec.~N` cross-references survive — **verified, not
>   assumed.** 26 fixed-input macros were lifted from `paper/main.tex` **mechanically, not
>   retyped**, so `check_workshop_cut.py`'s byte-identical test cannot fail (22 → 56 shared).
> - **N15.** The availability paragraph is now `\paragraph{Data and code.}` and points at the
>   four appendices. Checklist Q4 and Q5 repointed. No live "supplementary" promise survives
>   in either file; the remaining hits are template boilerplate and build comments.
> - **N16.** Checklist sits between `\bibliography` and `\appendix`. **Q8 went No → Yes** on
>   the measured per-run cost. **All sixteen were walked**, and that found two more defects
>   nothing else would have: **Q2 attributed the matched-objective bound to Sec. 5 when it is
>   stated in Sec. 3.3**, and **Q12 attributed the parameter provenance to Sec. 2 when it is
>   now App. A**. Both pre-existed this restructure.
> - **N17.** `consistency.yaml` first, as the rule says — new group
>   **`headline-failure-is-practical-recovery`** (18 groups, 78 sites). It FAILED with
>   "probe matched 0 sites, expected at least 4" and then named every site as they were
>   fixed. His wording landed at **5** sites across both manuscripts, the README and the
>   ledger. **The functional-identifiability vocabulary is KEPT** — both sibling groups still
>   find 8 and 6 sites. Division of labour: his phrasing where the paper ASSERTS, the Loman
>   frame where it EXPLAINS.
> - **P1 upkeep, unasked but owed.** The restructure took the em-dash rate 1.6 → **3.3** per
>   1k words. Eight prose dashes became colons, commas, parentheses or full stops; it is now
>   **1.1**, better than before the session started. The `---` cells in Table 1 are
>   placeholders, not prose, and were left alone.
> - **The bundler's closing instructions were WRONG and are fixed.** It still said "compile
>   main.tex and supplementary.tex, then upload both PDFs to OpenReview". That is a
>   desk-reject instruction now. It says ONE pdf.
>
> ### THE OVERFLOW WAS REAL, AND IT TOOK TWO CUT PASSES. THE ESTIMATE FAILED TWICE.
> **Compile 4 (2026-08-28) put body lines 178-186 on PAGE 6.** The "+4 model lines" estimate
> was wrong by ~9 because it counted the Stage-1 figure's CAPTION but not its GRAPHIC: a
> float adds page space while adding ZERO numbered text lines, so the template's line
> numbers moved only 183 -> 186 and hid ~11 lines of cost.
> **Cut pass 1** (six caption/prose edits, ~12 source lines) took the overflow from 9
> typeset lines to **4** -- because source-line counting OVER-CREDITS caption cuts: caption
> source lines here are ~78 chars but set at ~86.
> **Cut pass 2** stopped modelling and MEASURED. Each block was extracted from the live
> file, expanded to its paragraph, `\citep` rendered to real unsrtnat numbers, and set in a
> pdflatex harness reproducing `neurips_2026.sty` (ptm 10pt on 11pt, textwidth 5.5in,
> abovedisplayskip = belowdisplayskip = 7pt). **One typeset line = 10.95pt.** Four edits,
> **6.89 measured lines**, against a 4-line overflow:
> - inline the displayed loss equation and both `\eqref{eq:loss}` (3.89) -- the biggest
>   single win in the paper, because a displayed equation costs ~3 lines of skip alone;
> - Limitations paragraph 1 (1.00), the retrain-control's rhetorical tail (1.00), and
>   Discussion paragraph 1 (1.00).
> Two further edits are **pre-validated in reserve** (2.00 more) in the LENGTH DIAL.
>
> **THE LESSON, and it is the reusable part: DO NOT ESTIMATE LENGTH IN SOURCE LINES.**
> Source lines mis-price captions (too generous), displayed equations (far too cheap) and
> floats (invisible). If length matters, measure typeset height in points.
>
> ### A DIAGNOSIS WAS MADE HERE AND IT WAS WRONG. THE CORRECTION IS THE USEFUL PART.
> **What was claimed:** that ~79 % of every line cut was being absorbed, that pages 1-5 held
> ~18.5 lines of slack, and that `lushbottom` plus `[t]`-only floats were stretching
> under-filled pages so cutting prose fed the padding. A `ottomfraction` override was
> added on that basis.
>
> **The compiled PDF disproved it two ways, and both errors are instructive.**
> 1. **The "slack" was an accounting error.** Re-doing the page map against the real compile
>    -- captions are `\small` so ~0.9 line each, section headings carry their own space, and
>    `\parskip` is non-zero -- puts page 3 at ~59 lines and page 4 at ~58 against a
>    58.9-line page. **Both are full. There was never any slack.**
> 2. **The cuts had been working perfectly linearly.** Body 186 -> 179 numbered lines;
>    spill 9 -> 2. Seven for seven. The "79 % absorbed" figure compared **measured typeset
>    lines (9.69) against numbered-line reduction (7)** -- different units, because inlining
>    the loss equation freed ~2.7 lines of display skip that never carried a line number.
>
> The override was reverted: it changed nothing observable (LaTeX tries `t` before `b`, and
> `t` succeeded for all three floats), and an unnecessary override of a template parameter is
> desk-reject surface for no benefit. The `[tb]` placements stay -- harmless and standard.
>
> **THE LESSON, and it is worth more than the original one: when a length cut does not move
> the page boundary, CHECK THE UNITS OF YOUR ACCOUNTING BEFORE INVENTING A MECHANISM.** The
> paper was simply two numbered lines too long, which is the boring explanation, and the
> four-line "Data and code." paragraph that was spilling got compressed to two.
>
> ### THE ORIGINAL (WRONG) DIAGNOSIS, KEPT SO IT IS NOT RE-DERIVED.
> **Cut pass 3 removed 2.80 measured lines and the page-5 boundary did not move at all.**
> That is the signal to STOP CUTTING AND ACCOUNT. Across passes 2 and 3, **9.69 typeset
> lines were removed and the spill went 4 -> 2: 79 % of every line cut was absorbed.**
> A capacity audit of pages 1-5 (5 x 58.9 lines minus both figures, the table, their
> captions, the title block, the abstract and nine headings) leaves room for ~195 body text
> lines while only **177** were placed. **~18.5 lines of slack. THE BODY WAS NEVER TOO
> LONG; THE FLOATS WERE PACKING BADLY.**
>
> `neurips_2026.sty:168` sets **`lushbottom`**, so an under-filled page is STRETCHED to
> look exactly full instead of pulling text up from the next one. All three body floats were
> **`[t]`-only**, so a float that missed the top of its page was deferred and the page it
> vacated was padded out by stretching `	extfloatsep`, `loatsep` and the heading skips.
> **Cutting prose fed the padding, not the page count.**
>
> **The fix, and it costs no content:** the three body floats are now `[tb]`, plus
> `
enewcommand{ottomfraction}{0.45}` in the preamble. That second line is not optional
> decoration -- `[tb]` is a NO-OP for `fig:ident` without it, because that float is ~42 % of
> a page against the template's `ottomfraction` of 0.4 and LaTeX would refuse it the
> bottom slot. **This does not deviate from the required template:** geometry, margins and
> font size are untouched, the two CFP-named lines are byte-identical, and
> `neurips_2026.sty:249-252` renews these same four float parameters itself.
> **TO REVERT:** delete the FLOAT PACKING block in the preamble and put the three `[tb]`
> back to `[t]`.
>
> **THE GENERAL RULE FOR NEXT TIME: when a length cut does not move the page boundary by
> roughly what it removed, the constraint is not length. Audit capacity before cutting
> again.** Three passes and ~10 lines of good prose were spent before anyone checked.

| # | Do this | Notes / hazards |
|---|---|---|
| ~~**N11**~~ | ~~Verify whether a supplementary upload exists.~~ **ANSWERED 2026-08-28 FROM THE LIVE FORM: it does NOT. One `PDF *` field, no supplementary field.** Site and deadline verified as ours. The restructure is now REQUIRED, not conditional. §4n has the full field inventory. | **Three NEW blockers the form surfaced: the required `License` field is unchosen; Raj has no known OpenReview ID; and it is unverified whether the appendix counts toward the five pages.** |
| ~~**N12**~~ | ~~Decide the author list.~~ **DECIDED 2026-08-28: Vinayak Mokashi, Prathamesh Dinesh Joshi, Raj Abhijit Dandekar.** Not Rajat, not Sreedath. | **Two things are coupled and were RAISED, not acted on (§4n): the archival lists FIVE, and D4's "four of this paper's five authors" arithmetic breaks at three. Do not change `paper/main.tex`'s byline without asking him.** Also: **Raj needs an OpenReview profile ID** — not recorded anywhere. |
...
| **N14** | **Build the appendix.** Hyperparameter table, reproducibility, **compute/hardware** (§4n has the measured per-run number), parameter table, remaining ablation details. | **The CFP puts NO page limit on the appendix** (§4n), so 4–5 pp is Prathamesh's taste, not a rule — good taste, but not a constraint. **The real constraint is the other half of that CFP sentence: the five-page body must be SELF-CONTAINED, and reviewers are not obliged to read the appendix.** Move detail out, never substance. | **He gave explicit permission to CUT, and the 4–5 page cap makes that load-bearing:** *"It is perfectly fine to omit some details from the longer draft; in fact, submitting a full-length paper draft as a supplement is generally not a good practice."* Do not try to cram the whole archival in. Ports from the archival's App A + App B and Table 1 — they are the right content at roughly the right length. **Compute/hardware is genuinely new and only Vinayak knows it** (see N16). |
...
| ~~**N18**~~ | ~~Check the footer question.~~ **CLOSED 2026-08-28: the CFP names `\usepackage[dblblindworkshop]{neurips_2026}` and `\workshoptitle{Sim2Science}` verbatim and we match byte-for-byte. It says nothing about footer text. Nothing to change.** | `\workshoptitle{Sim2Science}` is already set. **Do NOT pass `final` to make the footer name the workshop — `neurips_2026.sty:49-51` sets `\@anonymousfalse` and would print the real byline.** §4n's footer box has the full mechanism. |
| ~~**N19**~~ **DONE.** Re-audited; round 10 (§4o) then went further and audited all three documents plus mentor-ask coverage. | New prose is new risk; §4i's profile for fresh writing is "every finding was an error introduced in the writing". Do not re-audit the untouched body. |
| **N20** | **DRAFTED, NOT SENT** — `paper/draft-email-N9-prathamesh.md`, reconciled against what actually shipped. **Send the revised draft back to Prathamesh.** *"Let me know once you make these changes."* | He expects one more look before submission. `paper/draft-email-N9-prathamesh.md` is the previous note and shows the house style. |
...
| **A13** | **FIX THE BUNDLER: it DELETES the author-overlap disclosure instead of anonymising it.** Found 2026-08-29 (round 11 follow-up), measured by hand, not by an agent. | See the box below. Affects A10 (arXiv / any future blind venue), NOT the Sim2Science submission — `supplementary.tex` is not uploaded, so no reviewer sees it. **Do not fix under deadline pressure.** |

> ### A13. THE BUNDLER DROPS A DISCLOSURE RATHER THAN BLINDING IT (2026-08-29)
> **Measured, not asserted:** `paper/main.tex` and `sim2science_upload/supplementary.tex`
> differ in **10 hunks / 50 lines out of 2212 (2.3 %)**. Four of the ten are LaTeX comments
> and never print. The **six printed** deltas are: the `\author` block, `pdfauthor`, an added
> "Supplementary material" header line, the abstract's closing sentence, the Data and Code
> Availability body — and this one:
>
> | | |
> |---|---|
> | `main.tex:489` | "--- though **that work shares two of this paper's three authors**, so we note the agreement rather than lean on it as independent ---" |
> | `supplementary.tex:484` | "--- though we note the agreement rather than lean on it as independent ---" |
>
> The substitution deletes the REASON and keeps the caution, so the anonymised document tells
> a reader we decline to treat `silvestri2023analysis`'s corroboration as independent and
> never says why. **Stripping "two of this paper's three authors" is correct** — with the
> citation it identifies the byline. **Deleting the whole clause is not.** The conventional
> blind phrasing discloses without the arithmetic: *"though that work shares authors with the
> present submission"*. Fix `SELF_ID_CLAUSE` in `scripts/make_sim2science_bundle.py` to
> REWRITE to that, not to excise.
>
> **THE GENERAL LESSON, and it is why this was missed for four rounds:** an anonymiser that
> only ever DELETES will silently turn a disclosure into a non-disclosure. Every
> identity-stripping substitution needs asking not just "is the name gone?" but "does the
> sentence still do the job it was written to do?" The bundler's own guard rail — it refuses
> to write on any surviving identifying string — is a deletion test, so it can never catch
> this class. Round 11's supplementary agent raised something adjacent and the refuter killed
> it on the ground that the file is not submitted: **a deadline argument, not a correctness
> one, which is exactly how this class kept slipping through.**
...
Rewritten 2026-08-29 (eighth time). The previous prompt sent a session to BUILD the
restructure; that is finished and the paper fits. **This one sends a session to AUDIT what
was built and then deliver it.** Copy everything between the fences into a fresh session.
...
READ HANDOFF.md FIRST, in this order:
  Sec 4o  - STATE AS OF 29 AUG. THE LIVE SECTION. What is done, what remains.
            It supersedes the planning in Sec 4n.
  Sec 13  - the to-do list, and the round-10 audit box inside it.
  Sec 8   - the THIRTEEN checks across twelve scripts, and the ONE thing they
            still do not verify.
  Sec 9   - gotchas. Non-optional; several cost a whole round when ignored.
  Sec 3   - current numbers AND the boxed DOMAIN RULE.
  Sec 4l  - why the framing (N0) is option (C). Do not reopen it.

--- THIS SESSION'S GOALS, IN ORDER ---
1. A THOROUGH AUDIT OF ALL THREE DRAFTS, each read SOLO first:
     A. paper/main.tex                        the archival, full length
     B. paper/sim2science/main.tex            THE SUBMISSION (5 pp + refs +
                                              checklist + appendix A-E)
     C. sim2science_upload/supplementary.tex  GENERATED from A, no longer
                                              submitted, still the cross-check
                                              target and the archival's own
                                              anonymisation route
2. THEN a CROSS-DOCUMENT consistency pass over all three, so nothing is left to
   bite us later.
3. THEN the remaining to-dos in Sec 4o (they are errands and delivery, not
   writing).

--- STATE ---
- DEADLINE: 2 Sep 2026 23:59 AoE = 3 Sep 17:29 IST. VERIFIED by me on the CFP.
  The earlier 30 Aug date on the OpenReview form is superseded; work to 2 Sep.
- THE SCIENCE IS DONE AND THE EVIDENCE BASE IS CLOSED. Ten audit rounds. All
  checkers green. NOTHING needs a new experiment. Do not propose one.
- THE PAPER FITS AND THE MARGIN IS ZERO. Body ends at the foot of page 5,
  References at the top of page 6. Pages 3, 4 and 5 are ALL FULL. One added
  sentence puts the body back onto page 6. If an audit finding needs a line,
  the LENGTH DIAL at the top of paper/sim2science/main.tex says what to spend.
  EVERY finding you propose must state whether its fix ADDS A LINE TO THE BODY.
  Appendix and checklist fixes are FREE - the CFP puts no limit on either.
- I compile on Overleaf; you get 403 and can NEVER measure a page count. Ask me
  and I will paste the PDF back. NEVER estimate length in source lines: they
  mis-price captions, make displayed equations look free and make float
  graphics invisible. Count words per printed line off the last compile.
- Prathamesh (co-author, mentor) is happy with the draft and wants ONE more look
  before we submit. Everything he asked for in review 2 is DONE (Sec 4o).

--- HOW TO RUN THE AUDIT, because a previous attempt DIED doing it wrong ---
- Give each agent ONE NARROW CONCERN and a tool-call budget (~12). A round with
  four "read this whole manuscript cover to cover" agents burned 35 minutes and
  returned NOTHING - they read themselves to death and nothing was cached.
- Tell every agent to RETURN PARTIAL FINDINGS rather than read exhaustively.
- Do SOLO audits first, then cross-document. The cross round should be given the
  solo findings so it does not re-report them.
- ADVERSARIALLY REFUTE every finding before believing it. Round 10 refuted FIVE
  of nine, several because they proposed narrowing a qualifier claims.yaml
  explicitly forbids narrowing. Default to refuted when uncertain: a false
  finding costs more than a missed one this close to the deadline.
- Do NOT report anything a checker already catches - they are all green, so by
  definition it is not there. Report what checkers CANNOT see: a sentence that
  disagrees with its evidence, a hedge lost in compression, a stale
  cross-reference, a claim stated at two different sizes in two documents.
- READ claims.yaml's `scope` fields before calling anything over-claimed, and
  search HANDOFF.md before calling anything a mistake. This project documents
  its decisions heavily and several look like bugs until you read why.
- A subagent CAN fabricate a computed result. Check the INPUT exists before
  believing the output.

--- WHAT ROUND 10 ALREADY FOUND AND FIXED. DO NOT RE-REPORT THESE. ---
  D1 no hyperparameter table -> tab:hyper added (he asked for it by name)
  D2 the appendix shipped ZERO figures -> App. E now carries the forecast
     overview and the voltage-only overview
  D3 checklist Q8 hand-duplicated App. C's numbers -> Q8 now points at App. C
  D4 App. C claimed "a consumer laptop" AND "a shared desktop"; BOTH were
     invented. Recorded hardware is "CPU only, no GPU, 16 GB RAM" and no form
     factor. Fixed in both places.
  Verdict was: the three documents ARE consistent. Your job is to try harder.

--- THE REMAINING TO-DOS (Sec 4o has the detail) ---
N20. SEND the revised draft to Prathamesh. Drafted and reconciled at
     paper/draft-email-N9-prathamesh.md. It asks for Raj's OpenReview ID and
     warns him the reciprocal-reviewer role costs two reviews.
BLOCKER. Raj Abhijit Dandekar's OpenReview profile ID. The form refuses authors
     without one. This is the ONLY thing that stops Submit.
N10. Submit: remove Vinayak from the reciprocal-reviewer box, add
     ~Prathamesh_Dinesh_Joshi1, licence CC BY 4.0, track "Workshop Paper",
     paste the RENDERED abstract (not the \val source), upload ONE pdf.
F3.  After upload: git tag -a sim2science-submitted on the commit whose bundle
     produced the UPLOADED pdf - not a later one - and push the tag.
F4.  Add the FROZEN banner to paper/sim2science/main.tex.
Post-deadline: A11, P1-P9, A10, A12, D3, T4-T8.

--- WHAT WE SUBMIT (get this wrong = desk reject) ---
  ONE pdf. NOT two. The form has a single `PDF *` field and no supplementary
  slot - verified on the live form. Body <= 5 pages + references + checklist +
  appendix A-E, all in that one file.
  ...
  NEVER upload a pdf built from paper/main.tex: it carries the real byline.
  supplementary.tex is still GENERATED but is NOT submitted. Do not delete that
  path: it is the cross-check target and the archival's anonymisation route.
...
- Law I: every reported number is a \val macro from paper/generated/metrics.tex,
  ...
  \newcommands with a source comment. Literals need "% numok:" ON THE SAME LINE
  as the literal, at the TRUE END of that line, with a space before the %.
- Law II: every claim lives in paper/claims.yaml, anchored as "% claim: <id>".
  The `scope` fields are where over-claiming dies; read them before judging.
- The supplementary is GENERATED by scripts/make_sim2science_bundle.py, which
  REFUSES to write if any identifying string or TODO- placeholder survives.
  Never hand-maintain a second copy; fix the BUNDLER.
- The two preambles share fixed-input macros; check_workshop_cut.py FAILS if a
  shared macro has a different body. Keep them byte-identical, and lift them
  MECHANICALLY rather than retyping.
- When you change a claim: update paper/consistency.yaml FIRST, then run
  check_consistency.py and let it name EVERY site in BOTH manuscripts. Fixing
  the site I point at and not its siblings is this project's single most common
  failure mode. A group whose `require` is an OR is a FLOOR, not a ceiling.

--- VERIFICATION: run ALL of these after ANY edit (Sec 8) ---
  python "Paper Writing Skills/scripts/extract_metrics.py" --map paper/metrics_map.yaml --results-root results --out paper/generated/metrics.tex --json paper/generated/metrics.json --check
  python "Paper Writing Skills/scripts/check_numbers.py" paper/main.tex --macros paper/generated/metrics.tex
  python "Paper Writing Skills/scripts/check_numbers.py" paper/sim2science/main.tex --macros paper/generated/metrics.tex
  python "Paper Writing Skills/scripts/check_claims.py" --claims paper/claims.yaml --tex paper/main.tex paper/sim2science/main.tex --metrics-tex paper/generated/metrics.tex --metrics-json paper/generated/metrics.json --figures-root .
  python "Paper Writing Skills/scripts/lint_prose.py" paper/main.tex
  python "Paper Writing Skills/scripts/lint_prose.py" paper/sim2science/main.tex
  python scripts/verify_references.py
  python scripts/check_tex_sanity.py
  python scripts/check_tex_sanity.py paper/sim2science/main.tex
  python scripts/check_bib_sanity.py
  python scripts/check_provenance_refs.py
  python scripts/check_consistency.py
  python scripts/check_workshop_cut.py
  python scripts/check_undefined_macros.py
  python scripts/make_sim2science_bundle.py
  python scripts/check_undefined_macros.py sim2science_upload/main.tex
EXPECTED: 322/322 macros - check_numbers OK on both - check_claims 0 errors
  (it takes BOTH --tex files; pointed at the cut alone it emits 13 spurious
  errors) - lint_prose clean on both - 36/38 refs verified, 2 review - tex
  sanity clean - bib sanity clean - provenance clean with 12 expected WARNs -
  consistency 78 sites / 18 groups - workshop_cut 56 shared macros / 47 \val /
  32 cite keys - undefined_macros clean on both the source and the bundle.
THE ONE THING STILL UNVERIFIED BY ANY CHECKER: paper/sim2science/checklist.tex.
  It ships INSIDE the submitted pdf and nothing reads it. Walk it by hand.

--- HARD-WON GOTCHAS ---
- NEVER write LaTeX or regex through a shell heredoc, and never through
  `python -c` either. Backslashes get eaten. Write the script to disk with the
  Write tool, then run it. This bit three times in one session.
- consistency.yaml `require` regexes must use \s+ between words, never a literal
  space: the checker joins context with ' '.join().
- A "% numok:" must be on the SAME LINE as the literal it exempts, at the TRUE
  end of that line, with a SPACE before the %. Rewrapping a line can silently
  orphan one. TeX also eats the newline after a comment, so that space is the
  only thing separating the last word of the line from the first of the next.
...
- Never run `git checkout --` on paper/sim2science/main.tex: older commits hold
  a 552-line version against the current ~1000.
- `git add -u` stages tracked files ONLY. New scripts and figures need `git add -A`
  or an explicit path. Every never-commit path is already in .gitignore, so
  `git add -A` is safe.

--- GROUND RULES ---
- I am the committer. NO Claude co-author line in commit messages.
- You may commit and push when I ask, not by default.
- NEVER commit: main.jl, Main_Clean.jl, "Paper Writing Skills/", .claude/,
  ...
  All are gitignored already.
- HANDOFF.md is gitignored and snapshot-only; archive it with
  powershell -ExecutionPolicy Bypass -File scripts\snapshot_handoff.ps1 -Push
```
