...
>
> ### THE NEXT ACTION, IF YOU ONLY DO ONE THING — UPDATED 2026-09-11 (evening)
> **Nothing is running and nothing is half-done. E1–E4 are CLOSED and W1 IS WRITTEN.**
> `paper/main.tex` now carries **§4.8, "What the spread is made of: objective, optimiser, and
> seed"** (commit `aea9647`), plus corrections to the three passages the new results falsified.
> All twelve checks are green and **the submitted workshop paper is untouched — zero printing
> lines changed against the tag.**
>
> **THE NEXT ACTION IS A CO-AUTHOR CONVERSATION, NOT A COMMAND.** The abstract, introduction
> and discussion still frame the finding as the *pre-round* evidence supports. Rewriting the
> paper's headline argument needs Prathamesh, who has not seen any of this. **Do not do it
> from a session.** See "WHAT W1 DELIBERATELY DID NOT DO" in the ★ RESULTS box.
>
...
> | representation effect | 3.84× (both arms n=5) | **4.68×** (numerator AND denominator n=28) |
...
>
> ### WHAT ALL THIS DOES TO THE PAPER'S STORY
> The thesis moves from *"flexible closures lose identifiability"* (a caution) to a
> mechanism plus a partial prescription: **the loss you choose creates an interaction between
> initialisation and noise; the likelihood removes it; what remains, 4.68×, is representation,
> and the optimiser is measurably not the culprit at the end where it can be checked.**
> **`paper/main.tex` says none of this yet, and E1 CONTRADICTS a spread it already prints.**
>
...
> **The ABSTRACT, INTRODUCTION and DISCUSSION still argue the pre-round story.** §4.8 reports
> the new results; it does not rewrite the paper's thesis around them. That is a co-author
> decision — see W2 in the work queue. **Do not do it from a session.**
>
> ### THE FILES
> | what | where |
> |---|---|
...
>
> **W2. THE FRAMING PASS — NEEDS PRATHAMESH, NOT A SESSION.**
> §4.8 reports the new results, but the ABSTRACT, INTRODUCTION and DISCUSSION still argue the
> pre-round story. The honest new framing is *"the loss you choose creates an interaction
> between initialisation and noise; the likelihood removes it; what remains is
> representation"* — that is a change to the paper's thesis, and this project has a scar from
> exactly that kind of change (§4k, the thesis reopened by a mentor video). **Send him the
> ★ RESULTS box first. Do not redraft the argument from a session.**
>
...
> implied, so the old decomposition was conservative.** The optimiser is still not isolated —
