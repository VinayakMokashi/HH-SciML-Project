...
> **§4k — the mentor's video review — is the newest and most important section, and it
> REOPENS the paper's framing.** Read it before §2, before §13, before touching anything.
> Then make the **N0 decision in §13**; the 5-page Sim2Science paper cannot be drafted
> around an undecided frame.
>
> **Deadline: 30 Aug 17:29 IST (= 29 Aug 23:59 AoE).**
>
> Note the §4x sections run NEWEST FIRST: 4k (mentor video) → 4j (figure audit) → 4i (prose
> audit) → 4h (venue) → 4g…4b (earlier rounds). §4–4g are chronological history below them.
...
**Figures and tables**
- **Table captions belong BELOW the table**, with the table at the top of the page. Ours are
  above. This applies to the archival paper too and is a mechanical fix across all 4 tables.
...
but it affects how much polish A1-A8 deserves.

### 4. AMBIGUOUS — ask him before acting
- At 12:39, looking at the figures: *"You definitely think of which trade-off bound you
  wanted to add."* Most likely the $g_{Ca}$–$E_{Ca}$ trade-off, which Fig. 7(a)'s loss
  surface already shows and which the profile-likelihood section discusses — but "bound" is
  not a word the paper uses there, and it could equally mean the representation-share upper
  bound. **Do not guess and implement; it is one question in a reply.**
...
about.**
- `paper/consistency.yaml` now lists `paper/sim2science/main.tex` in **all 15 groups**, so
...
```
> **2026-08-26: `check_workshop_cut.py` and `make_sim2science_bundle.py` are DORMANT, not
> obsolete.** The 5-page cut was deleted for a redraft, so both print a SKIP and exit 0.
> They re-arm automatically the moment `paper/sim2science/main.tex` exists again, and
> `consistency.yaml` still lists that path in all 15 groups for the same reason. **Do not
> delete either script or those 15 entries** — the redraft needs them on day one.
> Recover the old draft for reference: `git show bfc8768:paper/sim2science/main.tex`.

Current state (2026-08-22, after the round-6 audit and its fixes): **all TEN green —
extract_metrics 322/322 macros match disk · check_numbers OK on both manuscripts (0 flagged)
· check_claims 0 errors · lint_prose clean · verify_references 38 refs, 36 verified, 2 review
· tex_sanity clean on both · bib_sanity clean · check_provenance_refs clean · check_consistency
58 sites across 15 groups agree · check_workshop_cut 18 shared macros agree.**
Both upload bundles rebuilt after the fixes. No duplicate `\newcommand` in either manuscript. `doctor.py` reports 3 FAILs; those are the broken local MiKTeX and are expected — compilation is Overleaf-only.
...
**Paper state (2026-08-26):** all TEN checkers green, 0 unsupported claims, 322 macros, 38
refs verified. Seven adversarial audit rounds; the last two are §4i and §4j.
...
### Do this first
**Read §4k, then make the N0 framing decision.** The 5-page paper cannot be drafted around
an undecided frame — its title, abstract, contributions and section order all follow from it.

### Waiting on someone else
1. **Prathamesh's reviewer ID**, so he can be nominated as the reciprocal reviewer (N10). He
   volunteered in the video.
2. **His second round of feedback on the archival paper**, which he will send after
   Sim2Science.
...
|---|---|---|---|
| **N0** | **DECIDE THE FRAMING.** (A) keep the reversal as thesis, retitle and reflow; or (B) rebalance so HH/NODE/UDE forecasting is the spine and identifiability is a section. | nothing | §4k lays out both. He left it to you and did not overrule. |
| **N1** | **Retitle.** The current title was called out as reading AI-generated, and (A) requires a new title anyway. | N0 | |
| N2 | Draft the 5 pages into `paper/sim2science/main.tex`. Template assets are already vendored there. | N0, N1 | Structure per §4k: abstract 14-15 lines; intro exactly 4 paragraphs ending in contributions; related work <=2 paragraphs; methods dataset->preprocessing->NODE->UDE->symbolic; discussion+conclusion one section; limitations+future work one; <=2 paragraphs each. |
| N3 | Contributions: one headline + **two lines**. No bullet dump, no numbers. | N2 | |
| N4 | Figures for the main body: the NODE/UDE forecast plot **and one symbolic-recovery graph**. **Only ONE ablation case**, not all three. | N2 | Table captions BELOW tables; bold the best value per metric column; legends must name the colours. |
| N4b | **Add a data-generation subsection** to Methods, and **keep the training details** — he called them important. | N2 | We have all of it; it is currently scattered rather than a named step. |
| N4c | **Cite the source paper for the printed equations**, justify anything we added, and do a notation pass. | N2 | Includes the Table 1 caption's $m^2h$ collision with the sodium gates. |
| N5 | **AI-tell pass.** Kill the named offenders and their siblings: "known physics versus hidden current", "the identifiability vocabulary ... parallel line of work", "we should say plainly ...". | N2 | The risk he named is that a reviewer concludes the paper was agent-written. |
| N6 | Fill `checklist.tex` with justifications; code answer = reproducible from the details, **full codebase released upon acceptance**. Then `\input{checklist}` before `\end{document}`. | N2 | Does not count toward the page limit. |
| N7 | **NO repo URL, no names, no affiliations.** | N2 | He was explicit: avoid the GitHub link entirely for this submission. |
| N8 | `python scripts/make_sim2science_bundle.py`, compile on a NEW Overleaf project, **measure the page count**. | N2-N7 | The bundler and `check_workshop_cut.py` are dormant and re-arm automatically once `main.tex` exists again. |
| N9 | Send the 5-page draft to Prathamesh to finalise. | N8 | He asked for this explicitly. |
| N10 | **Nominate Prathamesh as reciprocal reviewer** (he volunteered; get his ID) and submit at `NeurIPS.cc/2026/Workshop/Sim2Sci`. | N9 | All authors listed at submission; none can be added after review opens. |
...
| # | Do this | Notes |
|---|---|---|
| A1 | **Move all 4 table captions BELOW their tables**, tables at the top of the page. | Mechanical; applies to `tab:params`, `tab:forecast`, `tab:parametric`, `tab:retrain`. |
| A2 | **Bold the best value in each metric column**, direction-aware. | |
| A3 | **Fig. 3 legend must name what blue and green are.** | |
| A4 | Plain ablation figure titles: "ablation: calcium, training window, noise". | |
| A5 | Shorten Discussion/Conclusion/Limitations to the §4k shape (two sections, <=2 paragraphs each). | Also flagged for the archival version. |
| A6 | AI-tell pass over the full manuscript. | Same offenders as N5. |
| A7 | Reconsider **paperbanana** for the Fig. 1 schematic. | §4k explains why the §4 refusal does not apply to a non-data diagram. |
| A8 | **Shrink the 6-page appendix by reducing figure sizes**, not by cutting content. | His suggested route. |
| A9 | **Cite the equations' source, justify our additions, notation pass** (incl. Table 1's $m^2h$ vs the sodium gates). | Same three requirements as N4c, applied to the archival paper. |
| A10 | **Ask him what "which trade-off bound you wanted to add" meant** (§4k item 4). | One line in a reply; do not guess. |
| A11 | Decide the destination: arXiv preprint, journal, or both. | He raised the journal route explicitly. |
| A12 | Second round of mentor feedback on the big paper. | He will send it after Sim2Science. |
...
`check_numbers.py` is line-based. Append a `% numok:` at the TRUE END of every line that
then carries a digit — mid-line, it silently deletes the rest of the line.
...
Rewritten 2026-08-26 after the mentor's video (§4k) and the deletion of the 5-page cut.
...
THIS SESSION I WANT TO: <FILL IN — most likely: draft the 5-page Sim2Science paper>

--- THE SITUATION IN FIVE LINES ---
- DEADLINE: Sim2Science @ NeurIPS 2026, 30 Aug 17:29 IST (= 29 Aug 23:59 AoE).
- My mentor sent a VIDEO review on 2026-08-26. It REOPENED the paper's framing.
  HANDOFF Sec 4k has the whole thing. Read it before anything else.
- The old 5-page draft was DELETED on purpose. We rewrite from scratch.
- The full archival paper is DONE, compiled, read, and ten checkers green.
  It needs a finishing pass (Sec 13, A-series), not surgery.
- There is a DECISION to make before any drafting: Sec 13, item N0.

--- READ FIRST, in this order ---
1. HANDOFF.md:
     Sec 4k  - THE MENTOR'S VIDEO. Start here. It reopens the framing, sets the
               Sim2Science rules, and lists the AI-tells he caught by name.
     Sec 2   - THE THESIS, with the contested-framing banner at the top. The
               science is NOT retracted; only the framing is open.
     Sec 13  - N0-N10 (the 5-page paper), A1-A8 (archival finishing pass),
               D1/D3/D4 open decisions. D2 is SKIPPED, do not reopen it.
     Sec 3   - current numbers AND the boxed DOMAIN RULE.
     Sec 4g  - why the ninth checker exists. Read before editing ANY claim.
     Sec 4i  - round 6 audit. Read the boxed caveat about the rate.
     Sec 4j  - round 7 audit. A refutation pass that returns ZERO is a smell.
     Sec 8   - the TEN verification commands. Two are dormant, see the note.
     Sec 9   - gotchas. Non-optional.
     Sec 7   - the Overleaf upload procedure and its table of traps.
2. paper/consistency.yaml - 15 groups, the repeated-claim ledger.
3. Skim paper/main.tex (2181 lines).

--- THE DECISION THAT GATES EVERYTHING (N0) ---
The mentor is against building the whole paper around the reverse claim, on
POSITIONAL grounds: Vizuara already published "SciML in the wild, why it hurts"
and an earlier stress-test paper, so a pure negative result reads as house style.
He explicitly left the call to me and did not overrule. Two live options:
  (A) Keep the reversal as the thesis - then the title AND the entire flow change.
  (B) Rebalance: HH/NODE/UDE forecasting is the spine, identifiability and the
      symbolic failure become a strong section rather than the frame.
His technical argument is that a different symbolic algorithm might recover it.
The honest reply is in Sec 4k and it is PARTIAL: our sanity fit on the TRUE
current returns a = 2.0000 exactly, so the regression is exonerated and the
scatter is upstream in the closure - but we never ran a different symbolic method
ON the learned closure, so that specific door is untested.
Do not decide this for me. Lay out the consequences and ask.

--- THE 5-PAGE PAPER: HARD RULES FROM THE MENTOR AND THE CFP ---
- Official NeurIPS 2026 template ONLY. Already vendored at
  paper/sim2science/{neurips_2026.sty,checklist.tex} with provenance headers.
  Use \usepackage[dblblindworkshop]{neurips_2026} + \workshoptitle{Sim2Science}.
  [sglblindworkshop] would silently turn anonymity OFF - do not use it.
- 5 pages excluding references. Appendix allowed but 4-5 pages MAX and NOT a
  dumping ground to make the body fit.
- DOUBLE-BLIND: no names, no affiliations, and NO GITHUB LINK AT ALL. He was
  explicit about the repo link.
- Abstract 14-15 lines, few numbers: problem (start from HH) -> traditional
  approaches -> what we did -> results qualitative then quantitative -> close.
- Introduction exactly 4 paragraphs, the last being contributions.
- Contributions: one headline + TWO LINES. No bullet dump, no results summary.
- Related work: 2 paragraphs max. No results in intro or related work.
- Methods order: dataset -> preprocessing -> neural ODE -> UDE -> symbolic.
  Cite UDE/SINDy and give 1-2 lines; do NOT explain them from first principles.
- Discussion+Conclusion = ONE section. Limitations+Future Work = ONE. <=2 paras each.
- Main body must carry the NODE/UDE forecast plot AND one symbolic-recovery graph.
- Table captions BELOW the table. Bold the best value per metric column.
- checklist.tex: fill with justifications. Code answer = reproducible from the
  details given, full codebase released upon acceptance.

--- THE AI-TELL PROBLEM (he named these; treat it as a hard requirement) ---
He flagged the TITLE, the subheading "known physics versus hidden current", and
sentences like "the identifiability vocabulary we use comes from a parallel line
of work" and "we should say plainly what our protocol does not do". His concern is
reputational: a reviewer concluding the paper was agent-written. Write plainly, own
every sentence, vary the rhythm, and do not over-exaggerate.
...
- Law I: every reported number is a \val macro from paper/generated/metrics.tex,
  ...
  \newcommands with a source comment. Literals need "% numok:" at the TRUE END of
  the line, with a space before the %.
- Law II: every claim lives in paper/claims.yaml, anchored as "% claim: <id>".
- consistency.yaml still lists paper/sim2science/main.tex in all 15 groups, and
  check_workshop_cut.py / make_sim2science_bundle.py are DORMANT, not obsolete.
  They re-arm the moment that file exists again. Do not delete them.
- The supplementary is GENERATED from paper/main.tex by
  scripts/make_sim2science_bundle.py, which anonymises at build time and REFUSES
  to write if any identifying string survives. Never hand-maintain a second copy.

--- THE ONE RULE THAT MATTERS MOST ---
When you change a claim: update paper/consistency.yaml FIRST, then run
`python scripts/check_consistency.py` and let it name EVERY site you still have to
touch, in BOTH manuscripts. Fixing the site I point at and not its siblings is the
single most common failure mode in this project's history.

--- HARD-WON GOTCHAS ---
- NEVER write LaTeX or regex through a shell heredoc. Backslashes get eaten.
  Write the file to disk with the Write tool.
- Editing ANY .jl file shifts provenance pointers. Keep the LINE COUNT identical
  and re-run check_provenance_refs.py.
- Bulk-applying patches causes damage every checker passes. Apply in ONE pass,
  then scan for duplicate \newcommand and repeated blocks.
- A confirmed audit finding does NOT mean its proposed fix is correct. Verify the
  remedy separately - one was wrong in round 6, and one refutation was wrong in
  round 7.
- The code says "state propagation"; the committed PNGs still say "state
  reconstruction". Deliberate and temporary - Sec 4j, decision D1.
...
- Evidence base is CLOSED unless I greenlight a run.
- I compile on Overleaf (you get 403).
```
