...
>
> ### THE LENGTH INVARIANT — CHECK THIS BEFORE AND AFTER ANY EDIT
> The body ends at the foot of page 5 and the References head opens page 6. That is the target
> state, measured on the 2026-08-28 compile. **THE MARGIN IS ZERO: one added sentence pushes
> the body onto page 6.** Every round-11 and round-12 fix was deliberately confined to the
> appendix, the checklist or an email, so **lines 1–770 of `paper/sim2science/main.tex` are
> byte-identical to that compile.** Verify with:
> `diff <(git show <compile-commit>:paper/sim2science/main.tex | sed -n '1,770p') <(sed -n '1,770p' paper/sim2science/main.tex)`
> Nobody in a Claude session can compile (Overleaf 403s), so **never estimate length in source
> lines** — they mis-price captions, displayed equations and float graphics. The LENGTH DIAL at
> the top of the cut says how to size an edit and what each remaining lever costs.
...
Q8 went 153 → 45 words, Q7 774 → 417 chars, Q5 now opens with his own phrasing. **Every
...
**1. N20 — SEND THE DRAFT TO PRATHAMESH. This is the critical path, because it is the only
route to the blocker.** Draft: `paper/draft-email-N9-prathamesh.md`, re-reconciled 2026-08-29
after rounds 11 and 12. It has a four-item pre-send check at the top — read it.
  - **Rebuild and recompile first.** `python scripts/make_sim2science_bundle.py`, then compile
    `sim2science_upload/main.tex` on Overleaf. **Any PDF built before 2026-08-29 night is
    superseded.** Attach that PDF.
  - **Confirm on the compile that the References head still opens page 6.** It should: the
    body is byte-identical to the 28 Aug compile (see the START HERE box for the exact `diff`).
  - The email asks for Raj's ID, warns that the reciprocal-reviewer role costs **two** reviews,
    and tells him what moved since his review so he does not discover it.
