...
Rewritten 2026-08-26 (third time), after the N0 decision was made, reversed, and remade as
option (C), and after the 5-page paper was drafted, compiled twice and verified. **Everything
above N9 is done.** Copy everything between the fences into a fresh session and fill in one
line.
...
- The 5-page paper is DRAFTED, COMPILED and FITS. All ELEVEN checkers green on
  both manuscripts. sim2science_upload/ is built and anonymity-verified.
- The framing question (N0) is CLOSED: option (C), an audit of the two-stage
  predict-then-interpret recipe. Do NOT reopen it without reading Sec 4l first.
- The archival paper is DONE and needs only the P1-P9 finishing pass, AFTER the
  workshop deadline.
- ONLY TWO THINGS ARE LEFT, and neither is analysis:
    N9  send the 5-page draft to Prathamesh (he asked; he will finalise it)
    N10 get his reviewer ID, nominate him, submit both PDFs
- Evidence base is CLOSED unless I greenlight a run. I compile on Overleaf; you
  get 403, so you can never measure a page count yourself.

--- READ FIRST, in this order ---
1. HANDOFF.md:
     Sec 4l  - THE LIVE SECTION. The N0 decision, why option (A) was drafted and
               then abandoned, the figure arithmetic, the first-compile failure.
     Sec 13  - BINDING vs ADVICE, P1-P9 (both papers), N0-N10 (N9/N10 are all
               that remain), A10-A12 (archival only), D1/D3/D4. D2 is SKIPPED.
     Sec 4k  - the mentor's video, all 64 points. HISTORY NOW, not a to-do list.
               Two of its premises are WRONG - Sec 4l says which and why.
     Sec 3   - current numbers AND the boxed DOMAIN RULE.
     Sec 8   - the ELEVEN verification commands, the current green state, and
               the note on the TWELFTH checker that does not exist yet.
...
3. Skim paper/sim2science/main.tex - read its PREAMBLE COMMENTS first, they carry
   the framing rationale, the exhibit arithmetic and the LENGTH DIAL.
...
- The two preambles share fixed-input macros and check_workshop_cut.py FAILS if
  any shared macro has a different body. Keep them byte-identical.

--- LENGTH: THE ONLY GAUGE THAT WORKS ---
You cannot compile. Do NOT trust a page-count estimate - mine was half a page
pessimistic. The gauge is WHERE THE REFERENCES HEAD LANDS in the compiled PDF:
page 5 = the body fits, page 6 = the body is six pages. As of the last compile it
sits ~75-80% down page 5. The LENGTH DIAL in the preamble of
paper/sim2science/main.tex says exactly what to add or drop, in order, and says
what must NEVER be traded for a figure.

--- HARD-WON GOTCHAS ---
- NEVER write LaTeX or regex through a shell heredoc, and never through
  `python -c` either. Backslashes get eaten in transit - it happened three times
  in one session. Write the script or the patch to disk with the Write tool.
...
- NOTHING verifies that a command you use is DEFINED. Eleven green checkers
  passed a document that would not compile (missing xcolor). See Sec 8.
...
- A confirmed audit finding does NOT mean its proposed fix is correct, and a
  refutation is not proof either.
